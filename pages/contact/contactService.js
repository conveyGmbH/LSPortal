    ...
    WinJS.Namespace.define("Contact", {
        ...
        contactNoteView: {
            select: function (complete, error, restriction) {
                ...
                var ret = Contact.getContactNoteView().select(complete, error, restriction);
                // this will return a promise to controller
                Log.ret(Log.l.trace);
                return ret;
            },
            update: function (complete, error, recordId, viewResponse) {
                Log.call(Log.l.trace, namespaceName + ".contactView.", "recordId=" + recordId);
                var ret = Contact.getContactNoteView().update(complete, error, recordId, viewResponse);
                Log.ret(Log.l.trace);
                return ret;
            },
            insert: function (complete, error, viewResponse) {
                Log.call(Log.l.trace, "wavSketchView.");
                var ret = Contact.getContactNoteView().insert(complete, error, viewResponse);
                Log.ret(Log.l.trace);
                return ret;
            },
            defaultValue: {
