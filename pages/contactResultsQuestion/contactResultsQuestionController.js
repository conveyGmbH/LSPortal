...
/// <reference path="~/www/pages/contactResultsQuestion/contactResultsQuestionService.js" />
    ...
    WinJS.Namespace.define("ContactResultsQuestion", {
        Controller: WinJS.Class.derive(Application.Controller, function Controller(pageElement, commandList) {
            ...
            var resultConverter = function (item, index) {
                ...
                item.textarea = getResourceText("ContactResultsQuestion.textarea");
                    ...
                    if (question &&
                        ...
                        if (curScope.type.substr(0,5) === "multi") {
                            ...
                            if (newRecord[prop] && newRecord[prop].length > 0 && newRecord[prop] !== "0" && newRecord[prop] !== "00") {
                                Log.call(Log.l.u1, "ContactResultsQuestion.Controller. Answer not empty" + newRecord.prop);
                                ret = false;
                            }
                            if (ret) {
                                break;
            ...
            var insertCameradata = function (imageData, width, height) {
                ...
                var ret = new WinJS.Promise.as().then(function () {
                    ...
                    return ContactResultsQuestion.questionnaireDocView.insert(function (json) {
                    ...
                    if (curScope) {
                        ...
                        switch (id) {
                            ...
                            case "useDateCombobox":
                                ...
                                Log.print(Log.l.trace, "save changes of recordId:" + recordId);
                                ContactResultsQuestion.questionnaireView.update(function (response) {
                                    // called asynchronously if ok
                    ...
                    if (eventInfo && eventInfo.detail) {
                        ...
                        if (visible) {
                            if (that.questions && that.nextUrl) {
                                ...
                                AppData.setErrorMsg(that.binding);
                                Log.print(Log.l.trace, "calling select ContactResultsQuestion.questionnaireView...");
                                var nextUrl = that.nextUrl;
                                that.nextUrl = null;
                                ContactResultsQuestion.questionnaireView.selectNext(function (json) {
                                    // this callback will be called asynchronously
                                    // when the response is available
                                    Log.print(Log.l.trace, "Questionnaire.questionnaireView: success!");
                                    // startContact returns object already parsed from json file in response
                                    if (json && json.d) {
                                        that.nextUrl = ContactResultsQuestion.questionnaireView.getNextUrl(json);
