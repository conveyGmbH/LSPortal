    ...
    WinJS.Namespace.define("GenDataModDetails", {
        Controller: WinJS.Class.derive(Application.Controller, function Controller(pageElement, commandList) {
            ...
            Application.Controller.apply(this, [pageElement, {
                modData: getEmptyDefaultValue(GenDataModDetails.adresseView.defaultValue),
                modpaData: copyByValue({ INITPersonKategorieID: 0 }),
            ...
            var updateImage = function (wFormat, mimeType, fileName, result) {
                ...
                var ret = new WinJS.Promise.as().then(function () {
                    ...
                    AppBar.busy = true;
                    var recordId = that.getAddressId();
                    return GenDataModDetails.adresseDOC.update(function (response) {
                        ...
                        WinJS.Promise.as().then(function () {
                            return that.saveData(function (response) {
                                Log.print(Log.l.trace, "GenDataModDetails saved");
                                //AppBar.modified = true;
                            }, function (errorResponse) {
                                Log.print(Log.l.error, "error saving GenDataModDetails");
                            });
                        }).then(function () {
                            that.loadData();
                        });
                ...
                clickDelete: function () {
                    ...
                    confirm(confirmTitle, function (result) {
                        if (result) {
                            ...
                            that.deleteData(function() {
                                var master = Application.navigator.masterControl;
                                if (master && master.controller && master.controller.binding) {
                                    master.controller.curRecId = null;
                                    master.controller.binding.personId = 0;
                                    master.controller.binding.addressId = 0;
                                    master.controller.binding.personCategoryId = 0;
                    ...
                    if (fieldcheck === true) {
                        ...
                        AppData.call("PRC_CreatePerson", {
                            ...
                            if (json && json.d && json.d.results && json.d.results.length > 0) {
                                newRecordId = json.d.results[0].NewPersonAdresseID;
                            }
                            if (newRecordId) {
                                var master = Application.navigator.masterControl;
                                if (master && master.controller && master.controller.binding) {
                                    ...
                                    master.controller.loadData().then(function() {
                                        return master.controller.selectRecordId(newRecordId);
                                    }).then(function() {
                                        that.binding.newmodData =
                                            getEmptyDefaultValue(GenDataModDetails.adresseView.defaultValue);
                                        newmod.style.display = "none";
                                    });
                                }
                            } else {
                                AppData.setErrorMsg(that.binding, "PRC_CreatePerson returned no data!");
                            }
                        }, function (errorResponse) {
                            AppBar.busy = false;
            ...
            var loadData = function () {
                ...
                var ret = new WinJS.Promise.as().then(function () {
                    ...
                    var recordId = getAddressId();
                    that.binding.docId = getAddressId();
                    if (recordId) {
                        //load of format relation record data
                        Log.print(Log.l.trace, "calling select contactView...");
                    ...
                    var recordId = getPersonAddressId();
                    if (recordId) {
                        //load of format relation record data
                        Log.print(Log.l.trace, "calling select contactView...");
            ...
            var saveData = function (complete, error) {
                ...
                var personAddressId = getPersonAddressId();
                if (AppBar.modified && !AppBar.busy && dataMod && datapaMod && addressId && personAddressId) {
                    AppBar.busy = true;
                    Log.print(Log.l.trace, "calling update adresseTable...");
                    ret = GenDataModDetails.adresseTable.update(function (response) {
                        ...
                        // called asynchronously if an error occurs
                        // or server returns response with an error status.
                        Log.print(Log.l.error, "update adresseTable returned error " + errorResponse);
                        AppData.setErrorMsg(that.binding, errorResponse);
                        error(errorResponse);
                    }, addressId, dataMod).then(function () {
                        //load of format relation record data
                        datapaMod.INITPersonKategorieID = parseInt(datapaMod.INITPersonKategorieID);
                        Log.print(Log.l.trace, "calling update personAdresseTable...");
                        return GenDataModDetails.personAdresseTable.update(function (response) {
                            AppBar.busy = false;
                            // called asynchronously if ok
                            Log.print(Log.l.info, "dataMod update: success!");
                            AppBar.busy = false;
                            ...
                            // called asynchronously if an error occurs
                            // or server returns response with an error status.
                            Log.print(Log.l.error, "update personAdresseTable returned error " + errorResponse);
                            AppData.setErrorMsg(that.binding, errorResponse);
                            error(errorResponse);
                        }, personAddressId, datapaMod);
                    });
                } else if (AppBar.busy) {
            ...
            var deleteData = function (complete, error) {
                ...
                var ret;
                var master = Application.navigator.masterControl;
                var recordId = master && master.controller && master.controller.curRecId;
                if (recordId) {
                    AppBar.busy = true;
                    ret = GenDataModDetails.personAdresseTable.deleteRecord(function (response) {
                        AppBar.busy = false;
                        // called asynchronously if ok
                        that.binding.modData = getEmptyDefaultValue(GenDataModDetails.adresseView.defaultValue);
                        if (typeof complete === "function") {
                            complete(response);
                        }
                    }, function (errorResponse) {
