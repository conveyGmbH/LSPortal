...
/// <reference path="~/www/fragments/uploadUserPhoto/uploadUserPhotoService.js" />
        ...
        Controller: WinJS.Class.derive(Fragments.Controller, function Controller(fragmentElement, options, commandList) {
            ...
            var insertImage = function (wFormat, mimeType, fileName, result) {
                ...
                var ret = new WinJS.Promise.as().then(function () {
                    ...
                    AppBar.busy = true;
                    return UploadUserPhoto.docView.insert(function (json) {
                        ...
                        if (json && json.d) {
                            ...
                            that.binding.docId = json.d.DOC1MitarbeiterVIEWID;
                        }
            ...
            function getFileData(file, name, type, size) {
                ...
                var fileExtensions = UploadUserPhoto.docFormatList.map(function (item) {
                    return item.fileExtension;
                });
                var index = fileExtensions.indexOf(type);
                if (index < 0 || UploadUserPhoto.docFormatList[index].mimeType !== type) {
                    var mimeTypes = UploadUserPhoto.docFormatList.map(function (item) {
                        return item.mimeType;
                    });
                    index = mimeTypes.indexOf(type);
                }
                var docFormat = (index >= 0) ? UploadUserPhoto.docFormatList[index] : null;
                if (docFormat) {
