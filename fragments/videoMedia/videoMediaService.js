    ...
    WinJS.Namespace.define("VideoMedia", {
        ...
        sketchDocView: {
            select: function (complete, error, recordId, isLocal) {
                ...
                var ret = VideoMedia.getSketchDocView(isLocal).selectById(complete, error, recordId);
        ...
        sketchView: {
            insert: function (complete, error, viewResponse, isLocal) {
                Log.call(Log.l.trace, "wavSketchView.");
                var ret = VideoMedia.getSketchView(isLocal).insert(complete, error, viewResponse);
                Log.ret(Log.l.trace);
                return ret;
            },
            deleteRecord: function (complete, error, recordId, isLocal) {
                Log.call(Log.l.trace, "wavSketchView.");
                var ret = VideoMedia.getSketchView(isLocal).deleteRecord(complete, error, recordId);
                Log.ret(Log.l.trace);
                return ret;
            }
        }
