...
(function () {
    ...
    WinJS.Namespace.define("AppData", {
        ...
        DbInit: WinJS.Class.define(function dbInit(complete, error, progress, bReInit) {
            ...
            that._promise = that.selectProjectRelations(error).then(function(res) {
                followFunction(res);
            }, function(err) {
                error(err);
            });
            ...
            selectProjectRelations: function(error) {
            ...
            initProject: function(that, complete, error) {
                ...
                insertLanguageSpec(function() {
                    ...
                    AppData._lgntInits = [];
                    // check for project tables
                    that.selectProjectRelations(error).then(function (res) {
                        var j;
                        if (res && res.rows) {
                    ...
                    if (dirEntry) {
                        var copyDbToStorage = function(success) {
                            if (dataDirectory !== cordova.file.dataDirectory &&
                                typeof DbCopyPlugin === "object" &&
                                typeof DbCopyPlugin.copyDbToStorage === "function") {
