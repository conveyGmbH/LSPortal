...
(function () {
    ...
    WinJS.Namespace.define("Colors", {
        ...
        changeCSS: function(selector, element, value) {
            Log.call(Log.l.trace, "Colors.", "selector=" + selector + " element=" + element + " value=" + value);
            var cssRules;
            var strValue;
            if (!value) {
                strValue = "";
                            ...
                            if (rule.selectorText === selector && rule.style) {
                                ...
                                if (prevValue) {
                                    if (prevValue.substr(0, 3) === "rgb" && 
                                        ...
                                        prevValue === strValue) {
                                        Log.print(Log.l.trace, "extra ignored: " + selector + " { " + element + ": " + prevValue + " == " + strValue + " }");
                                    } else {
                                        rule.style[element] = strValue;
                                        var newValue = rule.style[element];
                                        if (newValue.substr(0, 3) === "rgb" &&
                                            ...
                                            newValue === strValue) {
                                            Log.print(Log.l.trace, "changed style: " + selector + " { " + element + ": " + prevValue + " -> " + strValue + " }");
                                        } else {
                                            if (typeof sheet.addRule === "function") {
                                                var styleText = element + ":" + strValue;
                                                sheet.addRule(selector, styleText);
                                            } else if (typeof sheet.insertRule === "function") {
                                                var ruleText = selector + " { " + element + ":" + strValue + "; }";
                                                sheet.insertRule(ruleText, rules.length);
                                            } 
                                            Log.print(Log.l.trace, "added style: " + selector + " { " + element + ": " + prevValue + " -> " + strValue + " }");
                                        }
                                    }
                                    break;
                                }
        ...
        hex2rgb: function(h) {
            var r, g, b;
            Log.call(Log.l.u2, "Colors.", "h=" + h);
            if (h) {
                r = parseInt((Colors.cutHex(h)).substring(0, 2), 16);
                g = parseInt((Colors.cutHex(h)).substring(2, 4), 16);
                b = parseInt((Colors.cutHex(h)).substring(4, 6), 16);
            } else {
                r = 0;
                g = 0;
                b = 0;
        ...
        */
        ...
        loadSVGImageElements: function (pageElement, className, size, color, attribute, complete) {
            var ret = null;
            Log.call(Log.l.u1, "Colors.", "className=" + className + " size=" + (size ? size.toString() : "null") + " color=" + color);
            if (pageElement) {
                var newElementList = pageElement.querySelectorAll("." + className);
