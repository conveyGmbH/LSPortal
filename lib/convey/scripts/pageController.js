                                ...
                                that.hammer.on('panstart', function(ev) {
                                    ...
                                    if (ev.center && ev.pointerType === "touch") {
                                        ...
                                        if (rect &&
                                            ...
                                            targetTagName === "input" && Application._horzSwipeExcludedInputTypes.indexOf(ev.target.type) >= 0) ||
    ...
    WinJS.Namespace.define("Application", {
        ...
        RecordsetController: WinJS.Class.derive(Application.Controller, function RecordsetController(pageElement, addPageData, commandList, isMaster, tableView, showView, listView) {
            ...
            var scrollToRecordId = function (recordId) {
                ...
                } else if (that.listView && that.listView.winControl) {
                    var scope = that.scopeFromRecordId(recordId);
                    if (scope && scope.index >= 0) {
                        that.listView.winControl.ensureVisible(scope.index);
                    }
                }
                Log.ret(Log.l.trace);
            }
            ...
            var getFieldEntries = function (index) {
                var ret = {};
                var listControl = that.listView && that.listView.winControl;
                if (listControl) {
                    var element = listControl.elementFromIndex(index);
                    if (element) {
            ...
            var loadingStateChanged = function(eventInfo) {
                var i;
                Log.call(Log.l.u1, "RecordsetController.Controller.");
                var listController = that.listView && that.listView.winControl
                if (listController) {
                    Log.print(Log.l.u1, "loadingState=" + listView.winControl.loadingState);
                    if (WinJS.Utilities.hasClass(that.listView, "win-selectionstylefilled")) {
                        ...
                        if (!recordId) {
                            if (json && json.d) {
                                ...
                                if (!that.records) {
                                    ...
                                    if (listControl && listControl.selection && listControl.selection.count() === 1) {
                                        if (bFound) {
                                            that.selectRecordId(that.curRecId);
                                        } else {
                                            var selIndex = listControl.selection.getIndices()[0];
                                            if (typeof selIndex === "number") {
                                                listControl.selection.clear().then(function() {
