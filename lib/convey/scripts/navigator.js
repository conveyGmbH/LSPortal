    ...
    WinJS.Utilities._require([
        ...
        WinJS.Namespace.define("NavigationBar", {
                ...
                if (result && result.Title) {
                    ...
                    if (helpPane) {
                        ...
                        if (helpHiddenInput && helpHiddenInput.parentElement) {
                            if (helpHiddenInput && helpHiddenInput.value) {
                                helpHiddenInput.value = false;
                            }
            ...
            showGroupsMenu: function (results, bForceReloadMenu) {
                ...
                // TODO(dev-only): "crmConnections" has no AppListSpec row on the
                // server yet (license entitlement pending) — force it into the
                // enabled list on localhost only, so the new CRM catalog page can
                // be clicked/tested during development. Remove once the server
                // grants this entitlement like it already does for "crmExport".
                if (window.location && window.location.hostname === "localhost" &&
                    applist.indexOf("crmConnections") < 0) {
                    applist.push("crmConnections");
                }
