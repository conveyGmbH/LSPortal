                    ...
                    if (NavigationBar.groups && NavigationBar.groups.length > 0) {
                        for (i = 0; i < NavigationBar.groups.length; i++) {
                            if (!NavigationBar.groups[i].disabled && !NavigationBar.groups[i].predecGroup) {
                                var element = document.createElement("div");
                                navCommands.appendChild(element);
                                var navigationLabel = getResourceText(NavigationBar.groups[i].label || ("label." + NavigationBar.groups[i].id));
                                var resIdTooltip = NavigationBar.groups[i].tooltip || NavigationBar.groups[i].label || ("tooltip." + NavigationBar.groups[i].id);
                                var navigationTooltip = getResourceText(resIdTooltip);
                                if (!navigationTooltip || navigationTooltip === resIdTooltip) {
                                    navigationTooltip = navigationLabel;
                                }
