...
(function () {
    ...
    WinJS.Utilities._require([
                ...
                function PageControlNavigator(element, options) {
                    ...
                    _navigated: function () {
                        ...
                        WinJS.Promise.as().then(function () {
                            ...
                            //if (nav.location === Application.startPage) {
                            //    NavigationBar.createHelpPane("Hilfe zur Startseite", "Hier finden sie weiterführende Hilfe zur Bedienung dieser Seite.");
                            //}
                        });
                        Log.ret(Log.l.trace, "");
                    },
        ...
        WinJS.Namespace.define("NavigationBar", {
            ...
            createHelpPane: function (title, subtitle) {
                var ret = null;
                ...
                if (helpPane) {
                    ...
                    if (!helpPane.close) {
                        ...
                        var closeButtons = helpPane.querySelectorAll(".close-button");
                        if (closeButtons) for (var i = 0; i < closeButtons.length; i++) {
                            if (closeButtons[i]) {
                                closeButtons[i].addEventListener("click", helpPane.close);
                            }
                        }
                    }
                    var expandHelp = helpPane.querySelector(".help-expand");
                    if (expandHelp && expandHelp.firstElementChild) {
                        expandHelp.firstElementChild.textContent = getResourceText("label.showMore");
                    ...
                    if (titleTexts) for (var i = 0; i < titleTexts.length; i++) {
                        if (titleTexts[i] && titleTexts[i].firstElementChild) {
                            titleTexts[i].firstElementChild.textContent = title;
                        }
                    }
                    var subtitleTexts = helpPane.querySelectorAll(".help-subtitle");
                    if (subtitleTexts) for (var i = 0; i < subtitleTexts.length; i++) {
                        if (subtitleTexts[i] && subtitleTexts[i].firstElementChild) {
                            subtitleTexts[i].firstElementChild.textContent = subtitle;
                        }
                    }
                    if (WinJS.Utilities.hasClass(helpPane, "expanded")) {
                        WinJS.Utilities.removeClass(helpPane, "expanded")
