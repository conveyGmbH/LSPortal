                    ...
                    if (helpPane) {
                        ...
                        if (!helpPane.close) {
                            ...
                            if (helpClose && helpClose.firstElementChild) {
                                helpClose.firstElementChild.textContent = getResourceText("label.close");
                        ...
                        if (titleTexts) for (i = 0; i < titleTexts.length; i++) {
                            if (titleTexts[i] && titleTexts[i].firstElementChild) {
                                titleTexts[i].firstElementChild.textContent = result.Title;
                            }
                        }
                        var helpSubTitle = helpPane.querySelector(".help-subtitle");
                        if (helpSubTitle && helpSubTitle.firstElementChild) {
                            helpSubTitle.firstElementChild.innerHTML = result.SubTitle;
                            if (!result.SubTitle) {
                                helpSubTitle.style.display = "none";
                            }
                        }
                        var helpMediaContainer = helpPane.querySelector(".help-media-container");
                        if (helpMediaContainer) {
                            if (!result.MediaURL) {
                                helpMediaContainer.style.display = "none";
                            }
                        }
                        var helpText = helpPane.querySelector(".help-text");
                        if (helpText && helpText.firstElementChild) {
                            helpText.firstElementChild.innerHTML = result.BodyText;
                            if (!result.BodyText && helpText.parentElement) {
