        ...
        PageControlNavigator: WinJS.Class.define(
            ...
            function PageControlNavigator(element, options) {
                ...
                _navigated: function () {
                    ...
                    if (this._nextMaster === this._lastMaster) {
                        ...
                        if (this._nextMaster && !this._masterHidden) {
                            ...
                            this.resizeMasterElement(this.masterElement).then(function() {
                                ...
                                /*if (that.masterElement.resizeWidth) {
                    ...
                    if (!masterPromise) {
                        masterPromise = WinJS.Promise.as();
                    }
                    ...
                    if (masterElement && this._nextMaster) {
                        ...
                        masterElement.resizeHelper = new Application.ResizeHelper(masterElement, {
                            ...
                            onsizechanged: function (newWidth) {
                                ...
                                if (masterElement.resizeWidth <= 350 && Application.navigator) {
                                    Application.navigator._hideMaster();
                                }
                                if (!AppData._persistentStates.resizeWidths) {
         ...
         */
         ResizeHelper: WinJS.Class.define(
             function ResizeHelper(element, options) {
                 ...
                 WinJS.Utilities.addClass(this._gripper, "resize-gripper");
                 this._events = [
                     { 
                         name: "mouseover", 
                         handler: function onMouseOver(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && eventInfo.target === that._gripper) {
                     ...
                     { 
                         ...
                         handler: function onMouseDown(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && !that._pointerCaptured) {
                                 var position = WinJS.Utilities.getPosition(that._gripper);
                                 if (position) {
                                     /*if (eventInfo.pageX >= position.left &&
                                         eventInfo.pageX <= position.left + position.width &&
                                         eventInfo.pageY >= position.top &&
                                         eventInfo.pageY <= position.top + position.height) {*/
                                         ...
                                         if (that._options && that._options.vertical) {
                     ...
                     { 
                         ...
                         handler: function onMouseMove(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && that._initialSize && that._pointerCaptured &&
                                 ...
                                 if (that._options && that._options.vertical) {
                                     ...
                                     that._element.style.height = that._currentSize.toString() + "px";
                                 } else {
                                     ...
                                     that._element.style.width = (that._currentSize + 18).toString() + "px";
                                 }
                                 if (that._currentSize && that._options && typeof that._options.onsizechanging === "function") {
                     ...
                     { 
                         name: "mouseup", handler: function onMouseUp(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && 
                                 that._pointerCaptured  && 
                                 typeof document.releaseCapture === "function" &&
                                 typeof document.releaseCapture === "function" &&
                                 !(typeof that._gripper.setPointerCapture === "function")) {
                                 that._gripper.style.transition = "";
                                 that._gripper.style.webkitTransition = "";
                                 that._gripper.style.opacity = 0;
                                 if (that._currentSize && that._options && typeof that._options.onsizechanged === "function") {
                     ...
                     { 
                         ...
                         handler: function onMouseDown(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && 
                                 !that._pointerCaptured  && (typeof that._gripper.setPointerCapture === "function")) {
                                 var position = WinJS.Utilities.getPosition(that._gripper);
                                 if (position) {
                                     /*if (eventInfo.pageX >= position.left &&
                                         eventInfo.pageX <= position.left + position.width &&
                     ...
                     { 
                         name: "pointermove", handler: function onMouseMove(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && that._initialSize && 
                                 that._pointerCaptured  && (typeof that._gripper.setPointerCapture === "function") &&
                                 that._element && that._element.style) {
                                 if (that._options && that._options.vertical) {
                                     var deltaY = eventInfo.pageY - that._pageY;
                     ...
                     { 
                         name: "pointerup", handler: function onMouseUp(eventInfo) {
                             Log.call(Log.l.trace, "ResizeHelper.");
                             if (eventInfo && that._gripper && 
                                 that._pointerCaptured  && (typeof that._gripper.setPointerCapture === "function")) {
                                 that._gripper.style.transition = "";
                 ...
                 that._events.forEach(function(event) {
                     if (event && event.target) {
                         event.bound = event.handler.bind(that);
                         Log.print(Log.l.trace, "bind event '" + event.name + "'");
                         event.target.addEventListener(event.name, event.bound, false);
                     }
                 });
                         ...
                         if (this._gripper) {
                             if (this._events) {
                                 this._events.forEach(function (event) {
                                     if (event && event.target && event.bound) {
                                         Log.print(Log.l.trace, "remove event '" + event.name);
                                         event.target.removeEventListener(event.name, event.bound);
                                         event.bound = null;
                                     }
                                 });
                                 this._events = null;
    ...
    WinJS.Namespace.define("NavigationBar", {
        ...
        setMenuForGroups: function () {
            ...
            if (Application.navigator) {
                ...
                splitViewPaneScrollTop = splitViewPane && splitViewPane.scrollTop;
            }
