            ...
            if (!AppBar._commandList || !AppBar._commandList.length) {
                    ...
                    for (j = 0; j < removeCommands.length; j++) {
                        for (i = AppBar._commandList.length - 1; i >= 0; i--) {
                            ...
                            if (removeCommands[j].id === commandId) {
                                ...
                                // _eventHandlers and _disableHandlers won't be removed here!
                                break;
                            }
                        }
                    }
                ...
                if (addComands && addComands.length > 0) {
                    ...
                    if (typeof device === "object" &&
                        ...
                        for (i = 0; i < addComands.length; i++) {
                            if (addComands[i].id === "clickBack") {
                                addComands[i].section = "secondary";
                                break;
                            }
                        }
                    }
                    ...
                    for (j = 0; j < addComands.length; j++) {
                        Log.print(Log.l.u1,
                            "section=" + addComands[j].section +
                            " id=" + addComands[j].commandId +
                            " label=" + addComands[j].label +
                            " svg=" + addComands[j].svg);
                        if (!addComands[j].onclick) {
                            addComands[j].onclick = AppBar.outputCommand;
                        }
                        if (typeof addComands[j].disabled === "undefined") {
                            addComands[j].disabled = true;
                        }
                        addComands[j].commandId = addComands[j].id;
                        var command = new WinJS.UI.AppBarCommand(null, addComands[j]);

                        var bReplaced = false;
                        for (i = 0; i < prevLength; i++) {
                            commandId = AppBar._commandList[i].id;
                            if (addComands[j].id === commandId) {
                                Log.print(Log.l.trace, "replace commandId[" + i + "].id=" + commandId);
                                AppBar.barControl.data.setAt(i, command);
                                AppBar._commandList[i] = addComands[j];
                                bReplaced = true;
                                break;
                            }
                        }
                        if (!bReplaced) {
                            ...
                            AppBar._commandList.push(addComands[j]);
                        }
                    }
                    ...
                    if (idxKeyEnter >= 0) {
                        ...
                        if (idxPrimary >= 0 && idxPrimary !== idxKeyEnter) {
                            ...
                            if (AppBar.barControl.data) {
                                enterCommand = AppBar.barControl.data.slice(idxKeyEnter).getAt(0);
                                prevCommand = AppBar.barControl.data.splice(idxPrimary, 1, enterCommand).getAt(0);
