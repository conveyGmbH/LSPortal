...
class FakeDataGenerator {
    constructor() {
        ...
        // Salutations (Salesforce standard picklist values)
        this.salutations = [
            'Mr.', 'Ms.', 'Mrs.', 'Dr.', 'Prof.'
        ];

        // Lead Status (Salesforce standard picklist values)
        this.leadStatuses = [
            'Open - Not Contacted', 'Working - Contacted', 'Closed - Converted', 'Closed - Not Converted','New', 'Working', 'Nurturing', 'Qualified', 'Unqualified', 'Converted'
        ];

        // Rating (Salesforce standard picklist values)
        this.ratings = [
            'Hot', 'Warm', 'Cold'
        ];
        ...
        // Departments
        this.departments = [ 
            'Sales', 'Marketing', 'Finance', 'IT', 'Operations', 'HR', 'Customer Success' 
        ];
        ...
        // Account Types
        this.accountTypes = [
            'Prospect', 'Customer - Direct', 'Customer - Channel', 'Partner', 'Vendor', 'Distributor', 'Reseller'
        ];
        
        // Contact Roles
        this.contactRoles = [ 
            'Decision Maker', 'Influencer', 'Evaluator', 'End User', 'Executive Sponsor'
        ];

        // Campaign Members Status
        this.campaignStatus = [ 
            'Sent', 'Responded', 'Opened', 'Clicked', 'Bounced', 'Unsubscribed', 'Registered', 'Attended'
        ];
    ...
    // Generate fake salutation
    generateSalutation() {
        return this.randomItem(this.salutations);
    }

    // Generate fake fax number (similar to phone format)
    generateFax() {
        const areaCode = this.randomNumber(30, 999);
        const subscriber = String(this.randomNumber(1000000, 9999999)).padStart(7, '0');
        return `+49 (${areaCode}) ${subscriber}`;
    ...
    // Generate fake lead status
    generateLeadStatus() {
        return this.randomItem(this.leadStatuses);
    }

    // Generate fake rating
    generateRating() {
        return this.randomItem(this.ratings);
    }
    ...
    // Generate fake department
    generateDepartment() {
        return this.randomItem(this.departments);
    }

    // Generate fake opportunity stage
    generateOpportunityStage() {
        return this.randomItem(this.opportunityStages);
    }

    // Generate fake opportunity type
    generateOpportunityType() {
        return this.randomItem(this.opportunityTypes);
    }

    // Generate fake account type
    generateAccountType() {
        return this.randomItem(this.accountTypes);
    }

    // Generate fake contact role
    generateContactRole() {
        return this.randomItem(this.contactRoles);
    }

    // Generate fake campaign status
    generateCampaignStatus() {
        return this.randomItem(this.campaignStatus);
    }

    // Generate fake language
    generateLanguage() {
        return this.randomItem(this.languages);
    }
     ...
     * @param {Object} customFieldDefaults - Object mapping field names to their default values (from custom fields config)
     * @returns {Object} Lead data with fake values for empty required fields
     */
    fillEmptyFields(leadData, requiredFields = ['LastName', 'Company'], customFieldDefaults = {}) {
        ...
        for (const fieldName of requiredFields) {
            if (this.isEmpty(processedData[fieldName])) {
                ...
                switch (fieldName) {
                    ...
                    case 'Salutation':
                        fakeValue = this.generateSalutation();
                        break;

                    case 'Fax':
                        fakeValue = this.generateFax();
                        break;

                    case 'Status':
                    case 'LeadStatus':
                        fakeValue = this.generateLeadStatus();
                        break;

                    case 'Rating':
                        fakeValue = this.generateRating();
                        break;

                    case 'Department':
                        fakeValue = this.generateDepartment();
                        break;

                    case 'StageName':
                    case 'OpportunityStage':
                        fakeValue = this.generateOpportunityStage();
                        break;

                    case 'Type':
                    case 'OpportunityType':
                        fakeValue = this.generateOpportunityType();
                        break;

                    case 'AccountType':
                        fakeValue = this.generateAccountType();
                        break;

                    case 'Role':
                    case 'ContactRole':
                        fakeValue = this.generateContactRole();
                        break;

                    case 'CampaignMemberStatus':
                        fakeValue = this.generateCampaignStatus();
                        break;

                    case 'Language':
                    case 'LanguageLocaleKey':
                        fakeValue = this.generateLanguage();
                        break;

                    default:
                        // For unknown/custom fields, check if a default value was provided
                        if (customFieldDefaults && customFieldDefaults[fieldName]) {
                            fakeValue = customFieldDefaults[fieldName];
                        } else {
                            // Fallback to field name with Test_ prefix
