            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    const missingFieldsMsg = errorData.error === 'MISSING_FIELDS' && errorData.missingFields?.length
                        ...
                        : null;
                    // For validation errors (400), show the detail from error field, not the generic message
                    const validationMsg = errorData.error && errorData.error !== errorData.message
                        ? errorData.error
                        : null;
                    return {
                        success: false,
                        status: 'failed',
                        message: sfMessage || missingFieldsMsg || validationMsg || errorData.message || `HTTP ${response.status}`,
                        salesforceId: null,
