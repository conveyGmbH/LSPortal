    ...
    class SalesforceLeadLib {
        constructor(config = {}) {
            ...
            this.fieldMappingService = new CrmProviders.FieldMappingService({
                ...
                defaultMappings: STANDARD_SALESFORCE_LEAD_FIELDS,
                dataCache: SalesforceLeadLib._dataCache,
        ...
        static async openFieldMapping(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                const configCached = !localStorageOnlyMode &&
                    SalesforceLeadLib._dataCache.has(`fieldmap:${eventId}:LeadSuccess_Event_API`) &&
                    SalesforceLeadLib._dataCache.has(`apifields:${eventId}:LeadSuccess_Event_API`);
                if (!configCached) {
                ...
                try {
                    ...
                    if (configCached) {
                        ...
                        svc._applyConfigData(
                            SalesforceLeadLib._dataCache.get(`fieldmap:${eventId}:${svc.apiEndpointValue}`).configDataStr
                        );
                        svc.apiFields = SalesforceLeadLib._dataCache.get(`apifields:${eventId}:${svc.apiEndpointValue}`).apiFields;

                        this._renderFieldsGrid(rootElement, svc, 'active');
                        this._attachFieldConfiguratorListeners(rootElement, svc);
        ...
        static async _loadApiFieldsInBackground(rootElement, fieldMappingService) {
            try {
                ...
                if (eventId) {
                    const cachedApiFields = SalesforceLeadLib._dataCache.get(`apifields:${eventId}:${fieldMappingService.apiEndpointValue}`);
                ...
                if (apiFields && Object.keys(apiFields).length > 0) {
                    ...
                    if (eventId) {
                        SalesforceLeadLib._dataCache.set(`apifields:${eventId}:${fieldMappingService.apiEndpointValue}`, { apiFields });
                    }
                }
            } catch (apiError) {
        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            if (forceRefresh) {
                ...
                SalesforceLeadLib._dataCache.delete(`fieldmap:${eventId}:LeadSuccess_Event_API`);
                SalesforceLeadLib._dataCache.delete(`apifields:${eventId}:LeadSuccess_Event_API`);
            }
    ...
    if (window.CrmProviders && typeof window.CrmProviders.registerAdapter === "function") {
        window.CrmProviders.registerAdapter({
            ...
            renderContactList: function (rootElement, eventId, options) {
                return SalesforceLeadLib.renderContactList(rootElement, eventId, options);
            },
            openFieldMapping: function (rootElement, eventId, options) {
                return SalesforceLeadLib.openFieldMapping(rootElement, eventId, options);
            },
