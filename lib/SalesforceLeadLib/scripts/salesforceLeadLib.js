...
(function() {
    ...
    class SalesforceLeadLib {
        ...
        static _buildCrmExportHTML(leadData, allFields, contactId, options = {}) {
            ...
            return `
                ...
                <div class="contenthost-background" style="max-width: 1400px; margin: 0 auto; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; min-height: 100vh;">
                    ...
                    <div id="sf-attachment-modal" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.6); z-index:10010; align-items:center; justify-content:center;">
                        <div style="background:var(--Window,white); border-radius:12px; width:900px; max-width:95vw; max-height:90vh; display:flex; flex-direction:column; box-shadow:0 20px 40px rgba(0,0,0,0.3); overflow:hidden;">
                            <div style="display:flex; align-items:center; justify-content:space-between; padding:16px 20px; background:var(--accent-color,#2563eb); color:white; flex-shrink:0;">
                                <div style="display:flex; align-items:center; gap:10px;">
                                    <i class="fa-solid fa-paperclip"></i>
                                    <span id="sf-attachment-modal-title" style="font-weight:700; font-size:15px;">Attachment Preview</span>
                                </div>
                                <div style="display:flex; align-items:center; gap:8px;">
                                    <button id="sf-attachment-download-btn" style="padding:6px 14px; background:rgba(255,255,255,0.2); border:1px solid rgba(255,255,255,0.4); border-radius:6px; color:white; font-size:12px; font-weight:600; cursor:pointer; display:none;">
        ...
        static _attachCrmExportListeners(rootElement, leadData, allFields, options = {}) {
            ...
            const attachmentModal = rootElement.querySelector('#sf-attachment-modal');
            if (attachmentModal) {
                const closeBtn = rootElement.querySelector('#sf-attachment-close-btn');
                if (closeBtn) {
                    closeBtn.addEventListener('click', () => { attachmentModal.style.display = 'none'; });
                }
                ...
                rootElement.querySelectorAll('.sf-view-attachment-btn').forEach(btn => {
                    btn.addEventListener('click', async () => {
                        ...
                        await this._showAttachmentModal(rootElement, attachmentId);
                    });
                });
            }
         ...
         * @private
         */
        static async _showAttachmentModal(rootElement, attachmentId) {
            const modal = rootElement.querySelector('#sf-attachment-modal');
            const modalTitle = rootElement.querySelector('#sf-attachment-modal-title');
            const modalBody = rootElement.querySelector('#sf-attachment-modal-body');
            const downloadBtn = rootElement.querySelector('#sf-attachment-download-btn');

            if (!modal || !modalBody) return;

            // Show modal with loading state
            modal.style.display = 'flex';
            modalTitle.textContent = 'Loading attachment...';
            modalBody.innerHTML = `<div style="color:#6b7280; font-size:14px; text-align:center;"><div style="width:32px;height:32px;border:3px solid #e5e7eb;border-top-color:var(--accent-color,#2563eb);border-radius:50%;animation:spin 0.8s linear infinite;margin:0 auto 12px;"></div>Loading...</div>`;
            ...
            try {
                ...
                if (isSVG) {
                    try {
                        ...
                        body = (decoded.includes('<svg') || decoded.includes('<?xml'))
                            ? body.replace(/\s+/g, '')
                            : btoa(decodeURIComponent(encodeURIComponent(body)));
                    } catch { body = body.replace(/\s+/g, ''); }
                ...
                if (isSVG) {
                    ...
                    } catch { modalBody.innerHTML = `<object data="${dataUrl}" type="image/svg+xml" style="width:100%;height:60vh;">SVG not supported</object>`; }
                ...
                modalBody.innerHTML = `<div style="color:#ef4444; text-align:center; padding:40px;">Error loading attachment: ${this._escapeHtml(error.message)}</div>`;
            }
        }
        ...
        static BATCH_EXCLUDED_FIELDS = new Set([
            ...
            'KontaktViewId', 'LastExportStatus', 'LastExportTimestamp',
         ...
         */
        static async transferSingleLead(leadData, attachments) {
            try {
                const instance = this._getInstance();
        ...
        static async executeBatchTransfer(items, options = {}) {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                try {
                    ...
                    const transferResult = await this.transferSingleLead(leadData, attachments);
