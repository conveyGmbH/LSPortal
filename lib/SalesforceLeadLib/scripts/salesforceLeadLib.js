    ...
    class SalesforceLeadLib {
        ...
        static async openFieldMapping(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                const configCached = !localStorageOnlyMode &&
                    SalesforceLeadLib._dataCache.has(`fieldmap:${eventId}`) &&
                    SalesforceLeadLib._dataCache.has(`apifields:${eventId}`);
                if (!configCached) {
                ...
                try {
                    ...
                    if (configCached) {
                        ...
                        svc._applyConfigData(
                            SalesforceLeadLib._dataCache.get(`fieldmap:${eventId}`).configDataStr
                        );
                        svc.apiFields = SalesforceLeadLib._dataCache.get(`apifields:${eventId}`).apiFields;
        ...
        static async _loadApiFieldsInBackground(rootElement, fieldMappingService) {
            try {
                ...
                if (eventId) {
                    const cachedApiFields = SalesforceLeadLib._dataCache.get(`apifields:${eventId}`);
                ...
                if (apiFields && Object.keys(apiFields).length > 0) {
                    ...
                    if (eventId) {
                        SalesforceLeadLib._dataCache.set(`apifields:${eventId}`, { apiFields });
                    }
                }
            } catch (apiError) {
        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            if (forceRefresh) {
                ...
                SalesforceLeadLib._dataCache.delete(`apifields:${eventId}`);
            }
