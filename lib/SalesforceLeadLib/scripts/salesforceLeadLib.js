        ...
        static showBatchValidationModal(report, providerLabel) {
            return new Promise((resolve) => {
                ...
                const buildInvalidRow = (entry) => {
                    const name = this.getLeadDisplayName(entry.item);
                ...
                modal.innerHTML = `
                    <div class="sf-modal" role="dialog" aria-modal="true" aria-labelledby="sf-validate-title">
                        ...
                        <div style="display: flex; border-bottom: 1px solid var(--sf-border);">
                            ${buildStatBox(_t('crmExport.statValid', 'Valid'), report.valid.length, 'success')}
                            ${missingCount > 0 ? buildStatBox(_t('crmExport.statMissingRequired', 'Missing fields'), missingCount, 'error') : ''}
                            ${emailCount > 0 ? buildStatBox(_t('crmExport.statInvalidEmail', 'Invalid email'), emailCount, 'warning') : ''}
                        </div>
