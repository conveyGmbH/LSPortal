...
<div id="toast-container" class="fixed top-4 right-4 z-50 space-y-2"></div>
        ...
        static _renderFieldsGrid(rootElement, fieldMappingService, currentFilter) {
            ...
            customFields.forEach(cf => {
                allFields.push({
                    name: cf.sfFieldName || cf.fieldName || cf.name || 'Unnamed',
                    label: cf.label || cf.sfFieldName || cf.fieldName || cf.name || 'Unnamed',
            ...
            if (filteredFields.length === 0) {
                ...
                filteredFields.forEach(field => {
                    ...
                    html += `
                        <div class="field-item${isRequired ? ' required' : ''}${isActive ? ' active' : ''}${isCustom ? ' custom-field' : ''}"
                             data-field="${field.name}"
                             data-is-custom="${isCustom}"
                             data-field-id="${field.id || ''}"
                             style="${borderStyle}${bgStyle} ${mainBorder} border-radius: 6px; padding: 12px; display: flex; align-items: center; gap: 12px;">
                            ...
                            <div style="flex: 1;">
                                <div style="font-size: 14px; font-weight: 500; color: #1f2937;">
                                    ...
                                    ${isCustom ? '<span style="display: inline-block; background: #2563eb; color: white; font-size: 9px; padding: 2px 6px; border-radius: 3px; font-weight: 600; margin-left: 6px;">CUSTOM</span>' : ''}
                                </div>
                                <div style="font-size: 12px; color: #6b7280;">${field.name}</div>
                            </div>
                            ${isCustom ? `
                                <div style="display: flex; gap: 8px; flex-shrink: 0;">
                                    <button class="edit-custom-field-btn" data-field-id="${field.id || ''}" data-field-name="${field.name}"
                                            style="background: #3b82f6; color: white; border: none; border-radius: 4px; padding: 6px 10px; cursor: pointer; font-size: 12px; font-weight: 500; transition: background 0.2s;"
                                            onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'"
                                            title="Edit custom field value">
                                        ✏️ Edit
                                    </button>
                                    <button class="delete-custom-field-btn" data-field-id="${field.id || ''}" data-field-name="${field.name}"
                                            style="background: #ef4444; color: white; border: none; border-radius: 4px; padding: 6px 10px; cursor: pointer; font-size: 12px; font-weight: 500; transition: background 0.2s;"
        ...
        static _attachFieldConfiguratorListeners(rootElement, fieldMappingService) {
            ...
            filterTabs.forEach(tab => {
                tab.addEventListener('click', () => {
                    ...
                    // Re-attach checkbox, delete, and edit listeners after re-render
            ...
            // Attach checkbox, delete, and edit listeners
            ...
            if (saveBtn) {
                saveBtn.addEventListener('click', async () => {
                    try {
                        // Save locally (no backend call for LSPortal)
            ...
            if (saveBtn) {
                ...
                newSaveBtn.addEventListener('click', async () => {
                    ...
                    if (!fieldName.match(/^[a-zA-Z][a-zA-Z0-9_]*(__c)?$/)) {
                        alert('Invalid field name. Must start with a letter, contain only letters, numbers, and underscores. Custom fields should end with __c');
                        return;
                    }
        ...
        static _attachCheckboxListeners(rootElement, fieldMappingService) {
            ...
            checkboxes.forEach(checkbox => {
                checkbox.addEventListener('change', async (e) => {
                    ...
                    try {
                        // Update field config
                        await fieldMappingService.setFieldConfig(fieldName, { active: isActive });

                        // Re-render to update statistics and UI
                        ...
                        console.log(`Field ${fieldName} ${isActive ? 'activated' : 'deactivated'}`);
        ...
        /**
         * Attach edit listeners to custom field edit buttons
         * @private
         */
        ...
        static async openCrmExport(rootElement, contactId) {
            ...
            try {
                ...
                // Load lead data via LS_LeadReport
                const leadData = await this._callPortalAPI('LS_LeadReport', {
                    ContactId: contactId
                });
                ...
                const html = `
                    <div class="crm-export-container" style="padding: 20px;">
                        <h2 style="margin-bottom: 20px; font-size: 24px; font-weight: bold;">
