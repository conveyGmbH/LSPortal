        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                if (dupExcludeCb) {
                    ...
                    // reason never has it surprise-reselected. Deliberately
                    // does NOT touch the active status filter — switching to
                    // "Already in {CRM}" hid every remaining (still-checked)
                    // row, making it impossible to see or adjust what's left
                    // to transfer right after excluding.
