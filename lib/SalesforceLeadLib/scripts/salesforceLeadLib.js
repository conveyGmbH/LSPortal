...
(function() {
    ...
    class SalesforceLeadLib {
        ...
        static async callSetLeadExportStatus(kontaktViewId, status, message, milliseconds) {
            ...
            const credFromConfig = (this._portalConfig?.user && this._portalConfig?.password)
                ? btoa(`${this._portalConfig.user}:${this._portalConfig.password}`)
                : null;
            const credFromSession = sessionStorage.getItem('credentials');
            const credentials = credFromConfig || credFromSession;

            console.log('[LS_SetLeadExportStatus] credentials source:', credFromConfig ? 'portalConfig' : credFromSession ? 'sessionStorage' : 'NONE');
            console.log('[LS_SetLeadExportStatus] portalConfig user:', this._portalConfig?.user || '(undefined)');
            console.log('[LS_SetLeadExportStatus] credentials (first 20 chars):', credentials ? credentials.substring(0, 20) + '...' : 'null');
            ...
            try {
                ...
                console.log('[LS_SetLeadExportStatus] URL:', url);
                ...
                if (response.status === 401) {
                    console.warn('[LS_SetLeadExportStatus] 401 Unauthorized — credentials used:', credentials ? credentials.substring(0, 20) + '...' : 'null');
                    console.warn('[LS_SetLeadExportStatus] portalConfig:', JSON.stringify({ user: this._portalConfig?.user, serverUrl: this._portalConfig?.serverUrl, apiName: this._portalConfig?.apiName }));
                    return null;
                }
