...
(function() {
    ...
    function showConfirmDialog(title, message, options = {}) {
        return new Promise((resolve) => {
            ...
            const confirmBtn = dialog.querySelector('#sf-lib-confirm-confirm');
            const cancelBtn = dialog.querySelector('#sf-lib-cancel-confirm');
    ...
    function showAlertDialog(title, message, options = {}) {
        return new Promise((resolve) => {
            ...
            const closeBtn = dialog.querySelector('#sf-lib-close-alert');
    ...
    `;
                            ...
                            <header class="sf-header" style="padding: 16px 24px;">
                                <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 16px;">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                                        ...
                                        <button id="transferToSalesforceBtn" class="sf-btn-transfer" style="display: none; padding: 0 16px; height: 38px; background: #10b981; color: white; border: none; border-radius: 8px; font-size: 13px; font-weight: 500; cursor: pointer; align-items: center; gap: 8px; white-space: nowrap;">
                                            <i class="fa-solid fa-cloud-arrow-up"></i>
                                            <span data-win-res="{textContent: 'crmSettings.transferToSalesforce'}">Transfer to Salesforce</span>
                                        </button>
                                    </div>
                                </div>
                            </header>
        ...
        static _attachFieldConfiguratorListeners(rootElement, fieldMappingService) {
            ...
            // Transfer to Salesforce button (header)
            const transferToSfBtn = rootElement.querySelector('#transferToSalesforceBtn');
            if (transferToSfBtn) {
                transferToSfBtn.addEventListener('click', async () => {
                    try {
                        // Save configuration first
                        fieldMappingService.saveConfig();

                        if (fieldMappingService.currentEventId) {
                            await fieldMappingService.saveFieldMappingsToAPI('bulk_save', 'transfer_btn');
                        }

                        SalesforceLeadLib._showToast('Configuration saved! Navigate to a contact to transfer to Salesforce.', 'success');
                    } catch (error) {
                        console.error('Failed to save configuration:', error);
                        SalesforceLeadLib._showToast('Failed to save: ' + error.message, 'error');
                    }
                });
            }
        ...
        static _openEditCustomFieldModal(rootElement, fieldMappingService, fieldId, fieldName) {
            ...
            if (saveBtn) {
                ...
                newSaveBtn.addEventListener('click', async () => {
                    ...
                    if (!newName) {
                        SalesforceLeadLib._showToast('Field name is required', 'error');
                        return;
                    }
                    ...
                    try {
                        ...
                        // Update custom field name and value
                        customField.sfFieldName = newName;
                        customField.fieldName = newName;
                        customField.name = newName;
                        customField.label = newName; // Update label to match
                        customField.value = newValue;
                        ...
                        // Save to API if eventId is available
                        if (fieldMappingService.currentEventId) {
                            await fieldMappingService.saveFieldMappingsToAPI(newName, 'update');
                        }

                        console.log(`Custom field renamed from "${oldName}" to "${newName}" with value:`, newValue);
        ...
        static async openCrmTestExport(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                const html = this._buildCrmExportHTML(testLeadData, allFields, `test-${displayId}`);
                rootElement.innerHTML = html;
                ...
                // Attach event listeners - pass testLeadData which contains fake data
                this._attachCrmExportListeners(rootElement, testLeadData, allFields);
