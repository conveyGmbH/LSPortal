...
(function() {
    ...
    class FieldMappingService {
        ...
        // NOTE: saveActiveFieldsToBackend() and loadActiveFieldsFromBackend() removed
        // Field configuration is now saved via saveFieldMappingsToAPI() using OData endpoint LS_FieldMappings
    ...
    class SalesforceLeadLib {
        ...
        async _checkAuthenticationStatus() {
            ...
            if (!this.config.backendUrl ||
                ...
                console.log('Skipping Salesforce auth check - Portal context detected');
                return { success: false, reason: 'Portal context - OAuth backend not used' };
            }
    ...
    `;
        ...
        static _getInstance() {
            ...
            if (!this._instance) {
                console.log('Creating new SalesforceLeadLib instance for Salesforce operations');
                ...
                console.log(`Using backend URL: ${backendUrl} (${isProduction ? 'PRODUCTION' : 'DEVELOPMENT'})`);
        ...
        static async openFieldMapping(rootElement, eventId, options = {}) {
            ...
            if (localStorageOnlyMode) {
                console.log('Running in localStorage-only mode for recordId:', options.recordId);
            }
            ...
            try {
                            ...
                            <main style="flex: 1; overflow-y: auto; padding: 16px;">
                                ...
                                <div class="sf-card" style="border-radius: 12px; padding: 16px; margin-top: 16px;">
                                    ...
                                    <div id="normal-mode-buttons" style="display: none; justify-content: flex-end; gap: 12px;">
                                        <button class="sf-btn-secondary text-textcolor" style="padding: 12px 24px; border: none; border-radius: 8px; font-size: 16px; font-weight: 500; cursor: pointer;" data-win-res="{textContent: 'sforce.btnCancel'}">
                                             Cancel
                                        </button>
                                        <button class="accent-background-color" style="padding: 12px 24px; color: white; border: none; border-radius: 8px; font-size: 16px; font-weight: 500; cursor: pointer;" data-win-res="{textContent: 'crmSettings.save'}">
                                           Save
                                        </button>
                                    </div>
                ...
                try {
                    ...
                    if (localStorageOnlyMode) {
                        ...
                        console.log('Field Mapping running in localStorage-only mode (no API persistence)');
                    }
        ...
        static async _loadApiFieldsInBackground(rootElement, fieldMappingService) {
            try {
                console.log('Loading API fields in background...');
                ...
                if (eventId) {
                    ...
                    if (leadDataResponse && leadDataResponse.d && leadDataResponse.d.results && leadDataResponse.d.results[0]) {
                        ...
                        console.log(`Loaded ${Object.keys(apiFields).length} fields from LS_LeadReport API (event has contacts)`);
                    }
                }

                // Fallback: Load from $metadata if no contacts found (event without contacts)
                if (!apiFields) {
                    console.log('No contacts for this event - loading fields from $metadata...');
                ...
                if (apiFields && Object.keys(apiFields).length > 0) {
                    ...
                    console.log('Could not load API fields - using standard fields only');
                }
            } catch (apiError) {
                console.warn('Could not load API fields in background:', apiError.message);
        ...
        static async _loadFieldsFromMetadata() {
            try {
                if (!SalesforceLeadLib._portalConfig) {
                    console.warn('No portal config available for metadata request');
                    return null;
                }
                ...
                console.log('Metadata XML received, length:', xmlText.length);
                ...
                console.log('Found EntityTypes:', entityTypes.length);
                ...
                for (let entity of entityTypes) {
                    ...
                    if (entityName === 'LS_LeadReport') {
                        targetEntity = entity;
                        console.log('Found LS_LeadReport entity');
                        break;
                    }
                }

                if (!targetEntity) {
                    console.warn('LS_LeadReport not found in metadata. Available entities:');
                    for (let entity of entityTypes) {
                ...
                console.log(`Loaded ${Object.keys(apiFields).length} fields from $metadata`);
                return apiFields;
        ...
        static async saveFieldMapping(eventId) {
            if (!eventId) {
                console.warn('No eventId provided for saveFieldMapping');
                return Promise.resolve(false);
            }

            try {
                // Get the field mapping service instance from the current container
                const container = document.querySelector('#fieldConfiguratorContainer');
                if (!container || !container._fieldMappingService) {
                    console.warn('Field mapping service not found');
                    return Promise.resolve(false);
                }
        ...
        static _openAddCustomFieldModal(rootElement, fieldMappingService) {
            ...
            // Get buttons by ID or structure
            // Close button (X) - first button in header
            const closeBtn = modal.querySelector('.sf-modal-content > div:first-child button');
            // Cancel button - by ID
            const cancelBtn = modal.querySelector('#cancel');
            // Save button - by ID
            const saveBtn = modal.querySelector('#ok');
                ...
                button.addEventListener('click', async (e) => {
                    ...
                    if (confirm(`Are you sure you want to delete the custom field "${fieldName}"?`)) {
                        try {
                            ...
                            if (fieldMappingService.currentEventId) {
                                await fieldMappingService.saveFieldMappingsToAPI(fieldName, 'custom_field_delete');
                                console.log(`Custom field "${fieldName}" deleted and saved to database`);
                            } else {
                                console.log(`Custom field "${fieldName}" deleted (localStorage only)`);
                            }
                            ...
                            SalesforceLeadLib._showToast(`Custom field "${fieldName}" deleted successfully`, 'success');
        ...
        static _openEditCustomFieldModal(rootElement, fieldMappingService, fieldId, fieldName) {
            ...
            // Save button - by ID
            const saveBtn = modal.querySelector('#editOk');
        ...
        static async openCrmTestExport(rootElement, eventId) {
            ...
            try {
                ...
                // Load data from LS_LeadReport endpoint (limit to 1 record for performance)
                // We only need one record to get the field structure
                const endpoint = `LS_LeadReport?$top=1&$format=json`;
                const leadDataResponse = await this._callPortalODataAPI(endpoint);

                console.log('LS_LeadReport response:', leadDataResponse);

                // Extract first result
                const rawLeadData = leadDataResponse && leadDataResponse.d && leadDataResponse.d.results && leadDataResponse.d.results[0];

                if (!rawLeadData) {
                    throw new Error('No data found in LS_LeadReport');
                }
                ...
                for (const key in rawLeadData) {
                    if (rawLeadData.hasOwnProperty(key) && key !== '__metadata') {
                        // Use the key name as the value (for preview purposes)
                        leadData[key] = key;
                    }
                }

                console.log('Transformed lead data for preview:', leadData);

                // Load field configuration from API (same as normal mode)
                ...
                // In test mode, set value = name for preview (to show field names instead of actual values)
                allFields.forEach(field => {
                    field.value = field.name;
                });

                console.log('Active fields for test export:', allFields.length, 'fields');

                // Create HTML interface (same as real CRM Export)
                const html = this._buildCrmExportHTML(leadData, allFields, `test-${eventId}`);
                rootElement.innerHTML = html;
        ...
        static async openCrmExport(rootElement, contactId) {
            ...
            try {
                ...
                // Load lead data via LS_LeadReportById (faster than LS_LeadReport with filter)
        ...
        static async _callPortalODataAPI(endpoint) {
            ...
            console.log('Using credentials - User:', this._portalConfig.user);
            console.log('Password exists:', !!this._portalConfig.password);
        ...
        static _buildActiveFieldsList(leadData, fieldConfig, customFields, customLabels = {}) {
            ...
            standardFields.forEach(sf => {
                ...
                if (hasFieldConfig) {
                    ...
                    if (config) {
                        // If field is in config, use its active status
                        isActive = config.active === true;
                    } else {
                        // Field not in config - default to active
                        isActive = true;
                    }
                } else {
                ...
                // Always show required fields, otherwise check isActive
                if (sf.required || isActive) {
            ...
            return `
                ...
                <div class="contenthost-background" style="max-width: 1400px; margin: 0 auto; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; min-height: 100vh;">
                                    ...
                                    ${allFields.map(field => `
                                        <tr style="border-bottom: 1px solid;">
                                            <td class="text-textcolor" style="padding: 12px;">
                                                <div style="font-weight: 500;">${this._escapeHtml(field.name)}</div>
                                                ${field.isCustomField
                        ...
                        <div id="cards-view" style="display: none; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 16px;">
                            ${allFields.map(field => `
                                <div class="sf-field-card" style="border-radius: 8px; padding: 16px;">
                                    ...
                                    <div class="text-textcolor" style="font-weight: 600; margin-bottom: 8px;">${this._escapeHtml(field.name)}</div>
                    ...
                    <div style="margin-top: 20px; display: flex; flex-wrap: wrap; gap: 12px; justify-content: flex-end;">
                        ...
                        <button id="ok" class="transferToSalesforceBtn" style="padding: 10px 24px; background: #10b981; color: white; border: none; border-radius: 8px; font-weight: 500; cursor: pointer; font-size: 14px;">
        ...
        static _attachCrmExportListeners(rootElement, leadData, allFields) {
            ...
            if (transferBtn) {
                transferBtn.addEventListener('click', async () => {
                    ...
                    transferBtn.innerHTML = '<span style="display: inline-block; width: 14px; height: 14px; border: 2px solid white; border-top-color: transparent; border-radius: 50%; animation: spin 0.6s linear infinite; margin-right: 8px;"></span>Transferring...';
                    transferBtn.style.opacity = '0.8';
                    ...
                    try {
                        ...
                        console.log('All fields available:', allFields);
                        ...
                        allFields.forEach(field => {
                            if (field && field.name) {
                                // Include all fields, even if empty (Salesforce might need them)
                                transferData[field.name] = field.value || '';
                            }
                        });

                        console.log('Transferring lead to Salesforce:', transferData);
                        console.log('Number of fields:', Object.keys(transferData).length);
                        ...
                        // Success!
                        transferBtn.innerHTML = '✓ Transferred';
