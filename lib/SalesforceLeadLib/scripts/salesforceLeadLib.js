    ...
    class SalesforceLeadLib {
        ...
        async _reactivateSession() {
            ...
            try {
                ...
                if (data.sessionId) {
                    localStorage.setItem('sf_session_token', data.sessionId);
                }
                return true;
            } catch {
                return false;
        ...
        static async openCrmExport(rootElement, contactId) {
            ...
            try {
                ...
                // Fetch export status columns from LS_Lead (not always in LS_LeadReport)
                try {
                    const exportStatusEndpoint = `LS_Lead?$filter=Id eq guid'${contactId}'&$select=LastExportStatus,LastExportMessage,LastExportTimestamp,ExportAttempts&$format=json`;
                    const exportStatusResponse = await this._callPortalODataAPI(exportStatusEndpoint);
                    const exportStatusData = exportStatusResponse && exportStatusResponse.d && exportStatusResponse.d.results && exportStatusResponse.d.results[0];
                    if (exportStatusData) {
                        leadData.LastExportStatus = exportStatusData.LastExportStatus || '';
                        leadData.LastExportMessage = exportStatusData.LastExportMessage || '';
                        leadData.LastExportTimestamp = exportStatusData.LastExportTimestamp || '';
                        leadData.ExportAttempts = exportStatusData.ExportAttempts || '';
                    }
                } catch (exportStatusError) {
                    // Non-blocking — banner just won't show previous status
                    console.warn('Could not load export status:', exportStatusError.message);
                }
        ...
        static _buildCrmExportHTML(leadData, allFields, contactId, options = {}) {
            ...
            const lastAttempts = leadData.ExportAttempts || '';
            ...
            const exportInfo = hasPreviousExport
                ? `<div id="export-status-banner" style="background:${bannerBg};border:1px solid ${bannerBorder};border-radius:8px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;gap:12px;">
                       ...
                       <div>
                           <div class="text-textcolor" style="font-weight:600;font-size:14px;color:${bannerColor};">${leadData.LastExportStatus}${lastAttempts ? ` (${lastAttempts}×)` : ''}</div>
                ...
                btn.addEventListener('click', async (event) => {
                    ...
                    try {
                        ...
                        if (response.status === 401) {
                            ...
                            if (reactivated) response = await doFetch();
                        }
        ...
        static async callSetLeadExportStatus(kontaktViewId, status, message, milliseconds) {
            ...
            try {
                ...
                if (response.status === 401) {
                    console.warn('[LS_SetLeadExportStatus] 401 Unauthorized — check portal credentials');
                    return null;
                }
            ...
            try {
                ...
                rootElement.innerHTML = `
                    <style>
                        ...
                        .sf-cl-status-pill { padding:3px 10px; border-radius:12px; font-size:12px; font-weight:600; cursor:pointer; border:1px solid var(--ColorLabel); background:var(--ColorTileBackground); color:var(--ColorText); transition:background .15s,color .15s,border-color .15s; white-space:nowrap; }
                        .sf-cl-status-pill.active { background:var(--ColorAccent); color:#fff; border-color:var(--ColorAccent); }
                ...
                const updateResetBtn = () => {
                    ...
                    const hasValue = [...rootElement.querySelectorAll('.sf-cl-filter-input')]
                        .some(i => i.value.trim() !== '');
                    resetBtn.disabled = !hasValue && activeStatusFilter === '';
                };
                ...
                const applyFilters = () => {
                    ...
                    const filtered = allItems.filter(item => {
                        ...
                        const s = (item.LastExportStatus || '').toLowerCase();
                        if (activeStatusFilter === '__empty') return !s;
                ...
                rootElement.querySelector('#sf-cl-status-filter')?.addEventListener('click', (e) => {
                    const pill = e.target.closest('.sf-cl-status-pill');
                    if (!pill) return;
                    activeStatusFilter = pill.dataset.status;
                    rootElement.querySelectorAll('.sf-cl-status-pill').forEach(p => p.classList.remove('active'));
