...
(function() {
    ...
    const STANDARD_SALESFORCE_LEAD_FIELDS = {
        // Must always be offered so it can be mapped to the SF External ID (LS_LeadId__c)
        // for upsert — without this mapping every transfer creates duplicates.
    ...
    // --ColorAccent can be any CSS color format; color-mix() isn't available in
    // older WebViews, so resolve it to "r, g, b" here and expose it as
    // --sf-accent-rgb for rgba(var(--sf-accent-rgb), .x) tints in the CSS.
    ...
    // Bolds field names/values in error text, e.g. "Email: invalid: test@x.com" -> "**Email**: invalid: **test@x.com**"
    ...
    // Toast messages often embed a field name the user typed or an error
    // string from an external API (Salesforce/HubSpot) — neither is trusted
    // input, so it must never reach innerHTML unescaped.
    ...
    // Native <dialog> with showModal() stacks correctly above WinJS panels, unlike
    // the legacy WinJS confirmModal.
    ...
    function showEditLabelDialog(title, fieldName, currentLabel, options = {}) {
        return new Promise((resolve) => {
            ...
            const cancelText = options.cancelText || 'Cancel';
            const providerLabel = options.providerLabel || 'Salesforce';
            const fieldNameExample = providerLabel === 'Salesforce' ? 'a custom field ending in __c' : 'a lowercase property name, e.g. lead_source';
            ...
            dialog.innerHTML = `
                ...
                <div class="sf-modal__body">
                    <div style="margin-bottom: 16px;">
                        <label class="sf-field-label" for="sf-lib-field-name">${_t('crmSettings.fieldNameReadonly', 'Field Name (Read-only)')}</label>
                        <input type="text" id="sf-lib-field-name" class="sf-input" readonly value="${fieldName}" />
                    </div>
                    <div>
                        <label class="sf-field-label" for="sf-lib-custom-label">${_t('crmSettings.providerTargetField', '{0} target field').replace('{0}', providerLabel)}</label>
                        <input type="text" id="sf-lib-custom-label" class="sf-input" value="${currentLabel}" />
                        <p class="sf-field-hint">
                            ${_t('crmSettings.enterTargetFieldHint', 'Enter the {0} field this maps to (e.g. {1}). Use {2} to remove the mapping and restore the original behavior.')
                                .replace('{0}', providerLabel)
                                .replace('{1}', fieldNameExample)
                                .replace('{2}', `<b>${_t('crmSettings.resetToDefault', 'Reset to default')}</b>`)}
                        </p>
                    </div>
                </div>
                <div class="sf-modal__footer sf-modal__footer--split">
                    <button id="sf-lib-reset-edit" class="sf-btn sf-btn--ghost sf-btn--sm">
                        <i class="fa-solid fa-rotate-left" aria-hidden="true"></i> ${_t('crmSettings.resetToDefault', 'Reset to default')}
                    </button>
            ...
            // Reset returns a sentinel so the caller can remove the custom mapping
    ...
    function showAddCustomFieldDialog(options = {}) {
        return new Promise((resolve) => {
            ...
            const cancelText = options.cancelText || 'Cancel';
            const providerLabel = options.providerLabel || 'Salesforce';
            const fieldNameExample = providerLabel === 'Salesforce' ? 'Area__c' : 'lead_source_detail';
            ...
            dialog.innerHTML = `
                <div class="sf-modal__header">
                    ...
                    <h3 class="sf-modal__title" id="sf-lib-dialog-title">${_t('crmSettings.addCustomField', 'Add Custom Field')}</h3>
                    <button id="sf-lib-close-x" class="sf-icon-btn" aria-label="${cancelText}">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                <div class="sf-modal__body">
                    <div style="margin-bottom: 16px;">
                        <label class="sf-field-label" for="sf-lib-field-name">${_t('crmSettings.fieldName', 'Field Name *')}</label>
                        <input type="text" id="sf-lib-field-name" class="sf-input" placeholder="${_t('crmSettings.fieldNamePlaceholder', 'e.g., {0}').replace('{0}', fieldNameExample)}" />
                        <p id="sf-lib-field-name-error" class="sf-field-error" role="alert"></p>
                        <p class="sf-field-hint">${_t('crmSettings.verifyFieldName', 'You must first verify the exact field name in {0} and enter it here without spaces.').replace('{0}', providerLabel)}</p>
                    </div>
                    <div>
                        <label class="sf-field-label" for="sf-lib-field-value">${_t('crmSettings.defaultValue', 'Default Value')}</label>
                        <textarea id="sf-lib-field-value" class="sf-textarea" rows="3" placeholder="${_t('crmSettings.defaultValuePlaceholder', 'e.g., Germany, France')}"></textarea>
                        <p class="sf-field-hint">${_t('crmSettings.optionalDefaultValue', 'Optional default value for this custom field')}</p>
                    </div>
                </div>
                <div class="sf-modal__footer">
            ...
            const validateAndSave = () => {
                ...
                if (!fieldName) {
                    errorEl.textContent = _t('crmSettings.fieldNameRequired', 'Field name is required.');
                ...
                if (!fieldName.match(/^[a-zA-Z][a-zA-Z0-9_]*(__c)?$/)) {
                    errorEl.textContent = _t('crmSettings.fieldNameInvalid', 'Invalid field name: no spaces allowed. Use only letters, numbers and underscores (e.g. {0}).').replace('{0}', fieldNameExample);
                    errorEl.style.display = 'block';
                    nameInput.classList.add('sf-input--error');
                    nameInput.focus();
                    return;
    ...
    function showEditCustomFieldDialog(fieldName, currentValue, options = {}) {
        return new Promise((resolve) => {
            ...
            const providerLabel = options.providerLabel || 'Salesforce';
            const fieldNameExample = providerLabel === 'Salesforce' ? 'Branch__c' : 'branch';
            const providerFieldNameLabel = providerLabel === 'Salesforce' ? 'SF Field Name' : _t('crmSettings.providerFieldName', '{0} Field Name').replace('{0}', providerLabel);
            ...
            dialog.innerHTML = `
                <div class="sf-modal__header">
                    ...
                    <h3 class="sf-modal__title" id="sf-lib-dialog-title">${_t('crmSettings.editCustomField', 'Edit Custom Field')}</h3>
                    <button id="sf-lib-close-x" class="sf-icon-btn" aria-label="${cancelText}">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
                <div class="sf-modal__body">
                    <div style="margin-bottom: 16px;">
                        <label class="sf-field-label" for="sf-lib-field-name">${providerFieldNameLabel}</label>
                        <input type="text" id="sf-lib-field-name" class="sf-input" value="${fieldName}" />
                        <p class="sf-field-hint">${_t('crmSettings.exactFieldApiName', 'Exact field API name in {0} (e.g. {1})').replace('{0}', providerLabel).replace('{1}', fieldNameExample)}</p>
                    </div>
                    <div style="margin-bottom: 16px;">
                        ...
                        <p class="sf-field-hint">${_t('crmSettings.fieldListNameHint', 'Name shown in the field list (leave empty to use the {0} field name)').replace('{0}', providerLabel)}</p>
                    </div>
                    <div>
                        <label class="sf-field-label" for="sf-lib-field-value">${_t('crmSettings.defaultValue', 'Default Value')}</label>
                        <textarea id="sf-lib-field-value" class="sf-textarea" rows="3">${currentValue}</textarea>
                        <p class="sf-field-hint">${_t('crmSettings.updateDefaultValue', 'Update the default value for this custom field')}</p>
                    </div>
                </div>
                <div class="sf-modal__footer">
                    <button id="sf-lib-cancel" class="sf-btn sf-btn--secondary">
    ...
    // Guard: a user may map a read-only LS source (e.g. CreatedById) TO a writable
    // SF custom field, but never use one of these read-only fields as the SF
    // TARGET — Salesforce would reject the create/update.
    ...
    // Falls back to a generic plug icon for a provider with no matching FA brand glyph (e.g. Dynamics 365).
    function _providerIconClass(apiEndpointValue) {
        if (apiEndpointValue === 'LeadSuccess_Event_API') return 'fa-brands fa-salesforce';
        if (apiEndpointValue === 'HubSpot') return 'fa-brands fa-hubspot';
    ...
    class SalesforceLeadLib {
        constructor(config = {}) {
            ...
            this.fieldMappingService = new CrmProviders.FieldMappingService({
                serverName: config.serverName,
                apiName: config.apiName,
                // Defaults keep Salesforce's own behavior for callers that don't pass these;
                // openFieldMapping passes HubSpot's own values for its instance instead.
                apiEndpointValue: config.apiEndpointValue || 'LeadSuccess_Event_API',
                defaultMappings: config.defaultMappings || STANDARD_SALESFORCE_LEAD_FIELDS,
                // Provider-scoped cache so HubSpot's fieldmap/apifields entries never collide with Salesforce's for the same eventId
                dataCache: config.dataCache || SalesforceLeadLib._dataCache,
                portalConfig: SalesforceLeadLib._portalConfig
            });
        ...
        async connect(credentials) {
            try {
                ...
                // Centered on the BROWSER WINDOW (not the physical screen) so it lands correctly on multi-monitor / RDP setups.
                const width = 600;
                const height = 750;
        ...
        async disconnect() {
            try {
                // Must read these BEFORE clearing localStorage — the backend needs
                // X-Org-Id/X-Session-Token to find the connection to remove, or the
        ...
        // Returns { salesforceData, externalIdField }; non-standard SF fields get __c suffix automatically.
        ...
        async _reactivateSession() {
            ...
            // Fallback: existing access token (valid ~2h)
        ...
        /**
         * Static so it survives WinJS page destroy/re-create on every tab switch.
         * Keys: 'contacts:{eventId}' -> { items, skip, hasMore, savedAt } (SWR);
         * 'fieldmap:{eventId}:{apiEndpointValue}' -> { configDataStr } (raw JSON,
        ...
        static _detectEnvironment() {
            ...
            // Anchored suffix match (exact host or "*.domain"), not a bare substring
            // check — `includes('convey.de')` would also match an attacker-controlled
            // host like "convey.de.evil.com".
            ...
            // Any localhost/127.0.0.1 is dev regardless of port — dev servers pick random
            // high ports, and treating "port !== 3000" as production wrongly pointed
            // the client at the prod backend, which CORS then blocked.
        ...
        static clear(rootElement) {
            ...
            // renderContactList's infinite-scroll IntersectionObserver keeps firing on its
            // now-detached sentinel until explicitly disconnected — without this, navigating
            // away mid-scroll can trigger a stale loadNextPage() writing into a gone tbody.
            if (rootElement._sfScrollObserver) {
                rootElement._sfScrollObserver.disconnect();
                rootElement._sfScrollObserver = null;
            }
        ...
        // eventId is a UUID from UniqueRecordId; null + options.recordId means localStorage-only mode.
        static async openFieldMapping(rootElement, eventId, options = {}) {
            ...
            // Defaults to Salesforce so existing callers keep today's behavior; HubSpot's adapter passes its own values.
            const providerConfig = {
                apiEndpointValue: options.apiEndpointValue || 'LeadSuccess_Event_API',
                providerLabel: options.providerLabel || 'Salesforce',
                defaultMappings: options.defaultMappings || STANDARD_SALESFORCE_LEAD_FIELDS,
                dataCache: options.dataCache || null,
                normalizeCrmField: options.normalizeCrmField || null,
                crmAdapter: options.crmAdapter || null
            };

            // Stored on rootElement (same pattern as _currentFilter/_fieldMappingService
            // below) so Add/Edit Custom Field dialogs can read the active provider's
            // label without threading providerConfig through every call site.
            if (rootElement) {
                rootElement._providerLabel = providerConfig.providerLabel;
            }
            ...
            try {
                ...
                // Skip the loading skeleton only when BOTH config AND API fields are cached —
                // the grid needs apiFields to render, so a config-only hit would still flash an empty grid.
                const dataCache = providerConfig.dataCache || SalesforceLeadLib._dataCache;
                const configCached = !localStorageOnlyMode &&
                    dataCache.has(`fieldmap:${eventId}:${providerConfig.apiEndpointValue}`) &&
                    dataCache.has(`apifields:${eventId}:${providerConfig.apiEndpointValue}`);
                if (!configCached) {
                rootElement.innerHTML = `
                    <div class="sf-fieldmap-root" aria-busy="true" aria-label="Loading fields">
                        <div class="sf-cl-page-header">
                            <div class="sf-cl-page-header__main">
                                ...
                                <div style="flex: 1; min-width: 260px;">
                                    <div class="sf-skeleton" style="height: 20px; max-width: 220px; margin-bottom: 8px;"></div>
                                    <div class="sf-skeleton" style="height: 13px; max-width: 320px;"></div>
                                </div>
                            </div>
                        ...
                        <div class="sf-cl-stat-cards">
                            <div class="sf-skeleton" style="height: 58px;"></div>
                            <div class="sf-skeleton" style="height: 58px;"></div>
                            <div class="sf-skeleton" style="height: 58px;"></div>
                            <div class="sf-skeleton" style="height: 58px;"></div>
                        </div>
                ...
                // `this` is already the class SalesforceLeadLib is invoked as
                // (static method), so `new this(...)` builds the right instance
                // whether called as SalesforceLeadLib.openFieldMapping(...) or
                // through a future subclass — same object `new SalesforceLeadLib`
                // would have built, just not hardcoding the class name.
                const instance = new this({
                    backendUrl: this._portalConfig.baseUrl,
                    serverName: this._portalConfig.serverUrl.replace(/^https?:\/\//, ''),
                    apiName: this._portalConfig.apiName,
                    apiEndpointValue: providerConfig.apiEndpointValue,
                    defaultMappings: providerConfig.defaultMappings,
                    // Without this, the constructor's own default (SalesforceLeadLib._dataCache,
                    // see the constructor below) kicks in instead of the provider-scoped Map
                    // computed above (HubspotLeadLib._dataCache for HubSpot, etc.) — so a HubSpot
                    // toggle/save here lands in the wrong Map while renderContactList's crmExport
                    // reads/writes the right one, permanently desyncing the two views for every
                    // non-Salesforce provider.
                    dataCache: providerConfig.dataCache
                });

                const html = `
                    <div class="contenthost-background sf-fieldmap-root">
                        <div style="flex: 1; display: flex; flex-direction: column; overflow: hidden;">
                            <header class="sf-cl-page-header">
                                <div class="sf-cl-page-header__main">
                                    <div class="sf-cl-page-header__icon"><i class="fa-solid fa-sliders" aria-hidden="true"></i></div>
                                    <div class="sf-cl-page-header__text">
                                        <h1 class="sf-cl-page-header__title">${_t('crmSettings.pageTitle', 'CRM Export Settings')}</h1>
                                        <div class="sf-cl-page-header__sub">
                                            <i class="${_providerIconClass(providerConfig.apiEndpointValue)}" aria-hidden="true"></i>
                                            <span>${this._escapeHtml(providerConfig.providerLabel)}</span>
                                            <span class="sf-cl-page-header__dot">&middot;</span>
                                            <span id="sf-status-indicator" class="sf-dot sf-dot--neutral sf-dot--pulse"></span>
                                            <span id="sf-status-text" class="sf-connection-chip__text">${_t('sforce.statusChecking', 'Checking…')}</span>
                                            <span id="sf-user-section" class="sf-connection-chip__user">
                                                <span class="sf-cl-page-header__dot">|</span>
                                ...
                                <div class="sf-fieldmap-actions">
                                    <button id="sf-connect-btn" class="sf-btn sf-btn--primary" style="display:none;">
                                        <i class="${_providerIconClass(providerConfig.apiEndpointValue)}" aria-hidden="true"></i> <span>Connect to ${providerConfig.providerLabel}</span>
                                    </button>
                            ...
                            <div class="sf-fieldmap-info-banner">
                                <i class="fa-solid fa-sliders" aria-hidden="true"></i>
                                <p id="event-info" class="sf-fieldmap-desc"><strong>Field Configurator — ${providerConfig.providerLabel}</strong> Configure which fields will be transferred to ${providerConfig.providerLabel} for this event. Required fields (LastName, Company) are always included and cannot be disabled.</p>
                            </div>

                            <main style="flex: 1; overflow: hidden; padding: 16px 0; display: flex; flex-direction: column;">
                                ...
                                <div class="sf-cl-stat-cards">
                                    <button type="button" class="sf-cl-stat-card" style="cursor: default;">
                                        <div class="sf-cl-stat-card__text">
                                            <div class="sf-cl-stat-card__label" data-win-res="{textContent: 'crmSettings.totalFields'}">Total Fields</div>
                                            <div class="sf-cl-stat-card__sub" data-win-res="{textContent: 'crmSettings.totalFieldsSub'}">All available fields</div>
                                        </div>
                                        <div id="totalFieldsCount" class="sf-cl-stat-card__value" style="color: var(--sf-accent);">0</div>
                                    </button>
                                    <button type="button" class="sf-cl-stat-card" style="cursor: default;">
                                        <div class="sf-cl-stat-card__text">
                                            <div class="sf-cl-stat-card__label" data-win-res="{textContent: 'crmSettings.activeFields'}">Active Fields</div>
                                            <div class="sf-cl-stat-card__sub" data-win-res="{textContent: 'crmSettings.activeFieldsSub'}">Ready for export</div>
                                        </div>
                                        <div id="activeFieldsCount" class="sf-cl-stat-card__value" style="color: var(--sf-success);">0</div>
                                    </button>
                                    <button type="button" class="sf-cl-stat-card" style="cursor: default;">
                                        <div class="sf-cl-stat-card__text">
                                            <div class="sf-cl-stat-card__label" data-win-res="{textContent: 'crmSettings.inactiveFields'}">Inactive Fields</div>
                                            <div class="sf-cl-stat-card__sub" data-win-res="{textContent: 'crmSettings.inactiveFieldsSub'}">Not exported</div>
                                        </div>
                                        <div id="inactiveFieldsCount" class="sf-cl-stat-card__value" style="color: var(--sf-text-2);">0</div>
                                    </button>
                                    <button type="button" class="sf-cl-stat-card" style="cursor: default;">
                                        <div class="sf-cl-stat-card__text">
                                            <div class="sf-cl-stat-card__label" data-win-res="{textContent: 'crmSettings.customFields'}">Custom Fields</div>
                                            <div class="sf-cl-stat-card__sub" data-win-res="{textContent: 'crmSettings.customFieldsSub'}">User-defined</div>
                                        </div>
                                        <div id="customFieldsCount" class="sf-cl-stat-card__value" style="color: var(--sf-accent);">0</div>
                                    </button>
                                </div>

                                <div class="sf-card" style="padding: 16px; flex: 1; display: flex; flex-direction: column; overflow: hidden;">
                                    <div class="sf-fieldmap-toolbar">
                                    ...
                                    <div id="fieldsContainer" style="flex: 1; overflow: hidden; display: flex; flex-direction: column;">
                                    ...
                                    <div class="sf-fieldmap-footer">
                                        <span id="settings-active-count" class="sf-fieldmap-footer__count"></span>
                                        <button id="settings-save-btn" class="sf-btn sf-btn--primary sf-fieldmap-footer__save">
                ...
                // WinJS's navigator appends the new page before removing the old one, so
                // rootElement can already be detached here if the page navigated away.
                ...
                try {
                    ...
                    const svc = instance.fieldMappingService;

                    if (configCached) {
                        // Apply the cached config synchronously and render immediately (no
                        // spinner/empty-grid flash), then revalidate in the background.
                        svc.currentEventId = effectiveEventId;
                        svc._applyConfigData(
                            svc.dataCache.get(`fieldmap:${eventId}:${svc.apiEndpointValue}`).configDataStr
                        );
                        svc.apiFields = svc.dataCache.get(`apifields:${eventId}:${svc.apiEndpointValue}`).apiFields;

                        this._renderFieldsGrid(rootElement, svc, 'active');
                        this._attachFieldConfiguratorListeners(rootElement, svc, providerConfig);

                        // Re-render only if #fieldsContainer is still present, i.e. the page hasn't navigated away.
                        Promise.all([
                            svc.initializeFields({}, effectiveEventId),
                            this._loadApiFieldsInBackground(rootElement, svc)
                        ]).then(() => {
                            if (rootElement.isConnected && rootElement.querySelector('#fieldsContainer')) {
                                this._renderFieldsGrid(rootElement, svc, rootElement._currentFilter || 'active');
                                this._attachFieldConfiguratorListeners(rootElement, svc, providerConfig);
                            }
                        ...
                        // Page may have navigated away while the cold load was in flight.
                        if (!rootElement.isConnected) return;
        ...
        static async _loadApiFieldsInBackground(rootElement, fieldMappingService) {
            try {
                ...
                // Serve the cached field list instantly (avoids the empty-grid flash on
                // every tab switch); the fetch below still runs to refresh it.
                const dataCache = fieldMappingService.dataCache || SalesforceLeadLib._dataCache;
                if (eventId) {
                    const cachedApiFields = dataCache.get(`apifields:${eventId}:${fieldMappingService.apiEndpointValue}`);
                    if (cachedApiFields && cachedApiFields.apiFields) {
                        fieldMappingService.apiFields = cachedApiFields.apiFields;
                    }
                }
                ...
                if (apiFields && Object.keys(apiFields).length > 0) {
                    ...
                    if (eventId) {
                        dataCache.set(`apifields:${eventId}:${fieldMappingService.apiEndpointValue}`, { apiFields });
                    }
                }
            } catch (apiError) {
        ...
        static async _loadFieldsFromMetadata() {
            try {
                ...
                // 'Id' is intentionally not excluded: it must stay mappable to LS_LeadId__c (upsert key).
        ...
        static _renderFieldsGrid(rootElement, fieldMappingService, currentFilter) {
            ...
            // Read the default field list off the instance (not the Salesforce-only module
            // constant) so this grid renders correctly for whichever provider is active.
            const standardSalesforceFields = fieldMappingService.defaultMappings || STANDARD_SALESFORCE_LEAD_FIELDS;
            // Short target-provider prefix shown as "<prefix>: fieldName" (e.g. 'SF', 'HS').
            // Defaults to 'SF' so existing Salesforce callers that construct a plain FieldMappingService
            // (no explicit short label) keep today's exact text.
            const targetLabel = fieldMappingService.targetLabel ||
                (fieldMappingService.apiEndpointValue === 'LeadSuccess_Event_API' ? 'SF' : fieldMappingService.apiEndpointValue);
            ...
            for (const fieldName in apiFields) {
                if (apiFields.hasOwnProperty(fieldName) && !processedFields.has(fieldName)) {
                    ...
                    if (config) {
                        ...
                        // No config = inactive by default, except required fields (LastName, Company)
                        isActive = isRequired;
                    }
            ...
            const activeCountFooter = rootElement.querySelector('#settings-active-count');
            if (activeCountFooter) activeCountFooter.textContent = `${activeCount} ${_t('crmSettings.fieldsActive', 'fields active')}`;
            ...
            if (filteredFields.length === 0) {
                ...
                let html = `
                    <div class="sf-fieldmap-table-wrap">
                        <table class="sf-fieldmap-table">
                            <thead>
                                <tr>
                                    <th class="sf-fieldmap-table__col-name">${_t('crmSettings.colFieldName', 'Field Name')}</th>
                                    <th>${_t('crmSettings.colSourceField', 'Source Field')}</th>
                                    <th class="sf-fieldmap-table__col-center">${_t('crmSettings.colRequired', 'Required')}</th>
                                    <th class="sf-fieldmap-table__col-center">${_t('crmSettings.colCustom', 'Custom')}</th>
                                    <th class="sf-fieldmap-table__col-center">${_t('crmSettings.colStatus', 'Status')}</th>
                                    <th class="sf-fieldmap-table__col-center">${_t('crmSettings.colActive', 'Active')}</th>
                                    <th class="sf-fieldmap-table__col-actions"></th>
                                </tr>
                            </thead>
                            <tbody>`;

                const checkOrDot = (on, colorClass) => on
                    ? `<i class="fa-solid fa-check ${colorClass}" aria-hidden="true"></i>`
                    : `<i class="fa-regular fa-circle sf-fieldmap-table__dot-off" aria-hidden="true"></i>`;
                ...
                filteredFields.forEach(field => {
                    ...
                    html += `
                        <tr class="field-item sf-fieldmap-row${isRequired ? ' required' : ''}${isActive ? ' active' : ''}${isCustom ? ' user-custom-field' : ''}"
                            data-field="${field.name}"
                            data-is-custom="${isCustom}"
                            data-field-id="${field.id || ''}">
                            <td class="sf-fieldmap-table__col-name">
                                ${isCustom ? `
                                    <div class="sf-fieldmap-card__name">${field.label}</div>
                                    <div class="sf-fieldmap-card__mapping">
                                        <span class="sf-fieldmap-card__muted">${targetLabel}:</span> ${field.name}
                                    </div>
                                ` : `
                                    <div class="sf-fieldmap-card__name">
                                        <span class="sf-fieldmap-card__muted">LS:</span> ${field.name}
                                        ${(fieldMappingService.apiEndpointValue === 'LeadSuccess_Event_API' && isNonWritableSalesforceField(field.name)) ? '<span class="sf-pill sf-pill--info sf-pill--xs" style="margin-left: 6px;" title="Salesforce system field — it cannot be a mapping TARGET, but you may map it as a SOURCE to a writable custom field.">SYSTEM</span>' : ''}
                                    </div>
                                `}
                            </td>
                            <td class="sf-fieldmap-card__mapping sf-fieldmap-table__col-source">
                                ${isCustom ? `
                                ...
                                ` : (field.hasCustomLabel
                                    ? `<span class="sf-fieldmap-card__muted">${targetLabel}:</span> ${field.customLabel}`
                                    : `<span class="sf-fieldmap-card__muted">LS:</span> ${field.name}`)}
                            </td>
                            <td class="sf-fieldmap-table__col-center">${checkOrDot(isRequired, 'sf-fieldmap-table__dot-required')}</td>
                            <td class="sf-fieldmap-table__col-center">${checkOrDot(isCustom, 'sf-fieldmap-table__dot-custom')}</td>
                            <td class="sf-fieldmap-table__col-center">
                                <span class="sf-pill sf-pill--${isActive ? 'success' : 'neutral'} sf-pill--xs">${isActive ? _t('crmSettings.active', 'Active') : _t('crmSettings.inactive', 'Inactive')}</span>
                            </td>
                            <td class="sf-fieldmap-table__col-center">
                                <label class="sf-toggle" onclick="event.stopPropagation();">
                            ...
                            <td class="sf-fieldmap-table__col-actions">
                                <div class="sf-fieldmap-table__actions-group">
                                    ${isCustom ? `
        ...
        static _attachInlineListeners(rootElement, fieldMappingService) {
            rootElement.querySelectorAll('.field-checkbox').forEach(checkbox => {
                checkbox.addEventListener('change', async (e) => {
                    ...
                    // Disable the checkbox until the save round-trip finishes, so a user can't
                    // jump to crmExport while the toggle is still in flight and see stale data.
                    // Also guard fieldConfig the same way the label editor does, so a concurrent
                    // background revalidation can't reassign it out from under this save.
                    e.target.disabled = true;
                    fieldMappingService._editInFlight = true;
                    ...
                    try {
                        ...
                        fieldMappingService._editInFlight = false;
        ...
        static _attachFieldConfiguratorListeners(rootElement, fieldMappingService, providerConfig) {
            ...
            if (searchField) {
                searchField.addEventListener('input', (e) => {
                    ...
                    fieldItems.forEach(item => {
                        const labelDiv = item.querySelector('.sf-fieldmap-card__name');
                        if (labelDiv) {
                            const label = labelDiv.textContent.toLowerCase();
                            item.style.display = label.includes(searchTerm) ? 'table-row' : 'none';
                        }
                    });
                });
            }
            ...
            filterTabs.forEach(tab => {
                tab.addEventListener('click', () => {
                    ...
                    this._renderFieldsGrid(rootElement, rootElement._fieldMappingService, filter);
                });
            });
            ...
            // providerConfig.crmAdapter is the CrmProviders registry adapter for whichever CRM is
            // active; falls back to Salesforce's own ...InSettings helpers when none is supplied.
            const crmAdapter = providerConfig.crmAdapter;
            ...
            if (sfConnectBtn) {
                sfConnectBtn.addEventListener('click', async () => {
                    if (crmAdapter) {
                        await crmAdapter.connect();
                        this._refreshConnectionStatusInSettings(rootElement, crmAdapter);
                    } else {
        ...
        static async _refreshConnectionStatusInSettings(rootElement, crmAdapter) {
            if (!rootElement.isConnected) { return; }
            let result;
            try {
                result = await crmAdapter.checkConnection();
            } catch (err) {
            ...
            if (indicator) {
                // Drop the initial "checking" state (neutral + pulse) now that the
                // real status is known — sf-dot--neutral was never removed before,
                // and since it's declared after sf-dot--success in the stylesheet,
                // it silently won the cascade and kept the dot grey even when
                // sf-dot--success was also present.
                indicator.classList.remove('sf-dot--neutral', 'sf-dot--pulse');
                indicator.classList.toggle('sf-dot--success', connected);
                indicator.classList.toggle('sf-dot--warning', !connected);
            }
            if (statusText) { statusText.textContent = connected ? 'Connected' : 'Disconnected'; }
            if (statusText) { statusText.classList.toggle('connected', connected); }
            ...
            if (connected && result.userInfo) {
                const display = result.userInfo;
                if (userName) { userName.textContent = display; }
                if (userAvatar) { userAvatar.textContent = display.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase(); }
            }
        }
        ...
        static async _openAddCustomFieldModal(rootElement, fieldMappingService) {
            const result = await showAddCustomFieldDialog({
                saveText: 'Save Field',
                cancelText: 'Cancel',
                providerLabel: rootElement?._providerLabel || 'Salesforce'
            });
        ...
        static _updateStatisticsCounter(rootElement, fieldMappingService) {
            ...
            // isActive must use the SAME rule as _renderFieldsGrid (no config = required
            // fields count as active) — the two used to disagree, causing the Active
            // counter to jump to a wrong value when toggling a field.
            for (const fieldName in apiFields) {
                if (apiFields.hasOwnProperty(fieldName) && !processedFields.has(fieldName)) {
                    const isStandardSalesforce = standardSalesforceFields.hasOwnProperty(fieldName);
                    const sfInfo = isStandardSalesforce ? standardSalesforceFields[fieldName] : null;
        ...
        static async _openEditFieldLabelModal(rootElement, fieldMappingService, fieldName) {
            ...
            // Blocks a concurrent background revalidation from reassigning fieldConfig to a
            // fresh object while this dialog is open, which would silently orphan the in-place
            // edit below and cause the save to persist the pre-edit config. Cleared in
            // `finally` so it can't get stuck if this method throws or returns early.
            fieldMappingService._editInFlight = true;
            try {
            ...
            // Each provider rejects different things for a mapping TARGET, so each gets its own check.
            const targetApiEndpoint = fieldMappingService?.apiEndpointValue || 'LeadSuccess_Event_API';
            if (targetApiEndpoint === 'LeadSuccess_Event_API' && isNonWritableSalesforceField(newLabel)) {
            ...
            // HubSpot rejects any property name containing an uppercase letter
            // ("Names for properties should be lowercase") — catch a typo like
            // "Address" here, before it reaches the batch transfer and fails
            // every lead that used it (the bug this exact rule was added to fix).
            if (targetApiEndpoint === 'HubSpot' && newLabel.trim() !== newLabel.trim().toLowerCase()) {
                SalesforceLeadLib._showToast(
                    `"${newLabel.trim()}" must be lowercase — HubSpot property names cannot contain uppercase letters (e.g. use "address", not "Address").`,
                    'error'
                );
                return;
            }
        ...
        static async _openEditCustomFieldModal(rootElement, fieldMappingService, fieldId, fieldName) {
            // Match by id first, but fall back to the SF field name: after a config reload the
            // service holds fresh objects, and a strict id match alone can wrongly miss.
            const customFields = fieldMappingService.customFields || [];
            ...
            try {
                ...
                if (skipLeadDataCall) {
                    ...
                    if (rawLeadData) {
                        ...
                        // No contacts for this event yet — use default fields so mappings can still be configured
                        leadData = this._getDefaultLeadReportFields();
                    }
                }
                ...
                // Lets custom fields use their configured default value instead of "Test_fieldName"
        ...
        static _loadFieldConfig(eventId) {
            try {
                // Global key used by FieldMappingService; structure is { config: { fields: [...] } }
        ...
        // Standard LS_LeadReport fields for previewing mappings when the event has no contacts yet
        ...
        // Mirrors _renderFieldsGrid's active-field logic for consistency between crmSettings and crmExport
            ...
            if (hasFieldConfig) {
                fieldConfigArray.forEach(configField => {
                    ...
                    if (isActive || isRequired) {
                        ...
                        // QuestionXX holds the question title; AnswersXX holds the actual answer
                ...
                // Only add required fields if missing — do NOT auto-add other standard SF fields
                // not in config, so crmExport shows exactly what was configured in crmSettings.
                ...
                // No config saved yet — only required fields; the rest stay inactive until configured
                requiredFieldNames.forEach(reqFieldName => {
                    if (!addedFieldNames.has(reqFieldName)) {
                        const sfInfo = standardSalesforceFields[reqFieldName];
                        const customLabel = customLabels[reqFieldName];
         ...
         * @param {Object} [options.transferAdapter] - Provider-specific transfer
         *   logic: { buildContactData(itemData, fieldMappingService),
         *   detectUpsertKey(fieldMappingService), transferOne(contactData,
         *   attachments, upsertKey) -> result }. Omit for Salesforce's own
         *   behavior (buildLeadDataFromItem/detectExternalIdField/
         *   transferSingleLead) — other providers (HubspotLeadLib,
         *   DynamicsLeadLib) pass their own via providerConfig.transferAdapter.
         *   transferOne's return shape must match transferSingleLead's exactly
         *   (status/message/salesforceId/duplicateWarning/isUpdate/
         *   attachmentsTransferred) since the result handling below and in
         *   the click handler (~line 6900) is shared across all providers.
         * @returns {Object} Summary { results[], successCount, failedCount, duplicateCount, skippedCount, totalMs, cancelled }
         */
        /**
         * Pre-flight, client-side-only check of a selection before it's sent
         * to executeBatchTransfer. Never touches the network — everything it
         * looks at (item field values) is already loaded in memory. Reuses
         * the exact same buildContactData/transferAdapter resolution
         * executeBatchTransfer itself uses, so "will this fail" here matches
         * what would actually happen during the real transfer.
         * @param {Array} items - Array of itemData objects (from getSelectedItems)
         * @param {Object} options - { fieldMappingService, transferAdapter }
         * @returns {{ valid: Array, invalid: Array }} invalid entries are
         *   { item, reasons: string[] } where reasons is a subset of
         *   ['missingRequired', 'invalidEmail'].
         */
        static validateLeadsForTransfer(items, options = {}) {
            const { fieldMappingService, transferAdapter } = options;
            const buildContactData = transferAdapter
                ? (item) => transferAdapter.buildContactData(item, fieldMappingService)
                : (item) => this.buildLeadDataFromItem(item, fieldMappingService);
            // Simple RFC 5322 "good enough" check — matches the level of
            // strictness already used elsewhere in this codebase (none), so
            // this only catches clearly-malformed values, not edge cases.
            ...
            items.forEach((item) => {
                ...
                if (!item.LastName && !item.Company) {
                    reasons.push('missingRequired');
                }

                let contactData = null;
                try {
                    contactData = buildContactData(item);
                } catch (e) {
                    // If building the payload itself throws, treat as no email
                    // info available rather than failing the whole preview.
                }
                const emailValue = contactData ? (contactData.email || contactData.Email) : null;
                if (emailValue && !EMAIL_RE.test(String(emailValue).trim())) {
                    reasons.push('invalidEmail');
                }
        ...
        static async executeBatchTransfer(items, options = {}) {
            const { fieldMappingService, onProgress, onLeadComplete, skipExportStatus, transferAdapter, bypassRequiredFieldsCheck } = options;
            const buildContactData = transferAdapter
                ? (item) => transferAdapter.buildContactData(item, fieldMappingService)
                : (item) => this.buildLeadDataFromItem(item, fieldMappingService);
            const detectUpsertKey = transferAdapter
                ? () => transferAdapter.detectUpsertKey(fieldMappingService)
                : () => this.detectExternalIdField(fieldMappingService);
            const transferOne = transferAdapter
                ? (contactData, attachments, upsertKey) => transferAdapter.transferOne(contactData, attachments, upsertKey)
                : (contactData, attachments, upsertKey) => this.transferSingleLead(contactData, attachments, upsertKey);
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                // Validate required fields — skipped when the user explicitly
                // chose "Export anyway" on the Preview & Validate modal after
                // being shown which leads would trigger this: at that point
                // sending them (and letting the CRM itself reject them) is a
                // deliberate, informed choice, not a silent auto-skip.
                if (!bypassRequiredFieldsCheck && !item.LastName && !item.Company) {
                    const result = {
                        index: i,
                        itemData: item,
                        displayName,
                ...
                try {
                    // 1. Build the provider's payload shape
                    const leadData = buildContactData(item);

                    // 2. Fetch attachments
                    const attachments = await this.fetchAttachmentsForBatch(item.AttachmentIdList);

                    // 3. Transfer (with upsert key if configured)
                    const upsertKey = detectUpsertKey();
                    const transferResult = await transferOne(leadData, attachments, upsertKey);
         ...
         */
        static showBatchSummaryModal(summary, onClose, crmIdLabel = 'SF', providerIconClass = 'fa-brands fa-salesforce') {
            const modal = document.createElement('div');
            ...
            // Clicking a stat tile filters the result list below to just that
            // status (success/failed/duplicate/skipped); clicking the active
            // tile again clears back to "all". data-status-filter carries the
            // status value the click handler below reads, rather than
            // re-deriving it from the label text.
            const buildStatBox = (label, count, stateClass, statusFilter) => `
                <button type="button" class="sf-stat sf-stat--${stateClass} sf-stat--clickable" data-status-filter="${statusFilter}" aria-pressed="false">
            ...
            const buildResultRow = (r) => {
                ...
                const sfId = r.salesforceId ? `<span class="sf-result-row__meta">${crmIdLabel}: ${r.salesforceId}</span>` : '';
            ...
            modal.innerHTML = `
                <div class="sf-modal" role="dialog" aria-modal="true" aria-labelledby="batch-summary-title">
                    <div class="sf-modal__header">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            <h2 class="sf-modal__title" id="batch-summary-title"><i class="${providerIconClass}" aria-hidden="true" style="margin-right: 8px;"></i>${headerText}</h2>
                    ...
                    <div style="display: flex; border-bottom: 1px solid var(--sf-border);">
                        ${buildStatBox(_t('crmExport.statSuccess', 'Success'), summary.successCount, 'success', 'success')}
                        ${buildStatBox(_t('crmExport.statFailed', 'Failed'), summary.failedCount, 'error', 'failed')}
                        ${buildStatBox(_t('crmExport.statDuplicate', 'Duplicate'), summary.duplicateCount, 'warning', 'duplicate')}
                        ${buildStatBox(_t('crmExport.statSkipped', 'Skipped'), summary.skippedCount, 'neutral', 'skipped')}
                    </div>
                    <div id="batch-summary-results" style="flex: 1; overflow-y: auto; max-height: 350px; padding: 4px 12px;">
            ...
            // Stat-tile filtering: click a tile to show only that status,
            // click the already-active tile to clear back to "all".
            const resultsContainer = modal.querySelector('#batch-summary-results');
            const statButtons = [...modal.querySelectorAll('.sf-stat--clickable')];
            let activeFilter = null;
            const applyFilter = () => {
                const visible = activeFilter
                    ? summary.results.filter(r => r.status === activeFilter)
                    : summary.results;
                resultsContainer.innerHTML = visible.length
                    ? visible.map(r => buildResultRow(r)).join('')
                    : `<p style="text-align:center;color:var(--sf-text-2);padding:24px;">${_t('crmExport.noResultsForFilter', 'No leads with this status.')}</p>`;
                statButtons.forEach(btn => {
                    const isActive = btn.dataset.statusFilter === activeFilter;
                    btn.setAttribute('aria-pressed', String(isActive));
                    btn.classList.toggle('sf-stat--active', isActive);
                });
            };
            statButtons.forEach(btn => {
        ...
        /**
         * Show the pre-transfer validation report (built by
         * validateLeadsForTransfer). Only called when there's at least one
         * invalid lead — the caller skips straight to showBatchConfirmModal
         * when everything is already valid, so this never adds an extra
         * click for the common case.
         * @param {{ valid: Array, invalid: Array }} report
         * @param {string} providerLabel - e.g. 'Salesforce' / 'HubSpot'
         * @returns {Promise<'validOnly'|'all'|null>} which set to proceed
         *   with, or null if the user cancelled.
         */
        /**
         * Navigate to the app's own Edit Contact page (pages/contact/) for a
         * specific lead.
         *
         * KNOWN ISSUE (not fixed here): something reloads contactList's
         * master after this navigation and, not finding the record in its
         * own freshly-fetched page, zeroes AppData's Kontakt id and blanks
         * the right panel. A real fix needs a change in
         * pages/contactList/contactListController.js's selectRecordId()
         * (shared file, tracked separately — see conversation with Gilbert).
         */
        static _navigateToEditContact(kontaktViewId) {
            if (!kontaktViewId || typeof AppData === 'undefined' || typeof Application === 'undefined') return;
            AppData.setRecordId('Kontakt', kontaktViewId);
            Application.navigateById('contact');
        }

        static showBatchValidationModal(report, providerLabel) {
            return new Promise((resolve) => {
                const modal = document.createElement('div');
                modal.className = 'sf-modal-overlay';
                modal.style.zIndex = '10003';

                const missingCount = report.invalid.filter(r => r.reasons.includes('missingRequired')).length;
                const emailCount = report.invalid.filter(r => r.reasons.includes('invalidEmail')).length;
                const total = report.valid.length + report.invalid.length;

                const REASON_LABEL = {
                    missingRequired: _t('crmExport.missingRequiredFields', 'Missing required fields (Last Name or Company)'),
                    invalidEmail: _t('crmExport.invalidEmail', 'Invalid email format')
                };

                // Clicking a stat tile filters the invalid-rows list below to
                // just that reason (mirrors showBatchSummaryModal's stat-tile
                // filtering exactly). "Valid" has no rows to show (valid leads
                // aren't listed here at all) so it's not clickable.
                const buildStatBox = (label, count, stateClass, reasonFilter) => reasonFilter ? `
                    <button type="button" class="sf-stat sf-stat--${stateClass} sf-stat--clickable" data-reason-filter="${reasonFilter}" aria-pressed="false">
                        <div class="sf-stat__value">${count}</div>
                        <div class="sf-stat__label">${label}</div>
                    </button>
                ` : `
                ...
                // getLeadDisplayName falls back to 'Unknown Lead' when
                // FirstName/LastName/Company/Email are all empty — exactly
                // the leads flagged 'missingRequired' here. Fall back further
                // to the lead's own Id so rows stay distinguishable instead
                // of showing the same label repeated for every such lead.
                const displayNameFor = (item) => {
                    const name = this.getLeadDisplayName(item);
                    if (name !== 'Unknown Lead') return name;
                    const id = item.Id || item.KontaktViewId;
                    return id ? `${_t('crmExport.unknownLeadWithId', 'Unknown Lead')} (${id})` : name;
                };

                // Editing requires the local Kontakt id (KontaktViewId), not
                // the UUID (Id) used elsewhere for CRM transfer — see the
                // "contact" page (pages/contact/), which reads AppData's
                // stored "Kontakt" record id, keyed by KontaktViewId, not Id.
                const buildInvalidRow = (entry) => {
                    const name = displayNameFor(entry.item);
                    const reasonText = entry.reasons.map(r => REASON_LABEL[r] || r).join(' · ');
                    const kontaktViewId = entry.item.KontaktViewId;
                    const editBtn = kontaktViewId
                        ? `<button type="button" class="sf-result-row__edit" data-kontakt-view-id="${this._escapeHtml(String(kontaktViewId))}">${_t('crmExport.editContact', 'Edit')}</button>`
                        : '';
                    return `
                        <div class="sf-result-row" data-reasons="${entry.reasons.join(',')}">
                            <span class="sf-pill sf-pill--warning" style="width: 22px; justify-content: center; padding: 4px 0;" aria-hidden="true">!</span>
                            <div style="flex: 1; min-width: 0;">
                                <div class="sf-result-row__name">${this._escapeHtml(name)}</div>
                                <div class="sf-result-row__meta">${this._escapeHtml(reasonText)}</div>
                            </div>
                            ${editBtn}
                        </div>
                    `;
                ...
                modal.innerHTML = `
                    <div class="sf-modal" role="dialog" aria-modal="true" aria-labelledby="sf-validate-title">
                        <div class="sf-modal__header">
                            <span class="sf-icon-badge sf-icon-badge--warning" aria-hidden="true">
                                <i class="fa-solid fa-triangle-exclamation"></i>
                            </span>
                            <div style="flex: 1; min-width: 0;">
                                <h2 class="sf-modal__title" id="sf-validate-title">${_t('crmExport.previewValidate', 'Preview & Validate')}</h2>
                                <p class="sf-modal__subtitle">${total} ${_t('crmExport.leadsToTransfer', 'leads to transfer to')} ${this._escapeHtml(providerLabel)}</p>
                            </div>
                        </div>
                        <div style="display: flex; border-bottom: 1px solid var(--sf-border);">
                            ${buildStatBox(_t('crmExport.statValid', 'Valid'), report.valid.length, 'success', null)}
                            ${missingCount > 0 ? buildStatBox(_t('crmExport.statMissingRequired', 'Missing fields'), missingCount, 'error', 'missingRequired') : ''}
                            ${emailCount > 0 ? buildStatBox(_t('crmExport.statInvalidEmail', 'Invalid email'), emailCount, 'warning', 'invalidEmail') : ''}
                        </div>
                        <div id="sf-validate-results" style="flex: 1; overflow-y: auto; max-height: 350px; padding: 4px 12px;">
                            ${report.invalid.map(entry => buildInvalidRow(entry)).join('')}
                        </div>
                        <div class="sf-modal__footer">
                            <button id="sf-validate-cancel" class="sf-btn sf-btn--secondary">${_t('crmExport.btnCancel', 'Cancel')}</button>
                            <button id="sf-validate-all" class="sf-btn sf-btn--secondary">${_t('crmExport.exportAnyway', 'Export anyway')}</button>
                            <button id="sf-validate-valid-only" class="sf-btn sf-btn--primary">${_t('crmExport.exportValidOnly', 'Export valid only')} (${report.valid.length})</button>
                        </div>
                    </div>
                `;
                ...
                // Stat-tile filtering: click a tile to show only invalid rows
                // carrying that reason, click the already-active tile again
                // to clear back to "all invalid".
                const resultsContainer = modal.querySelector('#sf-validate-results');
                const statButtons = [...modal.querySelectorAll('.sf-stat--clickable')];
                let activeReasonFilter = null;
                const applyReasonFilter = () => {
                    const visible = activeReasonFilter
                        ? report.invalid.filter(entry => entry.reasons.includes(activeReasonFilter))
                        : report.invalid;
                    resultsContainer.innerHTML = visible.length
                        ? visible.map(entry => buildInvalidRow(entry)).join('')
                        : `<p style="text-align:center;color:var(--sf-text-2);padding:24px;">${_t('crmExport.noResultsForFilter', 'No leads with this status.')}</p>`;
                    statButtons.forEach(btn => {
                        const isActive = btn.dataset.reasonFilter === activeReasonFilter;
                        btn.setAttribute('aria-pressed', String(isActive));
                        btn.classList.toggle('sf-stat--active', isActive);
                    });
                };
                statButtons.forEach(btn => {
                    btn.addEventListener('click', () => {
                        const reason = btn.dataset.reasonFilter;
                        activeReasonFilter = (activeReasonFilter === reason) ? null : reason;
                        applyReasonFilter();
                    });
                });
                ...
                modal.querySelector('#sf-validate-cancel').addEventListener('click', () => cleanup(null));
                modal.querySelector('#sf-validate-all').addEventListener('click', () => cleanup('all'));
                modal.querySelector('#sf-validate-valid-only').addEventListener('click', () => cleanup('validOnly'));
                modal.addEventListener('click', (e) => { if (e.target === modal) cleanup(null); });

                // "Edit" on a flagged lead — close this modal and jump to the
                // app's own Edit Contact page (pages/contact/) for that lead,
                // so the user can fix the data there. Delegated on the
                // container (not per-row) since applyReasonFilter re-renders
                // its innerHTML on every stat-tile click.
                resultsContainer.addEventListener('click', (e) => {
                    const btn = e.target.closest('.sf-result-row__edit');
                    if (!btn) return;
                    const kontaktViewId = btn.dataset.kontaktViewId;
                    if (!kontaktViewId) return;
                    cleanup(null); // treat as cancelled — the user left to go fix data, not to proceed with a transfer
                    SalesforceLeadLib._navigateToEditContact(kontaktViewId);
                });
            });
        }

        /**
         * Show the "Check Duplicates" result recap — same layout family as
         * showBatchValidationModal (stat tiles + scrollable flagged-row
         * list), but for the pre-flight duplicate check rather than field
         * validation. Purely a display of results already fetched from the
         * CRM — makes no network calls itself.
         * @param {{ clean: Array, flagged: Array }} report - flagged entries
         *   are { item, status: 'synced'|'email_conflict', hubspotId, url }
         * @param {string} providerLabel - e.g. 'HubSpot'
         * @returns {Promise<'exclude'|null>} 'exclude' if the user asked to
         *   exclude the flagged leads from the current selection, null if
         *   they just closed the modal.
         */
        /**
         * Progress modal shown while Check Duplicates' single batch request is
         * in flight. There's no real per-lead count from the server (unlike
         * the batch-transfer progress modal, this isn't a client-side loop),
         * so the bar is indeterminate — it communicates "working", not a
         * fake percentage.
         * @returns {{ modal: HTMLElement, close: Function }}
         */
        static showDuplicateCheckProgressModal(totalCount, providerLabel) {
            const modal = document.createElement('div');
            modal.id = 'dupcheck-progress-modal';
            modal.className = 'sf-modal-overlay';
            modal.style.zIndex = '10003';

            modal.innerHTML = `
                <div class="sf-modal" role="dialog" aria-modal="true" aria-labelledby="dupcheck-progress-title">
                    <div class="sf-modal__header">
                        <span class="sf-icon-badge sf-icon-badge--accent" aria-hidden="true">
                            <i class="fa-solid fa-layer-group"></i>
                        </span>
                        <div style="flex: 1; min-width: 0;">
                            <h2 class="sf-modal__title" id="dupcheck-progress-title">${_t('crmExport.dupCheckTitle', 'Duplicate Detection')}</h2>
                            <p class="sf-modal__subtitle">${_t('crmExport.dupCheckInProgress', 'Checking {0} lead(s) against {1}').replace('{0}', totalCount).replace('{1}', this._escapeHtml(providerLabel))}</p>
                        </div>
                    </div>
                    <div class="sf-modal__body">
                        <div class="sf-progress sf-progress--indeterminate" role="progressbar" aria-valuetext="${_t('crmExport.checking', 'Checking…')}">
                            <div class="sf-progress__bar"></div>
                        </div>
                    </div>
                </div>
            `;
        ...
        static showDuplicateCheckModal(report, providerLabel) {
            return new Promise((resolve) => {
                const modal = document.createElement('div');
                modal.className = 'sf-modal-overlay';
                modal.style.zIndex = '10003';

                const syncedCount = report.flagged.filter(f => f.status === 'synced').length;
                const conflictCount = report.flagged.filter(f => f.status === 'email_conflict').length;
                const total = report.clean.length + report.flagged.length;

                const STATUS_LABEL = {
                    synced: _t('crmExport.dupCheckSynced', 'Already in {0}').replace('{0}', providerLabel),
                    email_conflict: _t('crmExport.dupCheckEmailConflict', 'Email already used by a different contact')
                };

                const buildStatBox = (label, count, stateClass, statusFilter) => statusFilter ? `
                    <button type="button" class="sf-stat sf-stat--${stateClass} sf-stat--clickable" data-status-filter="${statusFilter}" aria-pressed="false">
                        <div class="sf-stat__value">${count}</div>
                        <div class="sf-stat__label">${label}</div>
                    </button>
                ...
                const buildFlaggedRow = (entry) => {
                    const name = displayNameFor(entry.item);
                    const stateClass = entry.status === 'synced' ? 'success' : 'warning';
                    const linkHtml = entry.url
                        ? `<a href="${this._escapeHtml(entry.url)}" target="_blank" rel="noopener" class="sf-result-row__edit">${_t('crmExport.viewInCrm', 'View in {0}').replace('{0}', providerLabel)}</a>`
                        : '';
                    return `
                        <div class="sf-result-row" data-status="${entry.status}">
                            <span class="sf-pill sf-pill--${stateClass}" style="width: 22px; justify-content: center; padding: 4px 0;" aria-hidden="true">${entry.status === 'synced' ? '✓' : '!'}</span>
                            <div style="flex: 1; min-width: 0;">
                                <div class="sf-result-row__name">${this._escapeHtml(name)}</div>
                                <div class="sf-result-row__meta">${STATUS_LABEL[entry.status]}</div>
                            </div>
                            ${linkHtml}
                        </div>
                    `;
                ...
                modal.innerHTML = `
                    <div class="sf-modal" role="dialog" aria-modal="true" aria-labelledby="sf-dupcheck-title">
                        <div class="sf-modal__header">
                            <span class="sf-icon-badge sf-icon-badge--${report.flagged.length > 0 ? 'warning' : 'success'}" aria-hidden="true">
                                <i class="fa-solid ${report.flagged.length > 0 ? 'fa-triangle-exclamation' : 'fa-circle-check'}"></i>
                            </span>
                            <div style="flex: 1; min-width: 0;">
                                <h2 class="sf-modal__title" id="sf-dupcheck-title">${_t('crmExport.dupCheckTitle', 'Duplicate Detection')}</h2>
                                <p class="sf-modal__subtitle">${total} ${_t('crmExport.leadsChecked', 'leads checked against')} ${this._escapeHtml(providerLabel)}</p>
                            </div>
                        </div>
                        <div style="display: flex; border-bottom: 1px solid var(--sf-border);">
                            ${buildStatBox(_t('crmExport.dupCheckClean', 'Clean leads'), report.clean.length, 'success', null)}
                            ${syncedCount > 0 ? buildStatBox(_t('crmExport.filterAlreadyIn', 'Already in {0}').replace('{0}', providerLabel), syncedCount, 'success', 'synced') : ''}
                            ${conflictCount > 0 ? buildStatBox(_t('crmExport.filterEmailConflict', 'Email conflict'), conflictCount, 'error', 'email_conflict') : ''}
                        </div>
                        <div id="sf-dupcheck-results" style="flex: 1; overflow-y: auto; max-height: 350px; padding: 4px 12px;">
                            ${report.flagged.map(entry => buildFlaggedRow(entry)).join('')}
                        </div>
                        <div class="sf-modal__footer">
                            <button id="sf-dupcheck-close" class="sf-btn sf-btn--secondary">${_t('crmExport.btnClose', 'Close')}</button>
                            ${report.flagged.length > 0 ? `<button id="sf-dupcheck-exclude" class="sf-btn sf-btn--primary">${_t('crmExport.applyExclude', 'Apply & Exclude Duplicates')}</button>` : ''}
                        </div>
                    </div>
                `;
                ...
                const resultsContainer = modal.querySelector('#sf-dupcheck-results');
                const statButtons = [...modal.querySelectorAll('.sf-stat--clickable')];
                let activeFilter = null;
                const applyFilter = () => {
                    const visible = activeFilter
                        ? report.flagged.filter(entry => entry.status === activeFilter)
                        : report.flagged;
                    resultsContainer.innerHTML = visible.length
                        ? visible.map(entry => buildFlaggedRow(entry)).join('')
                        : `<p style="text-align:center;color:var(--sf-text-2);padding:24px;">${_t('crmExport.noResultsForFilter', 'No leads with this status.')}</p>`;
                    statButtons.forEach(btn => {
                        const isActive = btn.dataset.statusFilter === activeFilter;
                        btn.setAttribute('aria-pressed', String(isActive));
                ...
                modal.querySelector('#sf-dupcheck-close').addEventListener('click', () => cleanup(null));
                modal.querySelector('#sf-dupcheck-exclude')?.addEventListener('click', () => cleanup('exclude'));
                modal.addEventListener('click', (e) => { if (e.target === modal) cleanup(null); });
            });
        }

        /**
         * Show a read-only detail view for a single lead — the columns
         * currently visible in the table (so it reflects the same active
         * field mapping the user already configured), plus its current
         * export status. Purely a display of already-loaded data, no
         * network calls.
         * @param {Object} item - The lead's raw itemData
         * @param {Array<string>} displayColumns - Same column list the
         *   table itself renders, passed in by the caller (renderContactList's
         *   closure) rather than recomputed here
         * @param {(col: string, val: any) => string} formatCell - Same date/
         *   value formatter the table uses, so dates render identically
         * @returns {Promise<void>}
         */
        static showLeadDetailModal(item, displayColumns, formatCell) {
            return new Promise((resolve) => {
                const modal = document.createElement('div');
                modal.className = 'sf-modal-overlay';
                modal.style.zIndex = '10003';
                ...
                const name = this.getLeadDisplayName(item);
                const statusRaw = item.LastExportStatus || '';
                const stateClass = statusRaw ? (this.BATCH_STATUS_CLASS[statusRaw.toLowerCase()] || 'neutral') : 'neutral';

                const rows = displayColumns
                    .filter(col => item[col] !== undefined && item[col] !== null && String(item[col]).trim() !== '')
                    .map(col => `
                        <div class="sf-result-row">
                            <div class="sf-result-row__name" style="flex: 0 0 180px; color: var(--sf-text-2); font-weight: 600;">${this._escapeHtml(col)}</div>
                            <div style="flex: 1; min-width: 0; overflow-wrap: anywhere;">${this._escapeHtml(String(formatCell(col, item[col])))}</div>
                        </div>
                    `).join('');

                modal.innerHTML = `
                    <div class="sf-modal" role="dialog" aria-modal="true" aria-labelledby="sf-leaddetail-title">
                        <div class="sf-modal__header">
                            <span class="sf-icon-badge sf-icon-badge--${stateClass === 'success' ? 'success' : stateClass === 'error' ? 'error' : 'warning'}" aria-hidden="true">
                                <i class="fa-solid fa-user"></i>
                            </span>
                            <div style="flex: 1; min-width: 0;">
                                <h2 class="sf-modal__title" id="sf-leaddetail-title">${this._escapeHtml(name)}</h2>
                                ${statusRaw ? `<p class="sf-modal__subtitle">${this._escapeHtml(statusRaw)}${item.LastExportMessage ? ' · ' + this._escapeHtml(item.LastExportMessage) : ''}</p>` : ''}
                            </div>
                            <button id="sf-leaddetail-close-x" class="sf-icon-btn" aria-label="${_t('crmExport.btnClose', 'Close')}">
                                <i class="fa-solid fa-xmark"></i>
                            </button>
                        </div>
                        <div style="flex: 1; overflow-y: auto; max-height: 450px; padding: 8px 20px;">
                            ${rows || `<p style="text-align:center;color:var(--sf-text-2);padding:24px;">${_t('crmExport.noFieldData', 'No field data to show.')}</p>`}
                        </div>
                        <div class="sf-modal__footer">
                            <button id="sf-leaddetail-close" class="sf-btn sf-btn--primary">${_t('crmExport.btnClose', 'Close')}</button>
                        </div>
                    </div>
                `;
                ...
                modal.querySelector('#sf-leaddetail-close').addEventListener('click', cleanup);
                modal.querySelector('#sf-leaddetail-close-x').addEventListener('click', cleanup);
        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            // Provider-parameterized, same shape as openFieldMapping's
            // providerConfig — defaults preserve today's exact Salesforce
            // behavior for any caller that doesn't pass these. HubSpot's
            // adapter passes its own values (apiEndpointValue: 'HubSpot',
            // providerLabel: 'HubSpot', its own dataCache, its own crmAdapter).
            const providerConfig = {
                apiEndpointValue: options.apiEndpointValue || 'LeadSuccess_Event_API',
                providerLabel: options.providerLabel || 'Salesforce',
                defaultMappings: options.defaultMappings || STANDARD_SALESFORCE_LEAD_FIELDS,
                dataCache: options.dataCache || null,
                crmAdapter: options.crmAdapter || null,
                // Batch transfer (executeBatchTransfer -> buildContactData/
                // detectUpsertKey/transferOne) defaults to Salesforce's own
                // Lead-object/__c-suffix/ /api/salesforce/leads logic when no
                // transferAdapter is supplied. A provider without real
                // transfer support yet can still set supportsBatchTransfer:
                // false to disable just the button, without hiding the rest
                // of this rich UI (filters/search/sort/connect all still work).
                supportsBatchTransfer: options.supportsBatchTransfer !== false,
                transferAdapter: options.transferAdapter || null,
                // Optional pre-flight duplicate check ("Check Duplicates"
                // button) — only HubSpot has a real batch endpoint for this
                // today (POST /api/hubspot/contacts/batch-check). null means
                // no batch endpoint exists for this provider, so the button
                // simply isn't rendered (see the toolbar HTML below), same
                // pattern as supportsBatchTransfer hiding the transfer button.
                duplicateCheckAdapter: options.duplicateCheckAdapter || null
            };
            ...
            // Provider-scoped: HubSpot's own dataCache/apiEndpointValue keep its
            // entries structurally separate from Salesforce's for the same event.
            const dataCache = providerConfig.dataCache || SalesforceLeadLib._dataCache;
            const contactsCacheKey = `contacts:${eventId}:${providerConfig.apiEndpointValue}`;
            const forceRefresh = options.forceRefresh === true;
            if (forceRefresh) {
                dataCache.delete(contactsCacheKey);
                dataCache.delete(`fieldmap:${eventId}:${providerConfig.apiEndpointValue}`);
                dataCache.delete(`apifields:${eventId}:${providerConfig.apiEndpointValue}`);
            }
            const cachedContacts = dataCache.get(contactsCacheKey);
            ...
            try {
                // --- Load field mappings ---
                // Provider-parameterized: Salesforce's own singleton instance
                // (this._getInstance()) is Salesforce-backend-bound (OAuth
                // popup/session state, lsapisfbackend.convey.de) and not
                // reusable for another provider — but all this method needs
                // from it is fieldMappingService, so build one directly scoped
                // to whichever provider is active, same as openFieldMapping does.
                if (!this._portalConfig) throw new Error('SalesforceLeadLib not initialized. Call SalesforceLeadLib.init() first.');
                const fieldMappingService = new CrmProviders.FieldMappingService({
                    serverName: this._portalConfig.serverUrl.replace(/^https?:\/\//, ''),
                    apiName: this._portalConfig.apiName,
                    apiEndpointValue: providerConfig.apiEndpointValue,
                    defaultMappings: providerConfig.defaultMappings,
                    dataCache: dataCache,
                    portalConfig: this._portalConfig
                });
                ...
                // WinJS's navigator appends the new page fragment before removing
                // the old one, so rootElement can be detached by the time this
                // await resolves (page navigated away while this call was in
                // flight). Bail out rather than painting/caching on its behalf.
                if (!rootElement.isConnected) return;
                ...
                // DISABLED (urgent fix): this was showing 50 fake demo leads to
                // real clients whenever an event genuinely had 0 contacts,
                // with no way to tell demo rows from a real empty result.
                // Kept commented (not deleted) in case a future dev-only
                // gated version of this is wanted — do not silently re-enable
                // without an explicit flag guarding it.
                let isFakeData = false;
                /*
                const _fakeStatusKey = `sf_demo_export_status_${eventId}_${providerConfig.apiEndpointValue}`;
                ...
                */
                ...
                // Seed/update the session cache (never for demo data). Must use
                // the provider-scoped `dataCache` local (HubspotLeadLib._dataCache
                // for HubSpot, etc.) — not SalesforceLeadLib._dataCache directly,
                // or every non-Salesforce provider's writes land in the wrong Map
                // while reads (dataCache.get above) correctly check their own,
                // making this cache a permanent miss for those providers.
                if (!isFakeData) {
                    dataCache.set(contactsCacheKey, {
                ...
                // Filter inputs: exactly the active configured fields (same set as the
                // table's own columns) minus export-status bookkeeping fields — a filter
                // for a field the user hasn't activated in CRM Settings would filter on
                // data that isn't even shown, which is confusing. 'Id' is left in on
                // purpose (present in activeODataColumns only when the user explicitly
                // activated it, e.g. mapped to LS_LeadId__c for upsert).
                const activeFilterFields = activeODataColumns.filter(f => !exportStatusSet.has(f));

                // --- CRM connection check ---
                // `let` (not const): the connect/disconnect buttons refresh this in place
                // via refreshSfStatusBar() instead of re-rendering the whole list, so the
                // already-loaded contacts and the infinite-scroll state are preserved.
                // Provider-parameterized: uses providerConfig.crmAdapter's checkConnection()
                // when supplied (any registered provider), falling back to Salesforce's own
                // silent check for existing callers that don't pass a crmAdapter.
                let sfStatus = providerConfig.crmAdapter
                    ? await providerConfig.crmAdapter.checkConnection()
                    : await this._checkSfConnectionSilent();
                ...
                // CRM connection status card (same design as CRM Settings)
                const sfDisplayName = sfStatus.userInfo || `${providerConfig.providerLabel} User`;
                const providerIconClass = _providerIconClass(providerConfig.apiEndpointValue);
                const sfBtnHtml = `
                    <div class="sf-cl-status-wrap">
                        ...
                        <button id="sf-cl-connect-btn" class="sf-cl-btn-connect" style="display:${sfStatus.connected ? 'none' : 'flex'};"><i class="${providerIconClass}" aria-hidden="true"></i> Connect ${esc(providerConfig.providerLabel)}</button>
                    </div>`;

                if (!rootElement.isConnected) return;
                rootElement.innerHTML = `
                    <div class="sf-cl-wrap">
                        <div class="sf-cl-page-header" id="sf-cl-page-header">
                            <div class="sf-cl-page-header__main">
                                <div class="sf-cl-page-header__icon"><i class="fa-solid fa-cloud" aria-hidden="true"></i></div>
                                <div class="sf-cl-page-header__text">
                                    <h1 class="sf-cl-page-header__title">${_t('crmExport.pageTitle', 'CRM Batch Export')}</h1>
                                    <div class="sf-cl-page-header__sub">
                                        <i class="${providerIconClass}" aria-hidden="true"></i>
                                        <span>${esc(providerConfig.providerLabel)}</span>
                                        <span class="sf-cl-page-header__dot">&middot;</span>
                                        <span id="sf-cl-status-dot" class="sf-cl-status-dot${sfStatus.connected ? ' connected' : ''}"></span>
                                        ...
                                        <span id="sf-cl-user-section" class="sf-cl-user-section${sfStatus.connected ? ' visible' : ''}">
                                            <span class="sf-cl-page-header__dot">|</span>
                        ...
                        <div class="sf-cl-toolbar">
                            <button class="sf-cl-btn sf-cl-btn-batch" id="sf-cl-batch-btn" disabled><i class="fa-solid fa-play" aria-hidden="true"></i> ${_t('crmExport.startBatchTransfer', 'Start Batch Transfer')} <span class="sf-cl-btn-count" id="sf-cl-batch-count">0</span></button>
                            ${providerConfig.duplicateCheckAdapter ? `<button class="sf-cl-btn sf-cl-btn-dupcheck" id="sf-cl-dupcheck-btn" disabled><i class="fa-solid fa-layer-group" aria-hidden="true"></i> ${_t('crmExport.checkDuplicates', 'Check Duplicates')}</button>` : ''}
                            <button class="sf-cl-btn sf-cl-btn-refresh" id="sf-cl-refresh-btn"><i class="fa-solid fa-rotate" aria-hidden="true"></i> ${_t('crmExport.btnRefresh', 'Refresh')}</button>
                            ${providerConfig.duplicateCheckAdapter ? `
                            <label class="sf-cl-dupcheck-exclude" id="sf-cl-dupcheck-bar" style="display:none;">
                                <input type="checkbox" id="sf-cl-dupcheck-exclude-cb" />
                                <i class="fa-solid fa-circle-xmark sf-cl-dupcheck-exclude__icon" aria-hidden="true"></i>
                                <span id="sf-cl-dupcheck-exclude-label">${_t('crmExport.excludeAlreadySynced', 'Exclude already-in-{0}').replace('{0}', esc(providerConfig.providerLabel))}</span>
                            </label>` : ''}
                            ...
                            <div class="sf-cl-status-filter" id="sf-cl-status-filter">
                                <span class="sf-cl-filter-label">${_t('crmExport.filterCompleteness', 'Completeness:')}</span>
                        ...
                        <div class="sf-cl-stat-cards" id="sf-cl-stat-cards" role="group" aria-label="${_t('crmExport.filterStatus', 'Status filter')}">
                            <button type="button" class="sf-cl-stat-card active" data-status="">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterAll', 'All')}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardAllSub', 'Total leads')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-all-count">${allItems.length}</div>
                            </button>
                            <button type="button" class="sf-cl-stat-card card-empty" data-status="__empty">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterNotExported', 'Not exported')}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardNotExportedSub', 'Pending export')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-empty-count">0</div>
                            </button>
                            <button type="button" class="sf-cl-stat-card card-success" data-status="success">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterSuccess', 'Success')}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardSuccessSub', 'Exported successfully')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-success-count">0</div>
                            </button>
                            <button type="button" class="sf-cl-stat-card card-failed" data-status="failed">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterFailed', 'Failed')}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardFailedSub', 'Export errors')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-failed-count">0</div>
                            </button>
                            <button type="button" class="sf-cl-stat-card card-dup" data-status="duplicate">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterDuplicate', 'Duplicate')}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardDupSub', 'Rejected as duplicate')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-dup-count">0</div>
                            </button>
                            ${providerConfig.duplicateCheckAdapter ? `
                            <button type="button" class="sf-cl-stat-card card-alreadyin" id="sf-cl-pill-alreadyin" data-status="__alreadyin" style="display:none;">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterAlreadyIn', 'Already in {0}').replace('{0}', esc(providerConfig.providerLabel))}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardAlreadyInSub', 'Detected by Check Duplicates')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-alreadyin-count">0</div>
                            </button>` : ''}
                            ${providerConfig.duplicateCheckAdapter ? `
                            <button type="button" class="sf-cl-stat-card card-emailconflict" id="sf-cl-pill-emailconflict" data-status="__emailconflict" style="display:none;">
                                <div class="sf-cl-stat-card__text">
                                    <div class="sf-cl-stat-card__label">${_t('crmExport.filterEmailConflict', 'Email conflict')}</div>
                                    <div class="sf-cl-stat-card__sub">${_t('crmExport.cardEmailConflictSub', 'Owned by another contact')}</div>
                                </div>
                                <div class="sf-cl-stat-card__value" id="sf-cl-pill-emailconflict-count">0</div>
                            </button>` : ''}
                        </div>
                            ...
                            <table class="sf-cl-table" id="sf-cl-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sf-cl-th" style="width:96px;text-align:center;">${_t('crmExport.colAction', 'Action')}</th>
                ...
                const renderRows = (items) => {
                    ...
                    if (!tbody) return; // rootElement was torn down (e.g. navigated away) mid-render
                    ...
                    tbody.innerHTML = items.map(item => {
                        ...
                        // A "Check Duplicates" result (pre-flight) takes visual priority
                        // over a stale post-transfer export status — it's the more
                        // actionable/current information for a row not yet transferred.
                        const dupCheckResult = dupCheckStatusByLeadId.get(item.Id);
                        let dotClass, dotTitle;
                        if (dupCheckResult === 'synced') {
                            dotClass = 'dot-synced';
                            dotTitle = _t('crmExport.dupCheckSynced', 'Already in {0}').replace('{0}', providerConfig.providerLabel);
                        } else if (dupCheckResult === 'email_conflict') {
                            dotClass = 'dot-email-conflict';
                            dotTitle = _t('crmExport.dupCheckEmailConflict', 'Email already used by a different contact');
                        } else {
                            dotClass = getExportDotClass(exportStatus);
                            dotTitle = exportStatus;
                        }
                        // Initials avatar in front of the first visible column, same
                        // "FI".substring(0,2) pattern already used for user/lead
                        // avatars elsewhere in this file — a quick visual anchor per
                        // row, especially useful once many rows share a last name.
                        const rowDisplayName = this.getLeadDisplayName(item);
                        const rowInitials = rowDisplayName === 'Unknown Lead' ? '?' :
                            rowDisplayName.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
                        const cells = displayColumns.map((col, i) => {
                            const cellValue = item[col] !== undefined ? item[col] : (customFieldValues[col] || '');
                            if (i === 0) return `<td class="sf-cl-td sf-cl-td-first-data" style="position:sticky;left:${STICKY_COL_LEFT}px;z-index:1;background:inherit;"><span class="sf-avatar" style="margin-right:8px;" aria-hidden="true">${esc(rowInitials)}</span>${esc(formatCell(col, cellValue))}</td>`;
                        ...
                        const viewBtn = `<button class="sf-cl-edit-btn sf-cl-view-btn" data-lead-id="${esc(item.Id)}" title="${_t('crmExport.viewLead', 'View details')}" aria-label="${_t('crmExport.viewLead', 'View details')}"><i class="fa-regular fa-eye" aria-hidden="true"></i></button>`;
                        const editBtn = item.KontaktViewId
                            ? `<button class="sf-cl-edit-btn" data-kontakt-view-id="${esc(item.KontaktViewId)}" title="${_t('crmExport.editContact', 'Edit')}" aria-label="${_t('crmExport.editContact', 'Edit')}"><i class="fa-solid fa-pen" aria-hidden="true"></i></button>`
                            : '';
                        const retryBtn = exportStatus.toLowerCase() === 'failed'
                            ? `<button class="sf-cl-edit-btn sf-cl-retry-btn" data-lead-id="${esc(item.Id)}" title="${_t('crmExport.retryTransfer', 'Retry transfer')}" aria-label="${_t('crmExport.retryTransfer', 'Retry transfer')}"><i class="fa-solid fa-rotate-right" aria-hidden="true"></i></button>`
                            : '';
                        const editCell = `<td class="sf-cl-td" style="text-align:center;white-space:nowrap;">${viewBtn}${retryBtn}${editBtn}</td>`;
                        return `<tr class="sf-cl-row ${rowClass}" data-lead-id="${esc(item.Id)}">
                            ...
                            <td class="sf-cl-td sf-cl-td-first-data" style="text-align:center;position:sticky;left:32px;z-index:1;background:var(--Window);">
                                ${dotClass ? `<span class="sf-export-badge-dot ${dotClass}" title="${esc(dotTitle)}"></span>` : ''}
                            </td>
                            ${editCell}
                            ${attachCell}
                            ${cells}
                ...
                const appendRows = (items) => {
                    const tbody = rootElement.querySelector('#sf-cl-tbody');
                    if (!tbody) return; // rootElement was torn down (e.g. navigated away) mid-scroll
                    const countEl = rootElement.querySelector('#sf-cl-count');
                ...
                const updateBatchBtn = () => {
                    ...
                    // Disable unless connected AND at least one row is selected AND this
                    // provider actually supports batch transfer yet (see providerConfig.
                    // supportsBatchTransfer — Salesforce's Lead-object/__c-suffix transfer
                    // logic isn't generalized for other providers yet).
                    batchBtn.disabled = !providerConfig.supportsBatchTransfer || !sfStatus.connected || checked.length === 0;
                    batchBtn.title = !providerConfig.supportsBatchTransfer
                        ? `Batch transfer isn't available for ${providerConfig.providerLabel} yet`
                        : (sfStatus.connected ? '' : `Connect to ${providerConfig.providerLabel} first`);
                    const batchCount = rootElement.querySelector('#sf-cl-batch-count');
                    if (batchCount) batchCount.textContent = checked.length;
                    updateDupCheckBtn();
                };

                // Same gating as updateBatchBtn, but keyed on
                // duplicateCheckAdapter instead of supportsBatchTransfer — a
                // provider with no batch-check endpoint never rendered this
                // button in the first place (see the toolbar HTML above), so
                // this is a no-op for Salesforce/Dynamics today.
                const updateDupCheckBtn = () => {
                    const dupBtn = rootElement.querySelector('#sf-cl-dupcheck-btn');
                    if (!dupBtn) return;
                    const checked = rootElement.querySelectorAll('.sf-cl-row-cb:checked');
                    dupBtn.disabled = !sfStatus.connected || checked.length === 0;
                    dupBtn.title = sfStatus.connected ? '' : `Connect to ${providerConfig.providerLabel} first`;
                };

                // Re-check the CRM connection and update ONLY the status bar + batch
                // button. Used by the connect/disconnect buttons so the loaded contacts
                // and the infinite-scroll state (allItems / currentSkip / observer) stay
                // intact — the connection state never affects the contact list itself.
                const refreshSfStatusBar = async () => {
                    sfStatus = providerConfig.crmAdapter
                        ? await providerConfig.crmAdapter.checkConnection()
                        : await this._checkSfConnectionSilent();
                    ...
                    const name = sfStatus.userInfo || `${providerConfig.providerLabel} User`;
                ...
                let activeStatusFilter = ''; // '' = All, '__empty' = not exported, 'success', 'failed', 'duplicate', '__alreadyin'
                let activeCompleteFilter = ''; // '' = Alle, 'vollstaendig', 'unvollstaendig'
                // Results from "Check Duplicates" (pre-flight, HubSpot only) — kept
                // separate from LastExportStatus (a post-transfer, provider-agnostic
                // column) so the two never conflate. Keyed by lead Id, value is
                // 'synced' | 'email_conflict'. Persists across re-renders/filtering
                // (renderRows reads it) and drives the "Already in {CRM}" pill.
                const dupCheckStatusByLeadId = new Map();
                ...
                const updateStatusPillCounts = () => {
                    // '__alreadyin' = synced only (same ls_lead_id already in
                    // the CRM — a re-transfer just updates it, no real risk).
                    // '__emailconflict' is tracked separately since it means
                    // a DIFFERENT contact already owns that email — a real
                    // collision, not just "already sent", and worth
                    // surfacing on its own rather than folding it into the
                    // same count.
                    const counts = { '': allItems.length, '__empty': 0, 'success': 0, 'failed': 0, 'duplicate': 0, '__alreadyin': 0, '__emailconflict': 0 };
                    allItems.forEach(item => {
                        ...
                        const dupStatus = dupCheckStatusByLeadId.get(item.Id);
                        if (dupStatus === 'synced') counts['__alreadyin']++;
                        else if (dupStatus === 'email_conflict') counts['__emailconflict']++;
                    });
                    const map = { '': '#sf-cl-pill-all-count', '__empty': '#sf-cl-pill-empty-count', 'success': '#sf-cl-pill-success-count', 'failed': '#sf-cl-pill-failed-count', 'duplicate': '#sf-cl-pill-dup-count', '__alreadyin': '#sf-cl-pill-alreadyin-count', '__emailconflict': '#sf-cl-pill-emailconflict-count' };
                    Object.entries(map).forEach(([key, sel]) => {
                        const el = rootElement.querySelector(sel);
                        if (el) el.textContent = counts[key];
                    });
                    const alreadyInPill = rootElement.querySelector('#sf-cl-pill-alreadyin');
                    if (alreadyInPill) alreadyInPill.style.display = counts['__alreadyin'] > 0 ? '' : 'none';
                    const emailConflictPill = rootElement.querySelector('#sf-cl-pill-emailconflict');
                    if (emailConflictPill) emailConflictPill.style.display = counts['__emailconflict'] > 0 ? '' : 'none';
                };
                ...
                if (servedFromCache) {
                    (async () => {
                        try {
                            ...
                            // The page may have navigated away while this background
                            // refresh was in flight — rootElement would be detached.
                            // Bail out before touching the DOM or re-priming the
                            // shared cache on behalf of a render that's gone.
                            if (!rootElement.isConnected) return;
                ...
                if (hasMorePages) {
                    ...
                    const scrollObserver = new IntersectionObserver(async (entries) => {
                        ...
                        // rootElement is torn down by SalesforceLeadLib.clear()
                        // on page dispose, but the observer itself can still
                        // fire once more before disconnect() takes effect —
                        // bail out rather than write into a detached DOM.
                        if (!rootElement.isConnected) return;
                        await loadNextPage();
                    }, { root: tableWrap, threshold: 0.1 });

                    scrollObserver.observe(sentinel);
                    rootElement._sfScrollObserver = scrollObserver;
                }
                ...
                const applyFilters = () => {
                    ...
                    const filtered = allItems.filter(item => {
                        ...
                        // Status filter
                        if (activeStatusFilter === '__alreadyin') {
                            if (dupCheckStatusByLeadId.get(item.Id) !== 'synced') return false;
                        } else if (activeStatusFilter === '__emailconflict') {
                            if (dupCheckStatusByLeadId.get(item.Id) !== 'email_conflict') return false;
                        } else if (activeStatusFilter === '__notalreadyin') {
                            if (dupCheckStatusByLeadId.has(item.Id)) return false;
                ...
                // Status filter cards (data-status)
                rootElement.querySelector('#sf-cl-stat-cards')?.addEventListener('click', (e) => {
                ...
                // View / Retry / Edit buttons (delegated, shared .sf-cl-edit-btn
                // styling but distinct sub-classes) — check the more specific
                // classes first since View/Retry also carry .sf-cl-edit-btn for
                // their icon-button appearance.
                rootElement.querySelector('#sf-cl-tbody').addEventListener('click', (e) => {
                    const viewBtn = e.target.closest('.sf-cl-view-btn');
                    if (viewBtn) {
                        const item = allItems.find(i => i.Id === viewBtn.dataset.leadId);
                        if (item) SalesforceLeadLib.showLeadDetailModal(item, displayColumns, formatCell);
                        return;
                    }

                    const retryBtn = e.target.closest('.sf-cl-retry-btn');
                    if (retryBtn) {
                        const item = allItems.find(i => i.Id === retryBtn.dataset.leadId);
                        if (item) retrySingleLead(item, retryBtn);
                        return;
                    }

                    // Edit-contact button — jumps straight to the app's own
                    // Edit Contact page (pages/contact/) for this row's lead,
                    // same navigation the Preview & Validate modal's own
                    // "Edit" link uses.
                    const editBtn = e.target.closest('.sf-cl-edit-btn');
                    if (editBtn) {
                        const kontaktViewId = editBtn.dataset.kontaktViewId;
                        if (kontaktViewId) SalesforceLeadLib._navigateToEditContact(kontaktViewId);
                    }
                });
                ...
                // Connect / Disconnect buttons. Provider-parameterized: goes through
                // providerConfig.crmAdapter when supplied, falling back to Salesforce's
                // own popup/disconnect for existing callers that don't pass a crmAdapter.
                ...
                if (sfConnectBtn) {
                    sfConnectBtn.addEventListener('click', async () => {
                        if (providerConfig.crmAdapter) {
                            await providerConfig.crmAdapter.connect();
                        } else {
                ...
                if (sfDisconnectBtn) {
                    sfDisconnectBtn.addEventListener('click', async () => {
                        if (providerConfig.crmAdapter) {
                            await providerConfig.crmAdapter.disconnect();
                        } else {
                ...
                // Shared by the batch-transfer and Check Duplicates handlers
                // below — both need "which lead rows are currently checked",
                // resolved back to their full itemData objects.
                const getSelectedItems = () => {
                    const checkedRows = [...rootElement.querySelectorAll('.sf-cl-row-cb:checked')];
                    return checkedRows.map(cb => {
                        const row = cb.closest('tr');
                        return allItems.find(item => item.Id === row.dataset.leadId);
                    }).filter(Boolean);
                };

                // Batch Transfer — not generalized for other providers yet (see
                // providerConfig.supportsBatchTransfer at the top of this method).
                // Shared row-refresh logic for a single transfer result — used
                // by both the batch handler's onLeadComplete and retrySingleLead,
                // so a one-off retry updates its row exactly like a batch run does.
                const applyTransferResultToRow = (result) => {
                    ...
                    // Re-render the row's own retry button visibility (only
                    // shown while Failed) without a full table re-render.
                    const actionCell = row.querySelector('.sf-cl-retry-btn')?.closest('td') || row.querySelector('.sf-cl-view-btn')?.closest('td');
                    if (actionCell) {
                        const stillFailed = result.status === 'failed';
                        const existingRetry = actionCell.querySelector('.sf-cl-retry-btn');
                        if (stillFailed && !existingRetry) {
                            const retryHtml = `<button class="sf-cl-edit-btn sf-cl-retry-btn" data-lead-id="${esc(result.itemData.Id)}" title="${_t('crmExport.retryTransfer', 'Retry transfer')}" aria-label="${_t('crmExport.retryTransfer', 'Retry transfer')}"><i class="fa-solid fa-rotate-right" aria-hidden="true"></i></button>`;
                            actionCell.querySelector('.sf-cl-view-btn')?.insertAdjacentHTML('afterend', retryHtml);
                        } else if (!stillFailed && existingRetry) {
                            existingRetry.remove();
                        }
                    }

                    if (result.status !== 'skipped' && result.itemData?.Id) {
                ...
                // Retry a single Failed lead without going through the full
                // batch-selection flow — reuses executeBatchTransfer with a
                // one-item array so the same transferAdapter / status handling
                // as a real batch applies, just scoped to this one row.
                const retrySingleLead = async (item, btnEl) => {
                    if (!sfStatus.connected || !providerConfig.supportsBatchTransfer) return;
                    const originalHtml = btnEl.innerHTML;
                    btnEl.disabled = true;
                    btnEl.innerHTML = '<span class="sf-spinner" style="width:12px;height:12px;border-width:2px;"></span>';
                    try {
                        const summary = await this.executeBatchTransfer([item], {
                            fieldMappingService,
                            skipExportStatus: isFakeData,
                            transferAdapter: providerConfig.transferAdapter,
                            onLeadComplete: (result) => applyTransferResultToRow(result)
                        });
                        const result = summary.results[0];
                        if (result) {
                            this._showToast(
                                result.status === 'success'
                                    ? _t('crmExport.retrySucceeded', 'Lead transferred successfully.')
                                    : (result.message || _t('crmExport.retryFailed', 'Retry failed.')),
                                result.status === 'success' ? 'success' : 'error'
                            );
                        }
                    } catch (err) {
                        Log.print(Log.l.error, `${namespaceName}.SalesforceLeadLib. Retry failed: ${err && err.message}`);
                        this._showToast(_t('crmExport.retryFailed', 'Retry failed.'), 'error');
                    } finally {
                ...
                rootElement.querySelector('#sf-cl-batch-btn').addEventListener('click', async () => {
                    if (!providerConfig.supportsBatchTransfer) return;
                    if (!sfStatus.connected) return; // safety net — button is disabled when not connected
                    let selectedItems = getSelectedItems();
                    if (selectedItems.length === 0) return;

                    // Preview & Validate — purely client-side over already-loaded
                    // data (no network calls). Skipped entirely when everything
                    // is already valid, so the common case keeps today's exact
                    // one-confirm flow with no extra click.
                    const validationReport = this.validateLeadsForTransfer(selectedItems, {
                        fieldMappingService,
                        transferAdapter: providerConfig.transferAdapter
                    });
                    // "Export anyway" forces the full original selection through
                    // untouched, including the required-fields auto-skip inside
                    // executeBatchTransfer below — the user has just been shown
                    // exactly which leads will likely be rejected by the CRM and
                    // is choosing to send them anyway rather than have them
                    // silently skipped.
                    let bypassRequiredFieldsCheck = false;
                    if (validationReport.invalid.length > 0) {
                        const choice = await this.showBatchValidationModal(validationReport, providerConfig.providerLabel);
                        if (!choice) return; // user cancelled
                        if (choice === 'validOnly') {
                            selectedItems = validationReport.valid;
                            if (selectedItems.length === 0) return;
                        } else if (choice === 'all') {
                            bypassRequiredFieldsCheck = true;
                        }
                    }

                    const confirmed = await this.showBatchConfirmModal(
                        `${_t('crmExport.btnTransfer', 'Transfer')} ${selectedItems.length} contact(s) to ${providerConfig.providerLabel}?`,
                    ...
                    const summary = await this.executeBatchTransfer(selectedItems, {
                        ...
                        skipExportStatus: isFakeData,
                        transferAdapter: providerConfig.transferAdapter,
                        bypassRequiredFieldsCheck,
                        onProgress: (current, total, name) => updateProgress(current, total, name),
                        onLeadComplete: (result) => {
                            updateLeadStatus(result);
                            applyTransferResultToRow(result);
                        }
                    });

                    if (progressModal?.parentNode) progressModal.parentNode.removeChild(progressModal);

                    // For fake demo data only (isFakeData guard above): persist export
                    // statuses to localStorage so they survive refresh. Every value here
                    // (LastExportMessage included) originates from fakeDataGenerator.js —
                    // randomly generated phone numbers, employee counts, etc. for the
                    // no-real-contacts test/demo flow, never a real lead's data. Safe to
                    // store as plain text; nothing here is real customer PII.
                    if (isFakeData) {
                        const toSave = {};
                        allItems.forEach(item => {
                    ...
                    const crmIdLabel = providerConfig.apiEndpointValue === 'LeadSuccess_Event_API' ? 'SF' : providerConfig.providerLabel;
                    this.showBatchSummaryModal(summary, () => { /* cells already updated in real-time */ }, crmIdLabel, providerIconClass);
                ...
                // Check Duplicates (HubSpot only today — see providerConfig.
                // duplicateCheckAdapter). Runs a pre-flight batch lookup on
                // whatever's currently checked, records the result in
                // dupCheckStatusByLeadId (so renderRows/applyFilters/the new
                // "Already in {CRM}" status pill all pick it up), and reveals
                // the "Exclude already-in-{CRM}" checkbox. A toast + re-render
                // (rather than only mutating dots in place) makes the outcome
                // impossible to miss — a handful of small colored dots is easy
                // to overlook after a multi-second async call.
                const dupCheckBtnEl = rootElement.querySelector('#sf-cl-dupcheck-btn');
                if (dupCheckBtnEl && providerConfig.duplicateCheckAdapter) {
                    dupCheckBtnEl.addEventListener('click', async () => {
                        if (!sfStatus.connected) return;
                        const selectedItems = getSelectedItems();
                        if (selectedItems.length === 0) return;

                        const originalText = dupCheckBtnEl.innerHTML;
                        dupCheckBtnEl.disabled = true;
                        dupCheckBtnEl.innerHTML = `<span class="sf-spinner" style="width:14px;height:14px;border-width:2px;"></span> ${_t('crmExport.checking', 'Checking…')}`;

                        const progressModal = this.showDuplicateCheckProgressModal(selectedItems.length, providerConfig.providerLabel);

                        try {
                            const results = await providerConfig.duplicateCheckAdapter.checkDuplicates(selectedItems, fieldMappingService);
                            progressModal.close();
                            ...
                            selectedItems.forEach((item) => {
                                ...
                                if (result.status === 'synced') {
                                    dupCheckStatusByLeadId.set(item.Id, 'synced');
                                    syncedCount++;
                                } else if (result.status === 'email_conflict') {
                                    dupCheckStatusByLeadId.set(item.Id, 'email_conflict');
                                    emailConflictCount++;
                                } else {
                                    dupCheckStatusByLeadId.delete(item.Id);
                                }
                            });
                            const flaggedCount = syncedCount + emailConflictCount;

                            // Re-render so the dots (and row's dupCheckResult priority
                            // logic in renderRows) reflect the new Map immediately,
                            // preserving whatever filter is currently active. renderRows()
                            // rebuilds every row's HTML from scratch (no `checked` in the
                            // template), which would silently uncheck the very selection
                            // the user just ran Check Duplicates on — capture and restore
                            // it around the re-render, same pattern as the stale-while-
                            // revalidate refresh above.
                            const checkedIdsBeforeRerender = new Set([...rootElement.querySelectorAll('.sf-cl-row-cb:checked')]
                                .map(cb => cb.closest('tr')?.dataset.leadId).filter(Boolean));

                            if (isAnyFilterActive()) { applyFilters(); } else { renderRows(allItems); }
                            updateStatusPillCounts();

                            if (checkedIdsBeforeRerender.size > 0) {
                                rootElement.querySelectorAll('#sf-cl-tbody tr').forEach(tr => {
                                    if (checkedIdsBeforeRerender.has(tr.dataset.leadId)) {
                                        const cb = tr.querySelector('.sf-cl-row-cb');
                                        if (cb) cb.checked = true;
                                    }
                                });
                            ...
                            const dupCheckBar = rootElement.querySelector('#sf-cl-dupcheck-bar');
                            const excludeLabel = rootElement.querySelector('#sf-cl-dupcheck-exclude-label');
                            if (dupCheckBar) dupCheckBar.style.display = 'flex';
                            if (excludeLabel) {
                                excludeLabel.textContent = `${_t('crmExport.excludeAlreadySynced', 'Exclude already-in-{0}').replace('{0}', providerConfig.providerLabel)} (${flaggedCount})`;
                            }

                            // Explicit, hard-to-miss feedback on what the check found —
                            // a silent dot update left the user guessing whether the
                            // click did anything at all.
                            if (flaggedCount === 0) {
                                this._showToast(
                                    _t('crmExport.dupCheckNoneFound', 'No duplicates found among the {0} checked lead(s).').replace('{0}', selectedItems.length),
                                    'success');
                            } else {
                                // Recap modal instead of a toast — a handful of
                                // small colored dots (or a one-line toast) is
                                // easy to miss after a multi-second async call,
                                // and doesn't show WHICH leads were flagged or
                                // why without hunting through the table.
                                const clean = [];
                                const flagged = [];
                                selectedItems.forEach((item) => {
                                    const result = results.get(item.Id);
                                    if (result && (result.status === 'synced' || result.status === 'email_conflict')) {
                                        flagged.push({ item, status: result.status, hubspotId: result.hubspotId, url: result.url });
                                    } else {
                                        clean.push(item);
                                    }
                                });
                                const choice = await this.showDuplicateCheckModal({ clean, flagged }, providerConfig.providerLabel);
                                if (choice === 'exclude') {
                                    const excludeCb = rootElement.querySelector('#sf-cl-dupcheck-exclude-cb');
                                    if (excludeCb && !excludeCb.checked) {
                                        excludeCb.checked = true;
                                        excludeCb.dispatchEvent(new Event('change'));
                                    }
                                }
                            }
                        } catch (err) {
                            progressModal.close();
                            Log.print(Log.l.error, `${namespaceName}.SalesforceLeadLib. Check Duplicates failed: ${err && err.message}`);
                            this._showToast(_t('crmExport.dupCheckFailed', 'Check Duplicates failed. Please try again.'), 'error');
                        } finally {
                            dupCheckBtnEl.disabled = false;
                            dupCheckBtnEl.innerHTML = originalText;
                            updateDupCheckBtn();
                        }
                    });
                }

                const dupExcludeCb = rootElement.querySelector('#sf-cl-dupcheck-exclude-cb');
                if (dupExcludeCb) {
                    // One-way deselect: checking it deselects synced/
                    // email_conflict rows; unchecking it does NOT re-select
                    // anything, so a user who'd manually unchecked a row for
                    // an unrelated reason never has it surprise-reselected.
                    // Also filters the table to just what's left to send
                    // (__notalreadyin) so the cleaned-up selection is
                    // immediately visible instead of buried among rows that
                    // were just excluded; unchecking restores whatever
                    // status filter was active before.
                    let statusFilterBeforeExclude = null;
                    dupExcludeCb.addEventListener('change', () => {
                        if (dupExcludeCb.checked) {
                            rootElement.querySelectorAll('.sf-cl-row-cb:checked').forEach((cb) => {
                                const row = cb.closest('tr');
                                const leadId = row && row.dataset.leadId;
                                if (leadId && dupCheckStatusByLeadId.has(leadId)) {
                                    cb.checked = false;
                                }
                            });
                            updateBatchBtn();

                            statusFilterBeforeExclude = activeStatusFilter;
                            activeStatusFilter = '__notalreadyin';
                            rootElement.querySelectorAll('[data-status]').forEach(p => p.classList.remove('active'));
                            applyFilters();
                            updateResetBtn();
                        } else {
                            if (activeStatusFilter === '__notalreadyin') {
                                activeStatusFilter = statusFilterBeforeExclude || '';
                                const restoredPill = activeStatusFilter
                                    ? rootElement.querySelector(`[data-status="${activeStatusFilter}"]`)
                                    : rootElement.querySelector('[data-status=""]');
                                if (restoredPill) restoredPill.classList.add('active');
                                applyFilters();
                                updateResetBtn();
                            }
                            statusFilterBeforeExclude = null;
                        }
                    });
                }
    ...
    window.SalesforceLeadLib.FieldMappingService = CrmProviders.FieldMappingService;
    ...
    // Sync connection check — what CrmProviders.resolveActiveCrmProvider()'s
    // fallback tier calls. Mirrors the presence check already used inline
    // throughout this file (localStorage.getItem('sf_session_token')).
    window.SalesforceLeadLib.isConnected = function () {
        return !!localStorage.getItem('sf_session_token');
    };

    // Register with the CRM provider registry (lib/CrmProviders/scripts/
    // crmProviderRegistry.js), so crmExport/crmSettings/crmConnections can
    // call through CrmProviders.getAdapter(...) instead of hardcoding
    // SalesforceLeadLib. Guarded: the registry script must load first, but
    // don't hard-fail this file if load order is ever wrong.
    if (window.CrmProviders && typeof window.CrmProviders.registerAdapter === "function") {
        const salesforceAdapter = {
            id: "salesforce",
            label: "Salesforce",
            apiEndpointValue: "LeadSuccess_Event_API",
            status: "available",
            isConnected: window.SalesforceLeadLib.isConnected,
            checkConnection: function () { return SalesforceLeadLib._checkSfConnectionSilent(); },
            connect: function () { return SalesforceLeadLib._sfConnectPopup(); },
            disconnect: function () { return SalesforceLeadLib._sfDisconnect(); },
            renderContactList: function (rootElement, eventId, options) {
                return SalesforceLeadLib.renderContactList(rootElement, eventId, { ...options, crmAdapter: salesforceAdapter });
            },
            // Passes itself as options.crmAdapter so openFieldMapping's Connect/
            // Disconnect buttons go through this same adapter contract (see
            // _attachFieldConfiguratorListeners) instead of the old Salesforce-
            // only ...InSettings helpers.
            openFieldMapping: function (rootElement, eventId, options) {
                return SalesforceLeadLib.openFieldMapping(rootElement, eventId, { ...options, crmAdapter: salesforceAdapter });
            },
            saveFieldMapping: function (eventId) {
                return SalesforceLeadLib.saveFieldMapping(eventId);
            }
        };
        window.CrmProviders.registerAdapter(salesforceAdapter);
    }
