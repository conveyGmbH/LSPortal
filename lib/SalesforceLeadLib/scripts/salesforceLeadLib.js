    ...
    `;
        ...
        static async _loadApiFieldsInBackground(rootElement, fieldMappingService) {
            try {
                ...
                if (eventId) {
                    // Filtered by EventId - faster query
                    endpoint = `LS_LeadReport?$filter=EventId eq '${eventId}'&$top=1&$select=*&$format=json`;
                } else {
                    // Fallback without filter
                    endpoint = `LS_LeadReport?$top=1&$select=*&$format=json`;
                }
                ...
                if (leadDataResponse && leadDataResponse.d && leadDataResponse.d.results && leadDataResponse.d.results[0]) {
                    ...
                    // Extract ALL field names from the API response (including null values)
                    for (const key in sampleData) {
                        // Include all fields except __metadata
                        if (key !== '__metadata') {
                            apiFields[key] = key;
                        }
                    }

                    console.log(`Loaded ${Object.keys(apiFields).length} fields from LS_LeadReport API`);
                    ...
                    console.log('No data in LS_LeadReport - using standard fields only');
                }
            } catch (apiError) {
