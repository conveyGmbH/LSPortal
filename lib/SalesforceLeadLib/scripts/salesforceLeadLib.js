    ...
    class SalesforceLeadLib {
                ...
                btn.addEventListener('click', async (event) => {
                    ...
                    try {
                        ...
                        SalesforceLeadLib.callSetLeadExportStatus(
                            contactGuid,
                            'Success',
                            salesforceId !== 'Unknown' ? `SF ID: ${salesforceId}` : 'Transferred',
                            Date.now() - transferStartTime
                        );
        ...
        static async _showAttachmentListModal(attachmentIdList, leadName) {
            ...
            dialog.innerHTML = `
                ...
                <div id="sf-av-content" style="flex:1;overflow:auto;display:flex;align-items:center;justify-content:center;padding:16px;background:var(--Window);">
                    <div style="display:flex;align-items:center;gap:10px;color:#6b7280;font-size:14px;">
                        <div style="width:22px;height:22px;border:3px solid #e5e7eb;border-top-color:var(--ColorAccent,#2563eb);border-radius:50%;animation:sf-spin .8s linear infinite;"></div>
                        ${_t('sforce.loading','Loading...')}
                    </div>
                </div>`;
            document.body.appendChild(dialog);
            ...
            metas.forEach((meta, i) => {
                ...
                tab.title = meta.name;
                tab.dataset.idx = i;
                tab.style.cssText = 'padding:5px 12px;border-radius:6px;border:1px solid #d1d5db;background:#f3f4f6;color:#374151;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;transition:background .15s;';
            ...
            const renderFromMeta = (meta, tab) => {
                ...
                if (!meta.body) {
                    contentEl.innerHTML = `<div style="color:#6b7280;text-align:center;padding:32px;">${_t('sforce.noAttachmentContent','No file content available.')}</div>`;
                    return;
                }
        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                // Helper: extract the relative endpoint from an OData __next URL
                // __next is a full URL like http://server/odata_apisf/LS_LeadReport?...&$skiptoken=200
                // We extract the part after the last '/' in the path (e.g. "LS_LeadReport?...&$skiptoken=200")
                // and pass it to _callPortalODataAPI which prepends the correct baseUrl.
                const extractNextEndpoint = (nextUrl) => {
                    if (!nextUrl || typeof nextUrl !== 'string') return null;
                    const slashIdx = nextUrl.lastIndexOf('/');
                    return slashIdx >= 0 ? nextUrl.substring(slashIdx + 1) : null;
                };

                let allItems = [];
                let currentData = await this._callPortalODataAPI(
                    `LS_LeadReport?$filter=EventId eq '${eventId}'&$top=${pageSize}&$format=json`
                );
                while (currentData) {
                    const pageItems = currentData?.d?.results || [];
                    allItems = allItems.concat(pageItems);
                    const nextEndpoint = extractNextEndpoint(currentData?.d?.__next);
                    if (nextEndpoint) {
                        currentData = await this._callPortalODataAPI(nextEndpoint);
                    } else {
                        currentData = null;
                    }
                }
