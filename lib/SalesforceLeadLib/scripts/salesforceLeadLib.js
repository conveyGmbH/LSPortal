...
(function() {
    ...
    class SalesforceLeadLib {
        ...
        // Collect active fields only - sends ALL fields that are active in config
        // User decides which fields to send - if field exists in SF, it will work
        // Non-standard SF fields get __c suffix automatically
        _collectActiveFieldsOnly(leadData) {
            const salesforceData = {};

            // Only exclude __metadata (internal OData field)
            const excludedFields = new Set(['__metadata']);

            // Get the field config array from the service
            ...
            if (!hasFieldConfig) {
                // No config exists - use default behavior (standard SF fields only)
            ...
            // Build a map of active fields from config
            ...
            // Also add active custom fields (user-created)
            ...
            // Only include fields that are in the active fields map
            for (const [fieldName, configField] of activeFieldsMap) {
                if (excludedFields.has(fieldName)) continue;

                // Get the value from leadData
                let value = leadData[fieldName];

                // For custom fields, use the stored value if available
                ...
                // Determine SF field name - custom labels are display-only, use original name
        ...
        static async saveFieldMapping(eventId) {
            try {
                ...
                // IMPORTANT: Always use the service's currentEventId for saving
                // This ensures we save to the correct event even if the eventId param is wrong
                    ...
                    html += `
                        <label class="field-item ${fieldClass}${isRequired ? ' required' : ''}${isActive ? ' active' : ''}${isCustom ? ' user-custom-field' : ''}"
                            ...
                            <div class="field-info" style="flex: 1; min-width: 0;">
                                ${isCustom ? `
                                    <!-- Custom field layout - show both LS label and SF field name -->
            ...
            transferBtns.forEach(btn => {
                btn.addEventListener('click', async (event) => {
                    ...
                    try {
                        ...
                        allFields.forEach(field => {
                            if (field && field.name) {
                                // Determine SF field name - add __c suffix for non-standard fields
                                // Custom labels are display-only; always use original field name for SF API
