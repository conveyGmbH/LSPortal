        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                if (allItems.length === 0 && window.fakeDataGenerator) {
                    isFakeData = true;
                    const FAKE_FIELDS = ['LastName', 'FirstName', 'Company', 'Email', 'Phone', 'MobilePhone',
                        'Title', 'Street', 'City', 'PostalCode', 'State', 'Country', 'Description'];
                    for (let i = 0; i < 5; i++) {
                        const result = window.fakeDataGenerator.fillEmptyFields({}, FAKE_FIELDS);
                        const item = { ...result.data };
                        item.Id = `00000000-0000-0000-0000-${String(i + 1).padStart(12, '0')}`;
                        // Leave export status fields absent — they'll render as empty cells
                        allItems.push(item);
                    }
                ...
                const hasFieldConfig = !isFakeData && fieldMappingService?.fieldConfig?.config?.fields?.length > 0;
                ...
                // For fake data: don't prepend export status columns (no real export history to show)
                const displayColumns = isFakeData
                    ? [...new Set([...activeODataColumns, ...customFieldColumns])]
                    : [...new Set([...EXPORT_STATUS_FIELDS, ...activeODataColumns, ...customFieldColumns])];
