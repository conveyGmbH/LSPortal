...
(function() {
    ...
    // Standard Salesforce Lead fields definition
    ...
    class FieldMappingService {
        ...
        shouldFieldBeActiveByDefault(fieldName) {
            ...
            // All fields inactive by default - must be explicitly activated in CRM Settings
            return false;
        }
        ...
        async initializeFields(leadData, eventId) {
            try {
                ...
                // Check if DB has ANY configured fields
                // If yes: new/unknown fields default to inactive (user already configured their selection)
                // If no: first render, apply standard SF defaults (Salutation, FirstName, etc.)
                ...
                if (leadData) {
                    Object.keys(leadData).forEach(fieldName => {
                        ...
                        if (existingConfig && existingConfig.active !== undefined) {
                            ...
                            // No config for this field
                            // Only apply defaults on first render (no DB config at all)
    ...
    class SalesforceLeadLib {
        ...
        /**
         * Show attachment preview modal - fetches attachment from OData and displays it inline
         * Supports images, PDFs, SVG, audio, video; falls back to download prompt for other types
         * @param {HTMLElement} rootElement - CRM Export root element
         * @param {string} attachmentId - Attachment GUID
         * @private
         */
        static async _showAttachmentModal(attachmentId) {
            // Use native <dialog> to escape WinJS stacking context (same fix as edit label dialogs)
            const dialog = document.createElement('dialog');
            dialog.className = 'sf-lib-dialog';
            dialog.style.cssText = `
                border: none;
            ...
            dialog.innerHTML = `
                <div style="display:flex; align-items:center; justify-content:space-between; padding:16px 20px; background:var(--accent-color,#2563eb); color:white; flex-shrink:0;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <i class="fa-solid fa-paperclip"></i>
                        <span id="sf-attachment-modal-title" style="font-weight:700; font-size:15px;">Loading...</span>
                    </div>
            ...
            try {
                ...
                if (!attachment?.Body) {
                    modalBody.innerHTML = `<div style="color:#6b7280; text-align:center; padding:40px;">No content available for this attachment.</div>`;
                    return;
                }
                ...
                if (isSVG) {
                    try {
                        ...
                        else modalBody.innerHTML = `<object data="${dataUrl}" type="image/svg+xml" style="width:100%;height:60vh;">SVG not supported</object>`;
                    } catch {
                        modalBody.innerHTML = `<object data="${dataUrl}" type="image/svg+xml" style="width:100%;height:60vh;">SVG not supported</object>`;
                    }
                    ...
                    modalBody.innerHTML = `<iframe src="${dataUrl}#view=Fit" style="width:100%; height:70vh; border:none;"><p>PDF not supported. <a href="${dataUrl}" download="${this._escapeHtml(fileName)}">Download</a></p></iframe>`;
                } else if (finalType.startsWith('audio/')) {
                    modalBody.innerHTML = `<audio controls style="width:100%;"><source src="${dataUrl}" type="${finalType}">Your browser does not support audio.</audio>`;
                } else if (finalType.startsWith('video/')) {
                    modalBody.innerHTML = `<video controls style="max-width:100%; max-height:70vh;"><source src="${dataUrl}" type="${finalType}">Your browser does not support video.</video>`;
                } else {
                    modalBody.innerHTML = `<div style="text-align:center; padding:40px; color:#6b7280;"><i class="fa-solid fa-file" style="font-size:48px; margin-bottom:16px; display:block;"></i><div style="font-size:14px; margin-bottom:8px;">${this._escapeHtml(fileName)}</div><div style="font-size:12px;">Preview not available for this file type (${this._escapeHtml(finalType)})</div><div style="margin-top:8px; font-size:12px;">Use the Download button to open the file.</div></div>`;
                }

            } catch (error) {
                modalBody.innerHTML = `<div style="color:#ef4444; text-align:center; padding:40px;">Error loading: ${this._escapeHtml(error.message)}</div>`;
            }
        }
         ...
         * Replicates collectActiveFieldsOnly() logic but takes itemData as parameter
         * @param {Object} itemData - Raw OData lead item
         * @param {Object} fieldMappingService - FieldMappingService instance
         * @returns {Object} Salesforce-ready lead data
         */
        ...
        /**
         * Detect the external ID field configured in FieldMappingService.
         * If customLabels['Id'] points to a __c field, returns that field name, otherwise null.
         * @param {Object} fieldMappingService
         * @returns {string|null}
         */
         ...
         * Uses the instance's backendUrl from _getInstance()
         * @param {Object} leadData - Salesforce-ready lead data
         * @param {Array} attachments - Array of attachment objects
         * @returns {Object} { success, status, message, salesforceId, duplicateWarning, attachmentsTransferred }
         */
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    return {
                        success: false,
                        status: 'failed',
                        message: sfMessage || errorData.error || errorData.message || `HTTP ${response.status}`,
                        salesforceId: null,
        ...
        static async executeBatchTransfer(items, options = {}) {
            const { fieldMappingService, onProgress, onLeadComplete } = options;
        ...
        static showBatchProgressModal(totalCount) {
            ...
            modal.innerHTML = `
                <div style="background: var(--Window, white); border-radius: 12px; padding: 32px; width: 550px; max-width: 95vw; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 20px 40px rgba(0,0,0,0.3);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                        <h2 style="margin: 0; font-size: 20px; color: var(--WindowText, #111827);">Batch Transfer</h2>
                        <span id="batch-progress-counter" style="font-size: 14px; color: #6b7280;">0 / ${totalCount}</span>
                    </div>
                    <div id="batch-current-lead" style="background: var(--sf-bg-secondary, #f3f4f6); border-radius: 8px; padding: 12px; margin-bottom: 16px; font-size: 14px; color: var(--WindowText, #374151);">
                        Preparing...
                    </div>
                    ...
                    <div id="batch-results-list" style="flex: 1; overflow-y: auto; max-height: 300px; border: 1px solid #e5e7eb; border-radius: 8px; margin-bottom: 16px;"></div>
            ...
            return {
                ...
                updateLeadStatus(result) {
                    ...
                    entry.style.cssText = `display: flex; align-items: center; gap: 10px; padding: 8px 12px; border-bottom: 1px solid #f3f4f6; font-size: 13px;`;
                    entry.innerHTML = `
         ...
         */
        static showBatchSummaryModal(summary) {
            const modal = document.createElement('div');
            ...
            const buildResultRow = (r) => {
                ...
                const sfId = r.salesforceId ? `<span style="color: #9ca3af; font-size: 11px;">SF: ${r.salesforceId}</span>` : '';
                const msg = r.message ? `<span style="color: #9ca3af; font-size: 11px;">${SalesforceLeadLib._escapeHtml(r.message)}</span>` : '';
                return `
                    <div style="display: flex; align-items: center; gap: 10px; padding: 8px 20px; border-bottom: 1px solid #f9fafb;">
                        <span style="width: 22px; height: 22px; border-radius: 50%; background: ${color}; color: white; display: flex; align-items: center; justify-content: center; font-size: 12px; flex-shrink: 0;">${icon}</span>
                        <div style="flex: 1; min-width: 0;">
                            <div style="font-size: 13px; color: var(--WindowText, #111827); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${SalesforceLeadLib._escapeHtml(r.displayName)}</div>
            ...
            closeBtn.addEventListener('click', () => { if (modal.parentNode) modal.parentNode.removeChild(modal); });
            ...
            modal.addEventListener('click', (e) => { if (e.target === modal && modal.parentNode) modal.parentNode.removeChild(modal); });
         ...
         */
        static showBatchConfirmModal(message, { title = 'Confirm', okText = 'OK', cancelText = 'Cancel', okColor = '#2563eb' } = {}) {
            return new Promise((resolve) => {
                const modal = document.createElement('div');
                modal.style.cssText = `
                    position: fixed; top: 0; left: 0; right: 0; bottom: 0;
                ...
                modal.innerHTML = `
                    <div style="background: var(--Window, white); border-radius: 12px; padding: 28px; width: 420px; max-width: 90vw; box-shadow: 0 20px 40px rgba(0,0,0,0.25);">
                        ...
                        <p style="margin: 0 0 24px 0; font-size: 14px; color: #4b5563; line-height: 1.5; white-space: pre-line;">${this._escapeHtml(message)}</p>
                        <div style="display: flex; justify-content: flex-end; gap: 10px;">
                            <button id="confirm-cancel-btn" style="
                                padding: 10px 24px; border: 1px solid #d1d5db; background: var(--Window, white); color: var(--WindowText, #374151);
        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            rootElement.innerHTML = `<div class="sf-cl-loading" style="padding:32px;text-align:center;color:#6b7280;">Loading contacts…</div>`;
            ...
            try {
                ...
                // Ensure field config is loaded
                if (!fieldMappingService.fieldConfig?.config?.fields?.length) {
                    await fieldMappingService.loadFieldMappingsFromAPI(eventId);
                }

                // Determine display columns: active fields + export status fields always shown
                const EXPORT_STATUS_FIELDS = ['LastExportStatus', 'LastExportTimestamp', 'LastExportMessage', 'ExportAttempts'];
                const fieldConfigArray = fieldMappingService.fieldConfig?.config?.fields || [];
                const activeColumns = fieldConfigArray
                    .filter(f => f.active === true)
                    .map(f => f.fieldName);

                // All columns to display (active + export status, deduplicated)
                const displayColumns = [...new Set([...activeColumns, ...EXPORT_STATUS_FIELDS])];

                // --- Load LS_LeadReport data ---
                const endpoint = `LS_LeadReport?$filter=EventId eq '${eventId}'&$top=${pageSize}&$format=json`;
                const data = await this._callPortalODataAPI(endpoint);
                const allItems = data?.d?.results || [];

                // --- Build HTML ---
                const colHeaders = displayColumns.map(col => `<th class="sf-cl-th">${esc(col)}</th>`).join('');

                const filterInputs = displayColumns.map(col =>
                    `<div style="display:flex;flex-direction:column;gap:2px;">
                        <label style="font-size:11px;color:#6b7280;">${esc(col)}</label>
                        <input class="sf-cl-filter-input" data-col="${esc(col)}" type="text" placeholder="Filter…"
                            style="padding:4px 8px;border:1px solid #d1d5db;border-radius:4px;font-size:12px;width:130px;" />
                    </div>`
                ).join('');

                rootElement.innerHTML = `
                    <style>
                        .sf-cl-toolbar { display:flex; align-items:center; gap:8px; padding:10px 0 10px 0; flex-wrap:wrap; }
                        .sf-cl-btn { padding:8px 16px; border-radius:6px; font-size:13px; font-weight:600; cursor:pointer; border:none; }
                        .sf-cl-btn-batch { background:#2563eb; color:#fff; }
                        .sf-cl-btn-batch:disabled { background:#93c5fd; cursor:not-allowed; }
                        .sf-cl-btn-refresh { background:#f3f4f6; color:#374151; border:1px solid #d1d5db; }
                        .sf-cl-btn-filters { background:#f3f4f6; color:#374151; border:1px solid #d1d5db; }
                        .sf-cl-filter-bar { display:none; flex-wrap:wrap; gap:8px; padding:8px 0; }
                        .sf-cl-table-wrap { overflow-x:auto; margin-top:4px; }
                        .sf-cl-table { border-collapse:collapse; width:100%; font-size:13px; }
                        .sf-cl-th { background:#1f2937; color:#f9fafb; padding:8px 10px; text-align:left; white-space:nowrap; font-weight:600; position:sticky;top:0;z-index:1; }
                        .sf-cl-td { padding:7px 10px; border-bottom:1px solid #e5e7eb; white-space:nowrap; vertical-align:middle; }
                        .sf-cl-row:hover td { background:rgba(37,99,235,0.04); }
                        .sf-cl-row-success td { background:rgba(220,252,231,0.55) !important; }
                        .sf-cl-row-failed td { background:rgba(254,226,226,0.55) !important; }
                        .sf-cl-row-duplicate td { background:rgba(254,243,199,0.55) !important; }
                        ...
                        .sf-cl-empty { padding:32px; text-align:center; color:#6b7280; }
                        .sf-cl-count { font-size:12px; color:#6b7280; margin-left:auto; }
                    </style>
                    <div class="sf-cl-toolbar">
                        <button class="sf-cl-btn sf-cl-btn-batch" id="sf-cl-batch-btn" disabled>Start Batch Transfer (0)</button>
                        <button class="sf-cl-btn sf-cl-btn-refresh" id="sf-cl-refresh-btn">Refresh List</button>
                        <button class="sf-cl-btn sf-cl-btn-filters" id="sf-cl-filter-btn">Show Filters</button>
                        <span class="sf-cl-count" id="sf-cl-count">${allItems.length} contacts</span>
                    </div>
                    ...
                    <div class="sf-cl-table-wrap">
                        <table class="sf-cl-table" id="sf-cl-table">
                            <thead>
                                <tr>
                                    <th class="sf-cl-th" style="width:32px;"><input type="checkbox" id="sf-cl-select-all" title="Select all" /></th>
                                    <th class="sf-cl-th" style="width:20px;"></th>
                                    ${colHeaders}
                                </tr>
                            </thead>
                            <tbody id="sf-cl-tbody"></tbody>
                        </table>
                        <div id="sf-cl-empty" class="sf-cl-empty" style="display:none;">No contacts found.</div>
                    </div>
                `;
                ...
                const renderRows = () => {
                    ...
                    const filtered = filteredItems.filter(item => {
                        return Object.entries(filterValues).every(([col, val]) => {
                            if (!val) return true;
                            const cellVal = String(item[col] ?? '').toLowerCase();
                    ...
                    tbody.innerHTML = filtered.map(item => {
                        ...
                        const cells = displayColumns.map(col => `<td class="sf-cl-td">${esc(item[col])}</td>`).join('');
                        return `
                            <tr class="sf-cl-row ${rowClass}" data-lead-id="${esc(item.Id)}">
                                <td class="sf-cl-td"><input type="checkbox" class="sf-cl-row-cb" /></td>
                                <td class="sf-cl-td" style="text-align:center;">
                    ...
                    if (countEl) countEl.textContent = `${filtered.length} / ${allItems.length} contacts`;
                ...
                const updateBatchBtn = () => {
                    ...
                    batchBtn.textContent = `Start Batch Transfer (${checked.length})`;
                };
                ...
                // --- Event: Row checkboxes (delegated) ---
                ...
                // --- Event: Show/Hide Filters ---
                rootElement.querySelector('#sf-cl-filter-btn').addEventListener('click', () => {
                    const bar = rootElement.querySelector('#sf-cl-filter-bar');
                    const btn = rootElement.querySelector('#sf-cl-filter-btn');
                    const visible = bar.style.display !== 'none';
                    bar.style.display = visible ? 'none' : 'flex';
                    btn.textContent = visible ? 'Show Filters' : 'Hide Filters';
                });

                // --- Event: Filter inputs (delegated) ---
                rootElement.querySelector('#sf-cl-filter-bar').addEventListener('input', (e) => {
                    if (e.target.classList.contains('sf-cl-filter-input')) {
                        const col = e.target.dataset.col;
                        filterValues[col] = e.target.value;
                        renderRows();
                    }
                });
                ...
                rootElement.querySelector('#sf-cl-batch-btn').addEventListener('click', async () => {
                    ...
                    const selectedItems = checkedRows.map(cb => {
                        ...
                        return allItems.find(item => item.Id === leadId);
                    }).filter(Boolean);

                    const confirmed = await this.showBatchConfirmModal(
                        `Transfer ${selectedItems.length} contact(s) to Salesforce?`,
                        { title: 'Start Batch Transfer', okText: 'Transfer', okColor: '#2563eb' }
                    );
                    if (!confirmed) return;

                    const { modal: progressModal, updateProgress, addResult } = this.showBatchProgressModal(selectedItems.length);
                    ...
                    const summary = await this.executeBatchTransfer(selectedItems, {
                        ...
                        onLeadComplete: (result) => {
                            addResult(result);
                            // Update row tinting in real-time
                            ...
                            if (row) {
                                ...
                                if (dot) {
                                    dot.className = `sf-export-badge-dot ${getExportDotClass(result.status)}`;
                                    dot.title = result.status;
                                } else if (result.status) {
                                    const dotCell = row.cells[1];
                                    if (dotCell) dotCell.innerHTML = `<span class="sf-export-badge-dot ${getExportDotClass(result.status)}" title="${esc(result.status)}"></span>`;
                                }
                            }
                        }
                    });
                    ...
                    this.showBatchSummaryModal(summary);

                    // Uncheck all after batch
                ...
                console.error('renderContactList error:', err);
                rootElement.innerHTML = `<div class="sf-cl-empty" style="color:#dc2626;">Error loading contacts: ${this._escapeHtml(err.message)}</div>`;
            }
        }
