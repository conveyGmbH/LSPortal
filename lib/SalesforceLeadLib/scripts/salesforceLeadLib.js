...
(function() {
    ...
    // Font Awesome brand icon class for a provider's apiEndpointValue —
    // used everywhere the UI needs to show "which CRM are we in" (toolbar
    // badge, Field Configurator header, Connect button, batch summary modal).
    // Falls back to a generic plug icon for a provider with no matching
    // FA brand glyph (e.g. Dynamics 365).
    ...
    class SalesforceLeadLib {
        constructor(config = {}) {
            ...
            this.fieldMappingService = new CrmProviders.FieldMappingService({
                ...
                // Defaults preserve today's exact Salesforce behavior for any
                // caller that doesn't pass these (e.g. openCrmTestExport's own
                // `new SalesforceLeadLib(...)` call, which predates provider
                // parameterization) — openFieldMapping passes its own per
                // providerConfig so HubSpot's instance gets 'HubSpot'/
                // HS_DEFAULT_MAPPINGS instead.
                ...
                // Provider-scoped cache: HubSpot's instance passes its own
                // HubspotLeadLib._dataCache so its fieldmap/apifields entries
                // can never collide with Salesforce's under the same eventId.
         ...
         *   'fieldmap:{eventId}:{apiEndpointValue}' -> { configDataStr } (raw JSON string —
        ...
        static _detectEnvironment() {
            ...
            // Anchored suffix match (exact host or "*.domain"), not a bare
            // substring check — `includes('convey.de')` would also match an
            // attacker-controlled host like "convey.de.evil.com" or
            // "evilconvey.de.com".
        ...
        static clear(rootElement) {
            ...
            // renderContactList's infinite-scroll IntersectionObserver keeps
            // firing on its (now-detached) sentinel until explicitly
            // disconnected — without this, navigating away mid-scroll (e.g.
            // the CRM Export row's "Edit" button jumping to pages/contact/)
            // can trigger a stale loadNextPage() that crashes trying to
            // write into a tbody that no longer exists in this rootElement.
        ...
        static async openFieldMapping(rootElement, eventId, options = {}) {
            ...
            // Provider-parameterized: defaults to Salesforce so existing callers
            // that pass no options keep today's exact behavior. HubSpot's adapter
            // passes its own values (see registerAdapter at the bottom of this
            // file, or HubspotLeadLib once it delegates here).
            ...
            try {
                ...
                // with a shimmer sweep while config + API fields load. Skip it on a
                // cache hit: tab switches recreate the page, and re-playing the
                // shimmer over already-cached config reads as a needless reload.
                // Skip the skeleton only when BOTH the config AND the API field
                // list are cached — the grid needs apiFields to render, so a
                // config-only hit would still flash an empty grid.
                ...
                // the old one, so rootElement can be detached by the time we get
                // here (page navigated away while init/credentials setup ran).
                ...
                try {
                    ...
                    if (configCached) {
                        // Both config and API fields are cached: apply them
                        // synchronously and render immediately (no spinner, no
                        // empty-grid flash), then revalidate in the background.
                        ...
                        // Silent refresh — re-render only if the element is still
                        // attached AND still holds this exact render's markup
                        // (#fieldsContainer), i.e. the page hasn't navigated away.
                        Promise.all([
                        ...
                        // Cold load: fetch config and API fields in parallel.
                        await Promise.all([
                            svc.initializeFields({}, effectiveEventId),
                            this._loadApiFieldsInBackground(rootElement, svc)
                        ]);

                        // Page may have navigated away while the cold load was in
                        // flight — bail out before rendering into a detached element.
                        if (!rootElement.isConnected) return;
        ...
        static async _loadApiFieldsInBackground(rootElement, fieldMappingService) {
            try {
                ...
                // Session cache: the API field list is re-fetched on every tab
                // switch (fresh service each navigation), leaving the grid empty
                // until it returns — the "reload" flash the user sees. Serve the
                // cached list instantly; the fetch below still runs to refresh it.
        ...
        static _renderFieldsGrid(rootElement, fieldMappingService, currentFilter) {
            ...
            // Provider-parameterized: each FieldMappingService instance carries
            // its own default field list (SF's STANDARD_SALESFORCE_LEAD_FIELDS,
            // HubSpot's HS_DEFAULT_MAPPINGS, ...) — read it off the instance
            // instead of closing over the Salesforce-only module constant, so
            // this grid renders correctly for whichever provider is active.
            const standardSalesforceFields = fieldMappingService.defaultMappings || STANDARD_SALESFORCE_LEAD_FIELDS;
            // Short target-provider prefix shown as "<prefix>: fieldName" — 'SF'
            // for Salesforce (fieldMappingService.apiEndpointValue ===
            // 'LeadSuccess_Event_API'), or the provider's own short label
            // otherwise (e.g. 'HS' for HubSpot). Defaults to 'SF' so existing
            // Salesforce callers that construct a plain FieldMappingService
        ...
        static _attachFieldConfiguratorListeners(rootElement, fieldMappingService, providerConfig) {
            ...
            // Connect/Disconnect buttons in CRM Settings header. Provider-
            // parameterized: providerConfig.crmAdapter is the CrmProviders
            // registry adapter for whichever CRM is active (Salesforce,
            // HubSpot, ...) — same connect()/disconnect()/checkConnection()
            // contract crmExportController.js/crmSettingsController.js already
            // use. Falls back to Salesforce's own (pre-multi-CRM) ...InSettings
            // helpers when no adapter is supplied, so existing callers that
            // don't pass providerConfig keep today's exact behavior.
        ...
        // Generic connection-status refresh for the Field Configurator header
        // chip (#sf-status-*, #sf-user-*, #sf-connect-btn/#sf-disconnect-btn),
        // driven by any CrmProviders adapter's checkConnection() rather than
        // Salesforce's own _checkSalesforceConnectionInSettings.
        ...
        static async _openEditFieldLabelModal(rootElement, fieldMappingService, fieldName) {
            ...
            // Blocks a concurrent background revalidation (see openFieldMapping's
            // stale-while-revalidate re-render, and _applyConfigData's own guard)
            // from reassigning fieldMappingService.fieldConfig to a fresh object
            // while this dialog is open — that reassignment would silently orphan
            // the in-place edit made below via getFieldConfig(fieldName), so the
            // save that follows would persist the pre-edit config. Cleared in
            // `finally` so it can never get stuck on if this method throws or
            // returns early (cancel/reset paths included).
            ...
            // Guard: the CRM TARGET must be writable/valid for whatever provider this
            // FieldMappingService instance is scoped to (apiEndpointValue) — the two
            // providers reject different things, so each gets its own check rather
            // than always running Salesforce's rule.
