...
(function() {
    ...
    class SalesforceLeadLib {
         ...
         * specific lead. Coming from CRM Export, the record was never part
         * of contactList's own (slow, full, unrestricted) reload — so once
         * that reload finishes, its selectRecordId() won't find the record
         * on the freshly-fetched first page, resets AppData's Kontakt id to
         * 0, and force-reloads the detail page, wiping the right panel that
         * was showing our contact just fine a moment earlier.
         *
         * The full master reload itself is an accepted, pre-existing cost of
         * crossing master groups (see the fix/2fa-event-switch-reprompt-era
         * discussion) — what must NOT happen is losing the selected contact
         * once that reload settles. So: let the reload run as it always did,
         * but immediately after it settles, restore the id if it got zeroed
         * and re-render the contact page from it.
         */
        static _navigateToEditContact(kontaktViewId) {
            ...
            WinJS.Promise.as(Application.navigateById('contact')).then(function () {
                var master = Application.navigator && Application.navigator.masterControl;
                if (!master || !master.controller) return;
                // masterReadyPromise resolves once the master's own initial
                // load (kicked off by its constructor on navigation) settles
                // — contactListController exposes no public "loaded" event,
                // so poll its busy flag briefly rather than guessing a fixed
                // delay.
                var attemptsLeft = 40; // ~10s at 250ms, generous for a 2000+ row reload
                var waitForMasterIdle = function () {
                    if (!master.controller.busy || attemptsLeft-- <= 0) {
                        if (AppData.getRecordId('Kontakt') !== kontaktViewId) {
                            // selectRecordId() couldn't find this record on the
                            // freshly-loaded page and zeroed the id — restore it
                            // and re-render the still-active contact page.
                            AppData.setRecordId('Kontakt', kontaktViewId);
                            if (typeof AppBar !== 'undefined' && AppBar.scope &&
                    ...
                    WinJS.Promise.timeout(250).then(waitForMasterIdle);
                };
                WinJS.Promise.timeout(250).then(waitForMasterIdle);
            });
        }
        ...
        static async renderContactList(rootElement, eventId, options = {}) {
            ...
            try {
                ...
                const updateStatusPillCounts = () => {
                    const counts = { '': allItems.length, '__empty': 0, 'success': 0, 'failed': 0, 'duplicate': 0, '__alreadyin': 0 };
                    allItems.forEach(item => {
                        ...
                        if (dupCheckStatusByLeadId.has(item.Id)) counts['__alreadyin']++;
                    });
                    const map = { '': '#sf-cl-pill-all-count', '__empty': '#sf-cl-pill-empty-count', 'success': '#sf-cl-pill-success-count', 'failed': '#sf-cl-pill-failed-count', 'duplicate': '#sf-cl-pill-dup-count', '__alreadyin': '#sf-cl-pill-alreadyin-count' };
                ...
                if (dupExcludeCb) {
                    // One-way: checking it deselects synced/email_conflict
                    // rows; unchecking it does NOT re-select anything, so a
                    // user who'd manually unchecked a row for an unrelated
                    // reason never has it surprise-reselected. It also jumps
                    // the "Already in {CRM}" status pill to filter the table
                    // to those rows, so the user can actually see the full
                    // list of what's being excluded rather than only losing
                    // their checkmarks with no visibility into which rows.
                    dupExcludeCb.addEventListener('change', () => {
                        if (!dupExcludeCb.checked) return;
                        ...
                        const alreadyInPill = rootElement.querySelector('#sf-cl-pill-alreadyin');
                        if (alreadyInPill && alreadyInPill.style.display !== 'none') {
                            activeStatusFilter = '__alreadyin';
                            rootElement.querySelectorAll('[data-status]').forEach(p => p.classList.remove('active'));
                            alreadyInPill.classList.add('active');
                            applyFilters();
                            updateResetBtn();
                        }
                    });
