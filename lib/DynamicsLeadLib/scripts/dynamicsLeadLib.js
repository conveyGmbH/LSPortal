...
(function () {
    ...
    // LSPortal's OData lead columns (PascalCase, e.g. "FirstName") are not
    // Dynamics' logical attribute names (lowercase, e.g. "firstname" —
    // required per DYN_DEFAULT_MAPPINGS in fieldMappingService.js). Unlike
    // HubSpot, a plain .toLowerCase() isn't enough here: several Dynamics
    // Lead attributes don't just lowercase the OData name (Company ->
    // companyname, Phone -> telephone1, Website -> websiteurl, Street/City/
    // PostalCode/Country -> address1_*). This table is the source-OData-key
    // equivalent of DYN_DEFAULT_MAPPINGS (kept in sync with it by hand).
    const ODATA_TO_DYNAMICS_FIELD = {
        FirstName: "firstname",
        LastName: "lastname",
        Company: "companyname",
        Email: "emailaddress1",
    ...
    // Dynamics' 'salutation' attribute is a picklist (an integer option
    // value), not free text — sending the OData "Salutation" string value
    // (e.g. "Mr.") as-is 400s ("An invalid value was specified"). Excluded
    // rather than mapped, unlike Salesforce/HubSpot where Salutation is a
    // free-text field this same source data slots into cleanly.
    const DYN_EXCLUDED_FIELDS = new Set(["Salutation"]);
    ...
    class DynamicsLeadLib {
        ...
        static buildContactData(itemData, fieldMappingService) {
            ...
            Object.keys(processedData).forEach((sourceField) => {
                ...
                if (DYN_EXCLUDED_FIELDS.has(sourceField)) return;
                ...
                const dynamicsField = ODATA_TO_DYNAMICS_FIELD[sourceField] || sourceField;
