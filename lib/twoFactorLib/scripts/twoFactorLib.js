...
(function () {
    ...
    async function createMainInterface(container, username, password, userData = {}) {
        try {
            ...
            const passwordToUse = currentSession.userPassword || password;
            console.log('🔧 Using password type:', currentSession.userPassword ? 'session/dbPassword' : 'original');
        ...
        `;
            ...
            const status = await apiRequest('POST', '/auth/status', { 
                username: username,
        ...
        `;
            ...
            await switchTab('overview', container, username, passwordToUse, status);
    ...
    async function loadDevicesTab(container, username, password, userData) {
        ...
        try {
            const devices = await apiRequest('POST', '/devices/list', { 
                username: username,
                password: password 
            });
                ...
                form.addEventListener("submit", async (e) => {
                  ...
                  try {
                    ...
                    if (currentSession.setupData && currentSession.setupData.sessionToken) {
                      requestData.sessionToken = currentSession.setupData.sessionToken;
                    ...
                    const verifyResult = await apiRequest("POST", "/auth/verify-2fa", requestData);

                    if (verifyResult.success) {
                      closeModal("tfa-setup-modal");

                    if (verifyResult.dbPassword) {
                        currentSession.userPassword = verifyResult.dbPassword;
                        console.log('🔧 DBPassword stored in session after setup');
                    }
    ...
    let TwoFactorLib = {
                                ...
                                if (result.success) {
                                   console.log('2FA verified successfully');                                  
                                    ...
                                    if (result.dbPassword) {
                                        currentSession.userPassword = result.dbPassword;
                                    }
        ...
        // Disabled 2FA 
        disable2FA: async function (username, password) {
            try {
                ...
                if (result.success) {
                    showSuccess('2FA disabled successfully! You can now login with your original password.');
        ...
        // Remove a device 
        removeDevice: async function (deviceId, deviceName, username, password) {
            try {
                if (!confirm(`Are you sure you want to remove the device "${deviceName}"?\n\nThis action cannot be undone.`)) {
                    return;
                }

                //  POST au lieu de DELETE
                const result = await apiRequest('POST', '/devices/remove', {
                    deviceId: deviceId,
                    username: username,
                    password: password,
                    confirmDelete: false
                ...
                if (error.message.includes('This is your last device')) {
                    if (confirm(`This is your last device. Removing it will disable 2FA completely.\n\nAre you sure you want to continue?`)) {
                        try {
                            ...
                            if (result.success) {
                                showSuccess('Last device removed and 2FA disabled successfully! You can now login with your original password.');
        ...
        `;
            ...
            const saveEdit = async () => {
                ...
                if (newName && newName !== currentName && newName.length >= 3) {
                    try {
                        // POST au lieu de PUT
                        const result = await apiRequest('POST', '/devices/rename', {
                            deviceId: deviceId,
                            username: username,
                            password: password,
        ...
        clear: function (root) {
            try {
                if (root && 'innerHTML' in root) {
                    ...
                    //  Simplifier le reset de session
                    currentSession = {
                        username: null,
                        sessionToken: null,
                        userPassword: null,
