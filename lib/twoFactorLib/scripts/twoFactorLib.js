...
async function apiRequest(method, endpoint, data = null) {
    ...
    if (currentSession.sessionToken) {
        options.headers['X-Session-Token'] = currentSession.sessionToken;
    }
    ...
    console.log('🌐 2FA API Request:', { method, url, hasData: !!data });
...
/* detectIfCurrentDevice corresponding to the current user agent */
        ...
        const interfaceHtml = `
            ...
            ">
                ...
                <div style="margin-top: 20px; padding: 10px; background: #f8f9fa; border-radius: 4px; font-size: 12px;">
                    ...
                    API: ${API_BASE_URL} | Status: ${status.is2FAEnabled ? 'Enabled' : 'Disabled'}
                </div>
            </div>
        `;
...
async function show2FASetup(username) {
    try {
        ...
        if (result.success) {
            ...
            currentSession.setupSessionToken = result.sessionToken;
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const verifyResult = await apiRequest('POST', '/auth/verify-2fa', {
                        sessionToken: currentSession.setupSessionToken,
                        totpCode: code,
                    ...
                    if (verifyResult.success) {
                        ...
                        if (verifyResult.sessionToken) {
                            currentSession.sessionToken = verifyResult.sessionToken;
                        }
...
let TwoFactorLib = {
    ...
    getStatus: function (root, username, setTokenPassword, language) {
        const promise = (async () => {
            try {
                console.log('TwoFactorLib.getStatus called for user:', username);
                injectCSS();
    ...
    verify2FA: function (root, username, setTokenPassword, language, sessionToken) {
        const promise = (async () => {
            try {

                console.log('TwoFactorLib.verify2FA called for user:', username, 'sessionToken:', sessionToken ? 'provided' : 'missing');
