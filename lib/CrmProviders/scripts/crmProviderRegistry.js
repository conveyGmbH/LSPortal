// Vanilla-JS equivalent of LSPortalNext's CrmAdapter/registry pattern
// (src/services/crm/registry.ts, src/hooks/useClientCrm.ts). One CRM is
// active per client license at a time.
//
...
(function () {
    ...
    // needs one round-trip per event and session.
    ...
    // Salesforce is the default so callers that can't resolve a provider
    ...
    // Resolution order: (1) explicit stored choice if valid, (2) else the
    ...
    // True once the client has explicitly picked/switched a provider.
    // Pre-existing Salesforce clients never did this, so callers use it to
    // skip the real checkConnection() gate for them.
    function hasExplicitProviderChoice() {
        try {
            var stored = localStorage.getItem(ACTIVE_PROVIDER_STORAGE_KEY);
            return !!stored && PROVIDER_IDS.indexOf(stored) !== -1;
        } catch (e) {
            return false;
        }
    }
    ...
    window.CrmProviders = {
        ...
        hasExplicitProviderChoice: hasExplicitProviderChoice,
