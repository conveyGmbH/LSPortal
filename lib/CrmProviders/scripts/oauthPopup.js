// oauthPopup.js - shared OAuth popup dance for CRM provider connect() flows
//
// Extracted from SalesforceLeadLib._sfConnectPopup(), which is otherwise
// identical between providers except for the auth URL and the
// window.postMessage success message type (SALESFORCE_AUTH_SUCCESS vs.
// HUBSPOT_AUTH_SUCCESS). Both SalesforceLeadLib and HubspotLeadLib's
// connect() call this instead of duplicating the popup-open-and-listen code.

(function () {
    "use strict";

    // Opens authUrl in a centered popup and resolves with the
    // "<successMessageType>" postMessage payload once received (or resolves
    // with null if the popup is closed without ever posting that message).
    // Centers on the BROWSER WINDOW, not the physical screen, so it lands
    // correctly on multi-monitor / RDP setups.
