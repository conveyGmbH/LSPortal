// Shared LS_LeadReport OData reader: reading the portal's own lead rows has
// nothing to do with which CRM provider is active, so this is shared rather
...
(function () {
    ...
    // Call once with the Portal Admin credentials before fetchLeadReportPage().
    ...
    async function callPortalODataAPI(endpoint) {
        ...
        try {
            ...
            // Bad control characters in data (e.g. Description/Question fields) break JSON.parse — strip and retry
    ...
    // OData only supports __next for the first 2 pages; $skip drives full pagination instead.
    ...
    // Columns shown: the export-status columns always, plus whichever
    // LS_LeadReport columns are marked active in fieldMappingService, falling
    // back to every column when no field config exists yet (brand-new event)
    // so the table isn't empty-looking before the client configures anything.
    ...
    function computeDisplayColumns(items, fieldMappingService) {
        ...
        // Re-add fields marked active in the saved config but absent from the
        // sampled row, or an active field could silently vanish from the table.
        if (hasFieldConfig) {
            var already = {};
    ...
    // OData /Date(ms)/ strings and ISO date strings in date-named columns
    // become YYYY-MM-DD; everything else is returned as-is.
